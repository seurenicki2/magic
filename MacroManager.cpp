...
#include "PartyWar/PWScoreInfoDialog.h"
#include "PartyWar/PWResultDlg.h"
...
void CMacroManager::KeyboardInput_Normal( BOOL bDock, CKeyboard* keyInfo )
{
	...
	if (keyInfo->GetKeyPressed(KEY_SPACE))
	{
		cDialog* dialog = WINDOWMGR->GetWindowForID(APPLY_OPTION_DIALOG);
		cDialog* enchantDialog = WINDOWMGR->GetWindowForID(ITD_UPGRADEDLG);
		cEditBox* EditChatdialog = GAMEIN->GetChattingDlg()->GetInputBox();
		ASSERT(dialog);
		ASSERT(enchantDialog);
		ASSERT(EditChatdialog);

		if (EditChatdialog->IsFocus() == FALSE)
		{
			if (dialog->IsActive() == TRUE && enchantDialog->IsActive() == TRUE)
			{
				CHATMGR->AddMsg(CTC_SYSMSG, "Seems that both Enchant and Philo dialogs are active, open only the one you need to use");
				return;
			}
			if (dialog->IsActive() == TRUE)
			{
				dialog->OnActionEvent(APPLY_OPTION_SUBMIT_BUTTON, 0, 0);
			}
			if (enchantDialog->IsActive() == TRUE)
			{
				enchantDialog->OnActionEvent(ITD_UPGRADE_OKBTN, 0, 0);			
			}
		}
	}

	//Alemuri Changed Switch Fly Walk and ViceVersa--------------------------------------------
	if( keyInfo->GetKeyPressed( KEY_CONTROL ) && keyInfo->GetKeyPressed( KEY_F ) )
	{
		if ((m_flyMode != 0) && (m_flyMode != 1) && (m_flyMode != 2))
		{
			m_flyMode = 0;
		}
		...
		int flyMode = m_flyMode;
		GAMEIN->switchFlyWalk( flyMode );
	}
	//--------------------------------------------------------------------------------------
	...
	//wasd 2
		...
		|| keyInfo->GetKeyUp( KEY_D ) )
		{
			...
			for( int i = 0 ; i < ME_COUNT ; ++i )
			{
				...
				if( bDock && !macro.bAllMode )								{		continue;				}
				else if(FALSE == (macro.nSysKey & nSysKey))		{		continue;				}
				else if( macro.bUp )	{
					if( keyInfo->GetKeyUp( macro.wKey ) )				{		PlayMacro( i );	}
				}
				else if( keyInfo->GetKeyDown( macro.wKey ) )	{		PlayMacro( i );	}
			}
		}
	}
	else {
		// if wasd is off, shortcut S for open Skill window
		if( keyInfo->GetKeyUp( KEY_S ) || keyInfo->GetKeyDown( KEY_S ) ){
			if( bDock) {
				if( (nSysKey & MSK_ALT) && keyInfo->GetKeyDown( KEY_S ) ){	PlayMacro( ME_TOGGLE_SKILLDLG ); }
			} else {
				if( keyInfo->GetKeyDown( KEY_S ) ){		PlayMacro( ME_TOGGLE_SKILLDLG ); }
			}
		}
		else if( keyInfo->GetKeyUp( KEY_D ) || keyInfo->GetKeyDown( KEY_D ) ){
			if( bDock) {
				if( (nSysKey & MSK_ALT) && keyInfo->GetKeyDown( KEY_D ) ){	PlayMacro( ME_TOGGLE_MATCHINGDLG ); }
			} else {
				if( keyInfo->GetKeyDown( KEY_D ) ){		PlayMacro( ME_TOGGLE_MATCHINGDLG ); }
			}
		}
		else if( keyInfo->GetKeyUp( KEY_W ) || keyInfo->GetKeyDown( KEY_W ) ){
			if( bDock) {
				if( (nSysKey & MSK_ALT) && keyInfo->GetKeyDown( KEY_W ) ){	PlayMacro( ME_TOGGLE_WORLDMAP ); }
			} else {
				if( keyInfo->GetKeyDown( KEY_W ) ){		PlayMacro( ME_TOGGLE_WORLDMAP ); }
			}
		}
		else {
			for( int i = 0 ; i < ME_COUNT ; ++i )
			{
				const sMACRO& macro = m_MacroKey[m_nMacroMode][i];
				if( macro.wKey == KEY_J || macro.wKey == KEY_K || macro.wKey == KEY_N ) { continue; }  // ignore shorcut J, K & N if wasd off
				if( bDock && !macro.bAllMode )								{		continue;				}
				else if(FALSE == (macro.nSysKey & nSysKey))		{		continue;				}
				else if( macro.bUp )	{
					if( keyInfo->GetKeyUp( macro.wKey ) )				{		PlayMacro( i );	}
...
void CMacroManager::KeyboardPressInput( CKeyboard* keyInfo )
{
 ...
 // ---- wasd custom
		...
		for( int i = 0 ; i < MPE_COUNT ; ++i )	{
			if( i != MPE_MOVE_UP && i != MPE_MOVE_DOWN && i != MPE_MOVE_RIGHT && i != MPE_MOVE_LEFT
			){
				if( m_MacroPressKey[m_nMacroMode][i].nSysKey & nSysKey && keyInfo->GetKeyPressed( m_MacroPressKey[m_nMacroMode][i].wKey ) )
				{
					PlayMacroPress( i );													// ¸AAⓒ·I¸| ½CCaCN´U.
				}
		}
	}
...
//  if( m_isWasd ) {
	...
	{
		if( keyInfo->GetNextKeyPressed( KEY_W ) )
		{
			m_nKeyMoveDir |= MOVE_UP_LEFT;//MOVE_UP_LEFT
		}
		else if( keyInfo->GetNextKeyPressed( KEY_S ) )
		{
			m_nKeyMoveDir |= MOVE_DOWN_LEFT;//MOVE_DOWN_LEFT
		}
		else
	...
	if( keyInfo->GetKeyPressed( KEY_D ))											
	{
		if( keyInfo->GetNextKeyPressed( KEY_W ) )
		{
			m_nKeyMoveDir |= MOVE_UP_RIGHT;//MOVE_UP_RIGHT
		}
		else if( keyInfo->GetNextKeyPressed( KEY_S ) )
		{
			m_nKeyMoveDir |= MOVE_DOWN_RIGHT;//MOVE_DOWN_RIGHT
		}
		else
	...
	{
		if( keyInfo->GetNextKeyPressed( KEY_A ) )
		{
			m_nKeyMoveDir |= MOVE_UP_LEFT;//MOVE_UP_LEFT
		}
		else if( keyInfo->GetNextKeyPressed( KEY_D ) )
		{
			m_nKeyMoveDir |= MOVE_UP_RIGHT;//MOVE_UP_RIGHT
		}
		else
...
{
#ifndef _FORMOON_
	//fix slot machine stuck
	CSlotDialog* SlotDlg = GAMEIN->GetSlotDlg();
	if(SlotDlg->IsActive())
	{
		CHATMGR->AddMsg( CTC_GMCHAT, "Press X button if SLOT MACHINE is Open" );
		return;
	}
#endif
...
void CMacroManager::PM_SetAutoTarget()
{
	...
	else
	{
			...
			++iterator)
		{
			...
			if( pTargetObject )
			{
				if (MAP->GetMapNum() == 55 || MAP->GetMapNum() == 54) //Alemuri in case the map is Nera Harbor deactivate Tab Target on Mobs
				{
						if( pTargetObject->GetObjectKind() != eObjectKind_Player && //johan set auto target player with KYE_TAB
						pTargetObject->GetObjectKind() != eObjectKind_Vehicle &&
						pTargetObject->GetObjectKind() != eObjectKind_Pet&& 
						pTargetObject->GetObjectKind() != eObjectKind_Monster)
					{
...
void CMacroManager::PM_Toggle_PWScoreDlg()
{
#ifdef _GMTOOL_
	CPWScoreInfoDialog* pScoreDlg = (CPWScoreInfoDialog*)WINDOWMGR->GetWindowForID(PWSCOREDLG) ;
	if(!pScoreDlg)		return;

	CPWResultDlg* pResultDlg = (CPWResultDlg*)WINDOWMGR->GetWindowForID(PWRESULTDLG) ;
	if(!pResultDlg)		return;
...
void CMacroManager::Init()
{
	...
	sMACRO DefaultMacro[MM_COUNT][ME_COUNT] = {
		...
		{			
			...
			{ MSK_NONE,		KEY_K,	FALSE, FALSE },									// 스킬창을 토글한다.
			...
			{ MSK_NONE,		KEY_N,	FALSE, FALSE },									// 빅맵을 토글한다.
			...
			{ MSK_NONE,		KEY_J,	FALSE, FALSE },		//wasd 2			// Toggle the matching window.												
			{ MSK_NONE,		KEY_B,	FALSE, FALSE },									// Toggle the friend window.
