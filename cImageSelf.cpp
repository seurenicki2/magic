...
BOOL cImageSelf::LoadSprite(char * szFilename, long width, long height)
{
	m_pSurface = g_pExecutive->GetRenderer()->CreateSpriteObject(szFilename, 0, 0, width, height, 0);
	if(!m_pSurface) 
	{
		return FALSE;
	}
	...
	m_srcImageRect.left = 0;
	m_srcImageRect.top = 0;
	m_srcImageRect.right = width;
	m_srcImageRect.bottom = height;
