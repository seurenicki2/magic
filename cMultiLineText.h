...
*/
#define MultiLineText USINGTON(cMultiLineText)
class cMultiLineText  
{
public:
	cMultiLineText();
	virtual ~cMultiLineText();
	void Init(WORD fontIdx, DWORD fgColor, cImage * bgImage=NULL, DWORD imgColor = 0xffffffff, BOOL bModified = FALSE);
	...
	void AddLine(const char* text, DWORD dwColor = 0xffffffff, DWORD wItemIdx = 0, int imageIndexArray = 1000);
...
EXTERNGLOBALTON(cMultiLineText)
