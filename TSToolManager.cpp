...
BOOL CTSToolManager::OnItemCommand()
{
	...
	{
		BOOL rt;
		MONEYTYPE dwMoney = GetDlgItemInt( m_hWndSub[eTSMenu_Item], IDC_TSITEM_EDT_MONEY, &rt, TRUE );
		if( !rt )	return FALSE;
				...
				case IDC_TSWEATHER_BTN_APPLY:
					{
						...
						CHATMGR->AddMsg( CTC_TOGM, "Weather On : %s[%d]", weatherTxt, nIntensity );
					}
					break;
				...
				case IDC_TSWEATHER_BTN_STOP:
					{
						...
						CHATMGR->AddMsg( CTC_TOGM, "Weather Off" );
					}
					break;
