...
void CExchangeManager::NetworkMsgParse( BYTE Protocol, void* pMsg )
{
	...
	case MP_EXCHANGE_REMOVE_NOTEITEM:		Exchange_Remove_NoteItem( pMsg ) ;		break;
	}
}
...
void CExchangeManager::Exchange_Inputmoney_Ack( void* pMsg )
{
	...
	GAMEIN->GetExchangeDialog()->InputMoney( 0, msg->dweData );
	GAMEIN->GetExchangeDialog()->SetDisable( FALSE );
}
...
{
	MSG_DWORDEX* msg = (MSG_DWORDEX*)pMsg;
	GAMEIN->GetExchangeDialog()->InputMoney( 1, msg->dweData );
}
...
{
	MSG_DWORDEX* msg = (MSG_DWORDEX*)pMsg;
	HERO->SetMoney( msg->dweData );	//+인벤토리
}
...
void CExchangeManager::Exchange_Remove_NoteItem( void* pMsg ) 
{
	...
	if( !pInvenDlg )
		return ;
	CItem * pItem = NULL ;
	pItem = pInvenDlg->GetItemForPos( pmsg->wAbsPosition ) ;
	pItem->SetLock(FALSE);

	ITEMMGR->DeleteItemofTable(ITEMMGR->GetTableIdxForAbsPos(pmsg->wAbsPosition), pmsg->wAbsPosition, FALSE ) ; //eItemTable_Inventory
	QUICKMGR->DeleteLinkdedQuickItem( pItem->GetDBIdx() ) ;

	if( pInvenDlg->IsActive() ) pInvenDlg->SetDisable( FALSE ) ;
	QUICKMGR->RefreshQickItem() ;

	//This is the routine when you discard an item from Inventory-----------
	//MSG_ITEM_DISCARD_SYN msg;
	//msg.Category = MP_ITEM;
	//msg.Protocol = MP_ITEM_DISCARD_SYN;
	//msg.dwObjectID = gHeroID;
	//msg.TargetPos = pmsg->wAbsPosition;
	//msg.wItemIdx = pItem->GetItemIdx();
	//msg.ItemNum = pItem->GetDurability();
