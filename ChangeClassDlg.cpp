...
void PressSubmitInMessageBox(LONG, LPVOID, DWORD windowEvent)
{
    ...
    {
        // Verificación de armas antes de cambiar de clase
        const ITEMBASE* weaponItem1 = ITEMMGR->GetItemInfoAbsIn( HERO, TP_WEAR_START + eWearedItem_Weapon );
        const ITEMBASE* weaponItem2 = ITEMMGR->GetItemInfoAbsIn( HERO, TP_WEAR_START + eWearedItem_Weapon2 ); // Segundo espacio de arma

        const ITEM_INFO* weaponItemInfo1 = (weaponItem1 ? ITEMMGR->GetItemInfo( weaponItem1->wIconIdx ) : 0);
        const ITEM_INFO* weaponItemInfo2 = (weaponItem2 ? ITEMMGR->GetItemInfo( weaponItem2->wIconIdx ) : 0);
        
        // Verifica si ambos espacios de armas están ocupados
        if (weaponItemInfo1 && weaponItemInfo2) 
        {
            CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 1590 ) ); // Mensaje de advertencia
            return;
        }

        // Verifica si el héroe está usando un escudo con tipo de arma
        ...
        // Si hay un escudo y tiene un tipo de arma, no se permite el cambio de clase
        if (shieldItemInfo && shieldItemInfo->WeaponType != 0) 
        {
            CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 1590 ) );
            return;
        }

        // Realiza el cambio de clase si no hay problemas con el equipo
...
void CChangeClassDlg::SettingClassList()
{
	...
	// °¢ Á¾Á·, Á÷¾÷º° ÀüÁ÷ Á¤º¸¸¦ »ó¼ö·Î Á¤ÀÇÇÑ´Ù. ÇöÀç 5Â÷ÀüÁ÷±îÁö Á¤ÀÇ.
	struct ClassInfo
	{
		WORD	wClassNo;				// Å¬·¡½º ¹øÈ£( InterfaceMsg¿¡ Á¤ÀÇ)
		BYTE	byStage;				// ÀüÁ÷´Ü°è(1 ~ 5)
		BYTE	byRace;					// Á¾Á·(0:°øÅë 1:ÈÞ¸Õ 2:¿¤ÇÁ)
	}
	const stClassInfo[eClass_MAXCLASS][MAX_CLASS_COUNT] = 
	{
		...
		},
		{
			...
			{395, 5, 1}, {396, 5, 2}, {1106,5, 0}, {397, 6, 0}, {398, 6, 1}, {399, 6, 2},
	...
	// ÀüÁ÷Á¤º¸¸¦ ¾ò±âÀ§ÇØ Ä³¸¯ÅÍ Á¤º¸¸¦ ¾ò´Â´Ù.
	...
	// ÇöÀç ÀüÁ÷ ·¹º§±îÁö¸¸ ÄÞº¸¹Ú½º·Î Ã³¸®ÇÑ´Ù.
			...
			stClassInfo[byClassType][nIndex].byStage <= ChaInfo.JobGrade )
	{
		...
		// ¸®½ºÆ®¿¡ Á÷¾÷¸íÀ» Ãß°¡ÇÏ°í, ÇöÀç ÀüÁ÷Á¤º¸¸¦ Ç¥½ÃÇÑ´Ù.
