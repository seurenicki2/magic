...
void cText::Render()
{
	...
	line_node * curLineNode = topLine;
	// 070209 LYW --- cText : Modified Render part.
	if( IsMovedWnd() )
	{
		SetRenderArea() ;
		SetMovedWnd( FALSE ) ;
	...
	while(curLineNode)
	{
		if(strcmp(curLineNode->line,"")!=0)
			CFONT_OBJ->RenderFont(m_wFontIdx,curLineNode->line,curLineNode->len,&m_renderRect,RGBA_MERGE(m_fgColor, m_alpha * m_dwOptionAlpha / 100));
		m_renderRect.top +=LINE_HEIGHT;
		curLineNode = curLineNode->nextLine;
	}
}

// 070209 LYW --- cText : Add function to setting area of render part.
void cText::SetRenderArea()
{
	m_renderRect.left	= (long)(m_absPos.x + m_textXY.x) ;
