...
BOOL CGraphicEngine::CreateExecutive(HWND hWnd,DISPLAY_INFO* pDispInfo,DWORD MaxShadowNum,DWORD ShadowMapDetail,float SightDistance,DWORD FPS)
{
	...
	if(g_bUsingEnginePack)
	{
		PACKFILE_NAME_TABLE nameTable[] = {
			{"data/interface/2dImage/image.gpk", 0},
			{"data/3DData/map.gpk", 0},
			{"data/3DData/effect.gpk",0},
			{"data/3DData/monster.gpk",0},
			{"data/3DData/npc.gpk",0},
			{"data/3DData/farm.gpk",0},
			{"data/3DData/character.gpk",0},
			{"data/3DData/galaxychar.gpk", 0 },
			{"data/interface/2dImage/npcimage.gpk", 0}
		};
	...
	else
	{
		...
		m_pExecutive->InitializeFileStorageWithoutRegistry("GlobalFS.dll",
			351057,102184,MAX_PATH,FILE_ACCESS_METHOD_ONLY_FILE,NULL,0);
	}

	pDispInfo->dwRefreshRate = 0;
	m_pExecutive->InitializeWithoutRegistry("GlobalG.dll","GlobalR.dll",
		hWnd, pDispInfo, 90045, 512,0,1, 0,MHErrorHandleProc); //90000	512 0 1 0 //90045 will be checked in Executive
	...
	m_pExecutive->SetFramePerSec(FPS);//FPS
	m_pExecutive->GetRenderer()->SetVerticalSync(FALSE);
	m_pExecutive->GetRenderer()->EnableSpecular(0); //0
	m_pExecutive->GetRenderer()->SetAlphaRefValue(5); //5
