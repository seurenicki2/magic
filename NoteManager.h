...
#define NOTEMGR CNoteManager::GetInstance()
...
public:
	...
	void WriteNote(char* Note, char* Title, char* ToName, DWORD itemdbidx, POSTYPE position, MONEYTYPE mPackmoney);
	//void WriteNote(char* Note, char* Title, char* ToName);
	void WriteNoteID(char* Note, DWORD TargetID);
	void DelNoteSyn(DWORD NoteID, BOOL bLast);
	void SaveNoteSyn(DWORD NoteID, BOOL bLast);
	void DelAllNoteSyn();
