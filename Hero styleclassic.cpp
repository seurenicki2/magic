...
DWORD CHero::DoGetAttackRate()
{
	...
	DWORD wWeaponAttack = DWORD(mItemBaseStat.mPhysicAttack.mPlus * (1.0f + mItemBaseStat.mPhysicAttack.mPercent));
	...
	float wStrengthAll = (mCharStat.mStrength.mPlus + mItemBaseStat.mStrength.mPlus + mItemOptionStat.mStrength.mPlus + mItemOptionStat.mPhysicAttack.mPlus + GetBuffStatus()->Str) * (1.0f + mItemBaseStat.mStrength.mPercent);

	// ÈûÀ» ¾òÀ» ¶§ ¼¼Æ® ´É·Â Àû¿ëÇÑ ¼öÄ¡¸¦ ¾ò¾î³½´Ù
	{
...
DWORD CHero::DoGetDefenseRate()
{
	...
	{
		//salta monk, inquirer, soul arbiter e necromancer

		//char buffer[32];
		//sprintf(buffer, "%d", m_CharacterInfo.Level); 
			...
			(pGlove && pGlove->ArmorType == eArmorType_Leather))
		{
			...
			if (m_CharacterInfo.Job[2] == 3)
			{
				goto skiparmor;
			}
			if (m_CharacterInfo.Job[3] == 3)
			{
				goto skiparmor;
			}
			if (m_CharacterInfo.Job[4] == 2)
			{
				goto skiparmor;
			}
			if (m_CharacterInfo.Job[4] == 4)
			{
				goto skiparmor;
			}
	...
	float wDefenseBuffVal = fPlus + fPercent;

	float fDefense = (float)fDefenseAll / 2;
	float fDefenseA = fDefense + DEnchant + wLevel + wDefenseBuffVal;

	float fValA = (((float)fVitality - 20) * 10);
...
DWORD CHero::DoGetMagAttackRate()
{
	...
	//float fIntelligenceAll	= ( mCharStat.mIntelligence.mPlus + mItemBaseStat.mIntelligence.mPlus +GetBuffStatus()->Int) * ( 1.0f + mItemBaseStat.mIntelligence.mPercent );
	//{
	//	const PlayerStat::Value& value = GetSetItemStats().mIntelligence;

	//	fIntelligenceAll =	
	//		DWORD( max( 0, ( fIntelligenceAll + value.mPlus ) * ( 1.0f + value.mPercent ) ) )  +
	//		GetPassiveStatus()->Int+
	//		GetBuffStatus()->Int;
	//}

	fIntelligenceAll *= 0.2f;
	// ¼ø¼ö Áö´É
	...
	float wAttackBuffVal = fPercent + fPlus;

	float fAttackA = (float)(fMagicAttack + AEnchant + wLevel + wAttackBuffVal);

	float fValA = ((float)fIntelligenceAll - 10) * 35;
	...
	return DWORD(max(0, Round(dwTotalAttack, 1)));
...
DWORD CHero::DoGetMagDefenseRate()
{
	...
	float wDefenseBuffVal = fPercent + fPlus;

	float fDefense = (float)fDefenseAll / 5;
	float fDefenseA = fDefense + DEnchant + wLevel + wDefenseBuffVal;

	float fValA = ((float)fWisdomAll - 20) * 15;
	...
	float fTotalDefense = GetFloatRound(fResult);
...
DWORD CHero::DoGetAccuracyRate()
{
	...
	float fValA = ((float)fDexterityAll / 1.4f); //it was 2
	float fValC = ((float)fStrengthAll / 4);
	float fValB = (float)(fValA + wLevel + fClassOrderVal + fValC);

	float wAccuracy = GetFloatRound(fValB);
	float fTotalAccuracy = wAccuracy + fPercent + fPlus;

	//{
	//	const PlayerStat::Value& value = GetSetItemStats().mAccuracy;

	//	fTotalAccuracy = max( 0, ( fTotalAccuracy + value.mPlus ) * ( 1.0f + value.mPercent ) );
	//}
	...
	//	fTotalAccuracy = ( fTotalAccuracy + value.mPlus ) * ( 1.0f + value.mPercent );
	//}

	return DWORD(fTotalAccuracy);
	// ¸íÁß					= ( ( ( ¹ÎÃ¸ - ÃÖÃÊ ¹ÎÃ¸ - 5 ) / ( ·¹º§* 5 ) ) * 0.6f + (  ( ÃÖÃÊ ¹ÎÃ¸ - 30 ) / 1000 ) + ( ·¹º§ * ¸íÁß»ó¼ö / 15000 ) * 100 )
...
DWORD CHero::DoGetEvasionRate()
{
	...
	float fValA = ((float)fDexterityAll / 2);
	float fValB = (float)(fValA + wLevel + fClassOrderVal);

	float wEvasion = GetFloatRound(fValB);

	float dwTotalEvasion = wEvasion + fPercent + fPlus;
	...
	// È¸ÇÇ					= ( ( ( ¹ÎÃ¸ - ÃÖÃÊ ¹ÎÃ¸ ) / (  ( ·¹º§ - 1 ) * 5 ) ) * 0.6f + (  ( ÃÖÃÊ ¹ÎÃ¸ - 30 ) / 1000 ) + ( ·¹º§ * ¸íÁß»ó¼ö / 15000 ) * 100 )
...
DWORD CHero::DoGetCriticalRate()
{
	...
	// 080313 LUJ, 아이템 스탯을 일반과 세트로 분리
	{
		...
		dwCritical = (dwCritical + mItemOptionStat.mCriticalRate.mPlus + value.mPlus) * (1.0f + value.mPercent);
	}
