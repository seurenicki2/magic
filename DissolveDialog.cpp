...
{
	checkSlotPosition = 0; //Alemuri check position
}
...
void CDissolveDialog::OnActionEvent(LONG id, void * p, DWORD we)
{
	...
	case DIS_CANCELBTN:
		{
			Clear();
			checkSlotPosition = 0; //Alemuri check position reset
			break;
		}
	}
}
...
void CDissolveDialog::SetActive(BOOL isActive)
{
	...
	{
		CHATMGR->AddMsg( CTC_OPERATEITEM, "Destruct is disable at this map" );
		return;
	}
...
{
	//Alemuri check position---------------------------------------
	if (checkSlotPosition == 17)
	{
		CHATMGR->AddMsg(CTC_SYSMSG, "You cannot add more items");
		return;
	}
	//-------------------------------------------------------------
	// 완료된 결과가 있을 경우 초기화시킨다
	...
	WORD wEmptyPos = 18; //Alemuri Dissolve Fix (18 is the first number out of range)
	for(WORD cellIndex = 0; mSourceDialog->GetCellNum() > cellIndex; ++cellIndex)
	{
		CItem* const item = (CItem*)mSourceDialog->GetIconForIdx(
			cellIndex);
	...
	checkSlotPosition = wEmptyPos; //Alemuri check position
