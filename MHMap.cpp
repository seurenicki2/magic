...
void CMHMap::InitMap(MAPTYPE MapNum)
{
	...
	*/
	...
	DIRECTORYMGR->SetLoadMode(eLM_Map);
	{
		...
		SAFE_DELETE(sky);
		if(mapDesc->bSky)	//Alemuri it was commented
		{
			//LOGFILE("sky");
			sky = new CEngineSky;
...
/*
	if(GAMERESRCMNGR->m_GameDesc.bShadow == MHSHADOW_DETAIL)	// JOHAN CEK
	{
		VECTOR3 pos;
...
{	
	//Alemuri Map 11 forced to be 20 to avoid 11 files random crashes 
	if (MapNum == 11)
		MapNum = 20;
	//----------------------------------------------------------

	char mapdescfile[256]; //Alemuri 256
	CMHFile file;
	sprintf(mapdescfile,"data\\script\\mapset\\MAP%d.bmhm",MapNum);
	if(file.Init(mapdescfile,"rb") == FALSE)
		return FALSE;
	
	char value[64] = {0,}; //Alemuri 64
	SAFE_DELETE(mapDesc);
	mapDesc = new MAPDESC;
	...
	else if (MAINGAME->GetCurStateNum() == eGAMESTATE_TITLE)
	{
		...
		switch (MapNum)
		{
			case 20:
			{
				strcpy(mapDesc->MapFileName, "20.map"); //11.map
			} break;
			case 21:
			{
				mapDesc->bSky = 1;
	...
	else// (MAINGAME->GetCurStateNum() != eGAMESTATE_TITLE && MAINGAME->GetCurStateNum() != eGAMESTATE_CHARSELECT && MAINGAME->GetCurStateNum() != eGAMESTATE_CHARMAKE)
	{
		switch (MapNum){
			case Alker_Harbor:{
				if (st.wHour >= 7 && st.wHour < 12)
				{
					mapDesc->bSky = 1;
					strcpy(mapDesc->SkyMod, "19_sky.mod");//19_sky.MOD
			/*		mapDesc->fogdesc.bEnable = TRUE;
					mapDesc->fogdesc.dwColor = 0xAFAFAFAA;
					mapDesc->fogdesc.fDensity = 0.5f;
					mapDesc->fogdesc.fStart = 4000;
					mapDesc->fogdesc.fEnd = 16000;
					mapDesc->backColor = 0xAFAFAFAA;*/
					mapDesc->BGMSoundNum = 1521;
				}
				else if (st.wHour >= 12 && st.wHour < 18)
				{
					mapDesc->bSky = 1;
					strcpy(mapDesc->SkyMod, "sky_dusk(ok)_03.mod");
					mapDesc->fogdesc.bEnable = FALSE;
					mapDesc->backColor = 0xFFFFFFFF;
					mapDesc->BGMSoundNum = 8;
				}
				else if (st.wHour >= 18 || st.wHour < 7)
				{
					mapDesc->bSky = 1;
					strcpy(mapDesc->SkyMod, "sky_13night_wha.mod");//sky_54_nanju.mod
					strcpy(mapDesc->MapFileName,"20_night.map");//sky_54_nanju.mod
					//mapDesc->fogdesc.bEnable = TRUE;
			...
			case The_Town_of_Nera_Castle:{
								  ...
								  else if (st.wHour >= 18 || st.wHour < 7)
								  {
									  ...
									  strcpy(mapDesc->MapFileName,"52_night.map");//sky_54_nanju.mod
							/*		  mapDesc->fogdesc.bEnable = TRUE;
									  mapDesc->fogdesc.dwColor = 0x00000000;
									  mapDesc->fogdesc.fDensity = 5.5f;
									  mapDesc->fogdesc.fStart = 100;
