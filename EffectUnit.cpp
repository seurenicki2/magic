...
{
	if(NULL == m_pEffectUnitDesc)
		return FALSE;
	else if(m_bInited == FALSE && m_pEffectUnitDesc->IsDangledToOperator() == FALSE)
		return FALSE;
	...
	{
		if (pObject[n].pTarget != 0) //Alemuri Raid Boss Crash
		{
...
BOOL CEffectUnit::Attach(EFFECTPARAM* pParam,char* AttachPartName,BOOL EnableScale/* = FALSE*/)
{
	if(m_bInited == FALSE && m_pEffectUnitDesc->IsDangledToOperator() == FALSE)
		return FALSE;
	...
	for(int n=0;n<objectnum;++n)
	{
		if (pObject[n].pTarget != 0) //Alemuri Raid Boss Crash
		{
			if(pObject[n].pTarget->IsInited() == FALSE)	// 캐릭터가 존재하지 않으면 return
				continue;
			if( pObject[n].pTarget == HEROPETBACKUP )
				continue;
			m_nCurTargetNum = n;
			DoAttach(pParam,&pObject[n],AttachPartName,EnableScale);
		}
	}
	return TRUE;
}
