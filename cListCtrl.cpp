...
cListCtrl::cListCtrl()
{
	...
	ZeroMemory(m_nRowsMsgID, sizeof(m_nRowsMsgID));
	// E 데이트 존 추가 added by hseos 2007.11.14

	// 080326 LYW --- cListCtrl : Initialize a start index of printed items.
	m_wStartIdxOfPrintedItems = 0 ;
}

cListCtrl::~cListCtrl()
{
	for(	std::list< cWindow* >::iterator it = m_childList.begin();
			m_childList.end() != it;
			++it )
	{
		SAFE_DELETE( *it );
...
void cListCtrl::Render()
{
	...
	for(int l = 0 ; l < m_Cols ; l++)
	{
		...
		if( pColumnSize[l] == 0)
			_asm int 3;
//		리스트 컨트롤 ui 스크립트에서 #CREATE 에 집어넣은 숫자가 #COLS 에 집어넣은 숫자보다 크면 이런 일이 일어난다.
//		스크립트 체크																											2007/03/27 - 이진영
#endif
	...
	while (pos)
    {
        RITEM * ritem = m_pItemList.GetNextPos(pos);
		if(i < m_wLinePerPage)
		{
...
{
	for(WORD i = 0 ; i < m_wLinePerPage ; ++i)
	{
		if( m_absPos.x < x 
...
void cListCtrl::Add( cWindow* window )
{
	ASSERT( window && "It's 0" );

	m_childList.push_back( window );
}