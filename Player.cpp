...
void CPlayer::Process()
{
	...
	if(m_verifystate && m_verifyTimer >= 0)
	{
		if(m_verifyTime+1000 < gCurTime)
		{
			m_verifyTime = gCurTime;
			...
			if(m_verifyTimer > 0)
			{
				...
				if(ChaptchaDlg)
				{
					ChaptchaDlg->SetTimer(m_verifyTimer);
				}
...
void CPlayer::SetMotionInState(EObjectState State)
{
	...
	case eObjectState_Move:
		{
			//Alemuri BGM Sound Track Dragonya Change------------------------------------------------------------
			int currentMap = HERO->GetCharacterTotalInfo()->CurMapNum;
			if (currentMap == 74)
			{
				VECTOR3 heroPos;
				GetPosition(&heroPos);
				int x = heroPos.x /100; //255
				int y = heroPos.y;
				int z = heroPos.z /100; //375

				if (x > 255 && z > 375 && x < 420)
				{
					if (m_currentBgm != 1526)
					{
						AUDIOMGR->PlayBGM(1526);
						m_currentBgm = 1526;
						CHATMGR->AddMsg(CTC_OPERATOR, "You have entered the lair of the beast"); //CHATMGR->GetChatMsg(5123)
						dangerousArea = TRUE;
					}
				}
				else
				{
					if (m_currentBgm != 1245)
					{
						AUDIOMGR->PlayBGM(1245);
						m_currentBgm = 1245;
						if (dangerousArea == TRUE)
						{
							CHATMGR->AddMsg(CTC_OPERATOR, "You left the dangerous area");
							dangerousArea = FALSE;
						}
					}
				}
			...
			//Switch Fly Walk--------------------------------------------------------------------------------
			CVehicle* const vehicleObject = (CVehicle*)OBJECTMGR->GetObject(GetVehicleID());

			if ((vehicleObject == 0) || ((vehicleObject != 0) && (!vehicleObject->IsGetOn(this))))
			{
				if ((GetFlyMode() != 0) && (GetFlyMode() != 1) && (GetFlyMode() != 2))
				{
					SetFlyMode(0);
				}
				if (this->GetObjectName() == HERO->GetObjectName())
				{
					if (GAMEIN->heroFlyMode == 1)
					{
						if (m_MoveInfo.MoveMode == eMoveMode_Run)
						{
							m_Move_Start_Motion = 26;
							m_Move_Ing_Motion = 26;
						}
						else
						{
							m_Move_Start_Motion = 11;
							m_Move_Ing_Motion = 11;
						}
					}
					else if (GAMEIN->heroFlyMode == 2)
					{
						m_Move_Start_Motion = 199;
						m_Move_Ing_Motion = 199;
					}
					else if (GAMEIN->heroFlyMode == 0)
					{
						if (m_MoveInfo.MoveMode == eMoveMode_Run)
						{
							m_Move_Start_Motion = CHARACTER_MOTION[eCharacterMotion_Run][WeaponEquipType];
				...
				else
				{
					...
					else if (GetFlyMode() == 2)
					{
						m_Move_Start_Motion = 199;
						m_Move_Ing_Motion = 199;
					}
					else if (GetFlyMode() == 0)
					{
						if (m_MoveInfo.MoveMode == eMoveMode_Run)
						{
							m_Move_Start_Motion = CHARACTER_MOTION[eCharacterMotion_Run][WeaponEquipType];
...
{	
	if(pAttacker)
	{
		...
		if( HERO )
		if( pAttacker == HERO && HERO->IsPKMode() && !this->IsPKMode() && MAP->GetMapNum() != PVP )
//		if( GUILDFIELDWAR->GetEnemyGuildIdx() != 0 )
		{
			PKMGR->SetPKContinueTime( PKMGR->GetPKContinueTime() + 10*60*1000 );
			//CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 1239 ), 10 );	//---KES PK 071124	//Alemuri Party War (deactivated message, annoying in party war)
			//msg 악행으로 살기 지속시간이 추가로 60초만큼 늘어났습니다.
		}
	}

	//---KES PK 071124
	if( this == HERO && HERO->IsPKMode() && MAP->GetMapNum() != PVP )	//죽으면 PK시간 감소
	{
		//---죽음에 대한 PK모드 지속시간 감소
		DWORD dwPKContinueTime = PKMGR->GetPKContinueTime();
		//if( dwPKContinueTime == 0 ) return; //Alemuri Deactivated this for the animation death bug in PK mode
...
void CPlayer::RemoveEffects()
{
	...
	EFFECTMGR->ForcedEndEffect( mSelfEffect ) ;
	EFFECTMGR->ForcedEndEffect( mMarryPlayerEffect ) ;
...
//-------------------------------------------------------------------------------------------------
//	NAME		: StartMarryEffect
//	DESC		: Start Marry Effect when Marriage nearby
//	PROGRAMMER	: Death
//	DATE		: February 18, 2014
//-------------------------------------------------------------------------------------------------
void CPlayer::StartMarryEffect(CPlayer* pMarriagePlayer)
{
	// 활성화 중인 직업 이펙트가 있으면 종료한다.
	EndMarryEffect() ;
	mSelfEffect			= EFFECTMGR->StartPlayerEffectProcess( this ,MARRYEFFECT) ;
	mMarryPlayerEffect	= EFFECTMGR->StartPlayerEffectProcess( pMarriagePlayer ,MARRYEFFECT) ;
}

//-------------------------------------------------------------------------------------------------
//	NAME		: EndJobEffect
//	DESC		: Start Marry Effect when Marriage Out of Grid
//	PROGRAMMER	: Death
//	DATE		: February 18, 2014
//-------------------------------------------------------------------------------------------------
void CPlayer::EndMarryEffect()
{
	if( mSelfEffect )
	{
		EFFECTMGR->ForcedEndEffect( mSelfEffect ) ;
	}
	if( mMarryPlayerEffect )
	{
		EFFECTMGR->ForcedEndEffect( mMarryPlayerEffect ) ;
	}
}
...
void CPlayer::SetFlyMode(int flyMode)
{
	m_flyMode = flyMode;
}