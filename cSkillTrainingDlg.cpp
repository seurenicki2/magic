...
void cSkillTrainingDlg::AddSkillToSkillListDlg(BYTE byJobType, WORD wJobLevel, WORD* pPos)
{
	...
	//Alemuri Charinfo Dialog open in background when learning skill (Fix Attack Speed Bug)
	cDialog* charDgl = (cDialog*)WINDOWMGR->GetWindowForID(CI_CHARDLG);
	if (!charDgl)
		return;
	...
	{
		charDgl->SetAlpha(0);
		charDgl->SetActive(TRUE);
		charDgl->SetActive(FALSE);
		charDgl->SetAlpha(255);
	}
	//-------------------------------------------------------------------------------------
}
