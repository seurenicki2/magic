...
void cBalloonOutline::InitChatBalloon()
{
	...
	SCRIPTMGR->GetImage(20, &m_PlayerChatImage[eCHATBALLOON_LEFT1]); //balloon 1
	SCRIPTMGR->GetImage(21, &m_PlayerChatImage[eCHATBALLOON_CENTER1]);
	SCRIPTMGR->GetImage(22, &m_PlayerChatImage[eCHATBALLOON_RIGHT1]);

	SCRIPTMGR->GetImage( 66, &m_PlayerChatImage[eCHATBALLOON_LEFT2] );//balloon 2
	SCRIPTMGR->GetImage( 67, &m_PlayerChatImage[eCHATBALLOON_CENTER2] );
	SCRIPTMGR->GetImage( 68, &m_PlayerChatImage[eCHATBALLOON_RIGHT2] );

	SCRIPTMGR->GetImage( 69, &m_PlayerChatImage[eCHATBALLOON_LEFT3] );//balloon 3
	SCRIPTMGR->GetImage( 70, &m_PlayerChatImage[eCHATBALLOON_CENTER3] );
	SCRIPTMGR->GetImage( 71, &m_PlayerChatImage[eCHATBALLOON_RIGHT3] );

	//SCRIPTMGR->GetImage(20, &m_PlayerChatImage[eCHATBALLOON_LEFT4]); //balloon 4
	//SCRIPTMGR->GetImage(21, &m_PlayerChatImage[eCHATBALLOON_CENTER4]);
	//SCRIPTMGR->GetImage(22, &m_PlayerChatImage[eCHATBALLOON_RIGHT4]);
...
void cBalloonOutline::RenderPlayerChatBalloon( int Sx, int Sy, int TailX, int nWidth, int nLineNum, int nBorder, int nAlpha )
{
	...
	vScale.y = 1.f;
	//i did some changes here load tail first so tail will be back layer
	//tail first
	//채팅꼬리
	vPos.x = (float)TailX;
	...
	//CObject* pObject = OBJECTMGR->GetObject(gHeroID);
	//if (objectType == 1)
	//{
	//	//use this for temporary
	//	m_PlayerChatImage[eCHATBALLOON_LEFT4].RenderSprite(NULL, NULL, 0, &vPos, RGBA_MERGE(0xffffff, nAlpha));
	//	vPos.x += nMiddleStartPos;
	//	m_PlayerChatImage[eCHATBALLOON_CENTER4].RenderSprite(&vScale, NULL, 0, &vPos, RGBA_MERGE(0xffffff, nAlpha));
	//	vPos.x += nWidth;
	//	m_PlayerChatImage[eCHATBALLOON_RIGHT4].RenderSprite(NULL, NULL, 0, &vPos, RGBA_MERGE(0xffffff, nAlpha));
	//}
	//else
	//{
	...
	{
		m_PlayerChatImage[eCHATBALLOON_LEFT2].RenderSprite(NULL, NULL, 0, &vPos, RGBA_MERGE(0xffffff, nAlpha));
		vPos.x += nMiddleStartPos;
		m_PlayerChatImage[eCHATBALLOON_CENTER2].RenderSprite(&vScale, NULL, 0, &vPos, RGBA_MERGE(0xffffff, nAlpha));
		vPos.x += nWidth;
		m_PlayerChatImage[eCHATBALLOON_RIGHT2].RenderSprite(NULL, NULL, 0, &vPos, RGBA_MERGE(0xffffff, nAlpha));
	}
	*/
	////채팅꼬리
	//vPos.x = (float)TailX;
