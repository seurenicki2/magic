...
{
	// 100607 ONS 최초 접속시만 카메라 x축각도와 거리를 초기화한다.
...
void CMHCamera::GXOAlphaProcess()
{
	...
	if( ElapsedTime >= 50 )	//---너무 자주 호출 되는 것을 방지
	{
		...
		POINT pt = { GAMERESRCMNGR->m_GameDesc.dispInfo.dwWidth/2, GAMERESRCMNGR->m_GameDesc.dispInfo.dwHeight/2 };	//---현재 해상도의 중심점
		DWORD dwPickNum = g_pExecutive->GXOGetMultipleObjectWithScreenCoord( gxoDesc, 10, &pt, 0, /*PICK_TYPE_PER_FACE*/PICK_TYPE_PER_COLLISION_MESH); //---속도 우선
		...
		{
			if( gxoDesc[i].fDist + 300.f < distCamera )	//--- 속도향상(PICK_TYPE_PER_COLLISION_MESH)을 하였기 때문에, 원치 않는 결과가 나올 수 있다. 300.f의 버퍼를 두자.
			{
				if(gxoDesc[i].gxo)
				{
					CEngineObject* pEngineObject = (CEngineObject*)g_pExecutive->GetData(gxoDesc[i].gxo);

					if( pEngineObject == NULL )	//map object의 경우엔 EngineObject가 없군
					{
						...
						if ( setAlpha > 100 )
						{
							...
							if( m_AlphaHandleList.Find(gxoDesc[i].gxo) == NULL )	//---리스트에 없다면 리스트에 추가.
							{
...
void CMHCamera::CalcCameraPosAngle()
{
	...
	VECTOR3 pos;
	// 091109 ShinJS --- 탈것에 탑승중인 경우 캐릭터의 특정 본의 위치를 카메라의 위치로 설정
	if(pCurCamera->m_bPivotObject)
	{
		pCurCamera->m_pPivotObject->GetPosition(&pos);

		// 탈것에 탑승중인 경우
		// 091215 ShinJS --- 탈것 탑승시 실행중인 OnceFollowView 모드가 아닌 FollowView 모드 인 경우 본의 위치로 설정
...
#define MOUSEROTATE_RATE 0.3f //0.5f //johan sbelumx 0.3f
...
{
	// 100607 ONS 게임중 맵이동시의 카메라 x축 초기화를 막는다.
	if( !m_bIsFirstTime )
	{
		return;
	}
...
//			}
			...
			// 091215 ShinJS --- 탈것에 탑승중인 경우 && OnceFollowView 가 아닌경우 실행하지 않는다.
			if( ! m_bExecuteOnceFollowView &&
...
{
	// Render 루틴실행중 제거시 에러가 발생하므로 제거목록에 추가후 Hide처리하여 다음프레임에 제거되도록 한다.
	CameraFilterObjectMap& cameraFilterObjectMap = m_mapCameraFilter[ dwFilterIndex ];
	CameraFilterObjectMap::iterator cameraFilterObjectIter = cameraFilterObjectMap.find( dwObjectIndex );
	if( cameraFilterObjectIter == cameraFilterObjectMap.end() )
		return;
...
BOOL CCameraFilterObject::Init( DWORD dwObjectIndex, const stCameraFilterInfo& filterInfo, BOOL bNoRepeat, BOOL bFadeIn, float fFadeTime, DWORD dwRemainTime, eCAMERA_FILTER_ACTION eAction )
{
	...
	else
	{
		...
		// 비율 계산(필터의 기본 Size와 현재 화면 Size를 비교하여 맞추어 준다)
	...
	// 원하는 시간(초)에 맞는 FadeAlphaValue 설정
...
void CCameraFilterObject::Process()
{
	...
	{
		m_dwRemainTime = 0;
		// Fade 처리가 된경우 FadeOut으로 제거되도록 한다.
...
//wasd movement//Beyond SEA2//beyond
...
//	VECTOR3 CameraPos = pCurCamera->m_CameraPos;
	...
	if(Forward.x > 0.5f)
	{
		...
		{
			//diPos.x = Forward.z *-1;
			//diPos.z = Forward.z *-1;//Forward.z *-1;
			diPos.x = Forward.x *-1;
			diPos.z = Forward.x;//Forward.z *-1;
		}
		else
		{
		diPos.x = Forward.x *-1;
	...
	else if(Forward.z > 0.5f)
	{
		if(calcv > 0)
		{
			//diPos.x = Forward.x *-1;
			//diPos.z = Forward.x;//Forward.z *-1;
			diPos.x = Forward.z *-1;//Forward.x *-1;
			diPos.z = Forward.z *-1;
		}
		else
		{
	...
	//diPos.z = 1;
	HERO->GetPosition(&HeroPos);
	VECTOR3 RightVector = diPos; //HeroPos - CameraPos - 
	RightVector.y = 0.f;
