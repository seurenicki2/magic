...
void CConsignmentMgr::NetworkMsgParse(BYTE Protocol,void* pMsg)
{
	...
	case MP_CONSIGNMENT_REGIST_ACK:
		{
			MSG_DWORD5_AUC* pmsg = (MSG_DWORD5_AUC*)pMsg;
	...
	case MP_CONSIGNMENT_UPDATE_ACK:
		{
			...
			case eConsignmentDBResult_Buy_Ack:
				{
					//CHATMGR->AddMsg(CTC_SYSMSG, CHATMGR->GetChatMsg(2252), pmsg->szItemName, AddComma(dwePrice) );
					CHATMGR->AddMsg(CTC_SYSMSG,"%s purchased for %s gold.", pmsg->szItemName, AddComma(pmsg->dwTotalPrice) );
