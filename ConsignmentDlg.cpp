	...
	case eConsignment_Mode_Regist:
		{
			if( m_bActive )
			{
				...
				if( money )
				{
					color = DWORD(GetMoneyColor((DWORD)money));
...
void CConsignmentDlg::SetActive( BOOL val )
{
	...
	{
		CHATMGR->AddMsg( CTC_OPERATEITEM, "Auction is disable at this map" );
		val = FALSE;
	}
...
void CConsignmentDlg::SendRegistMsg()
{
	...
	else if(dwUnitPrice > MAXMONEY2B)//MAX_INVENTORY_MONEY)
	{
		CHATMGR->AddMsg(CTC_SYSMSG, "Max total item price 2,000,000,000");//CHATMGR->GetChatMsg(2279) );
		return;
	}
	DWORD duritemx = m_RegistIcon->GetDurability();
	MONEYTYPE totpricex = duritemx * dwUnitPrice;
	if(dwUnitPrice > MAXMONEY2B || totpricex > MAXMONEY2B)
	{
		CHATMGR->AddMsg(CTC_SYSMSG, "Max total item price 2,000,000,000" );
		return;
	}
	...
	m_dwLastRegistItemUnitPrice = (DWORDEX)dwUnitPrice;
...
DWORDEX CConsignmentDlg::GetBuyPrice(DWORD dwConsignmentIndex)
{
	int nCount = -1;
...
void CConsignmentDlg::OnBuyItem( LONG iId, void* p, DWORDEX param1, void * vData1, void* vData2 )
{
	...
	if (param1 > 1000)
	{
		...
		maxbuyitem = DWORD(param1);
	}
	pDlg->m_dwBuyDurability = maxbuyitem;
