...
void CNoteManager::NetworkMsgParse(BYTE Protocol,void* pMsg)
{
	...
	case MP_NOTE_GETPACKAGE_NOTIFY:
		{
			MSG_DWORD4_N2* pmsg = (MSG_DWORD4_N2*)pMsg;
	...
	case MP_NOTE_SENDNOTE_NACK:
		{
			...
			if(pmsg->wData == 2) //invalid user
				CHATMGR->AddMsg( CTC_BUYITEM,"Invalid Recipient");//936
			else if(pmsg->wData == 3) //full list
				CHATMGR->AddMsg( CTC_BUYITEM, CHATMGR->GetChatMsg(937), pmsg->Name );
		}
		break;
	case MP_NOTE_SENDNOTE_SYN_NACKMAP:
		{
			...
			case 1:
				{
					CHATMGR->AddMsg( CTC_CHEAT_1, CHATMGR->GetChatMsg(324));
				} break;
			case 2:
				{
					CHATMGR->AddMsg( CTC_CHEAT_1, "Item error1!");
				} break;
			case 3:
				{
					CHATMGR->AddMsg( CTC_CHEAT_1, "Item error2!");
				} break;
			case 4:
				{
					CHATMGR->AddMsg( CTC_CHEAT_1, "Item error3!");
				} break;
			case 5:
				{
		...
		break;
	case MP_NOTE_SENDNOTE_SYN_MAPSUKSES:
		{
			MSG_WORD* pmsg = (MSG_WORD*)pMsg;
			if (pmsg)
			{
				CHATMGR->AddMsg( CTC_BUYITEM, "Note sent!");
			}
		}
		break;
	}
...
void CNoteManager::WriteNote(char* Note, char* Title, char* ToName, DWORD itemdbidx, POSTYPE position, MONEYTYPE mPackmoney)
//void CNoteManager::WriteNote(char* Note, char* Title, char* ToName)
{
	//MSG_FRIEND_SEND_NOTE msg;
	if (mPackmoney > MAXMONEY2B)
	{
		return;
	}

	MSG_FRIEND_SEND_NOTETOMAP msg;
	msg.Category = MP_NOTE;
	msg.Protocol = MP_NOTE_SENDNOTE_SYN_TOMAP;
	msg.FromId = HEROID;
	msg.Nitemdbidx = itemdbidx;
	msg.PCposition = position;
	msg.MoneyNote = mPackmoney;
