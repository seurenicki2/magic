...
//aziz font tahoma
	...
	font.lfUnderline = 0;
	font.lfStrikeOut = 0;
	font.lfCharSet = DEFAULT_CHARSET;//DEFAULT_CHARSET; //THAI_CHARSET;
	font.lfOutPrecision = 0;
	font.lfClipPrecision = 0;
	font.lfQuality = ANTIALIASED_QUALITY;//ANTIALIASED_QUALITY;//CLEARTYPE_QUALITY; //johan cek
	font.lfPitchAndFamily = 0; //FIXED_PITCH|FF_MODERN;
	lstrcpy(font.lfFaceName,"Microsoft JhengHei"); //"Microsoft JhengHei" //"Tahoma"
	...
	font.lfUnderline = 0;
	font.lfStrikeOut = 0;
	font.lfCharSet = THAI_CHARSET;//DEFAULT_CHARSET;
	font.lfOutPrecision = 0;
	...
	font.lfUnderline = 0;
	font.lfStrikeOut = 0;
	font.lfCharSet = THAI_CHARSET;//DEFAULT_CHARSET; //HANGUL_CHARSET
	font.lfOutPrecision = 0;
	font.lfClipPrecision = 0;
	font.lfQuality = CLEARTYPE_NATURAL_QUALITY;//ANTIALIASED_QUALITY;//CLEARTYPE_QUALITY; //johan cek
	font.lfPitchAndFamily = FIXED_PITCH | FF_MODERN;
	lstrcpy(font.lfFaceName, "Tahoma"); //Microsoft JhengHei
...
void cWindowManager::CreatePWScoreInfoDlg()
{
	cWindow * window = GetDlgInfoFromFile("Data/Interface/Windows/PWScore.bin", "rb");
	AddWindow(window);

	CPWScoreInfoDialog* pDlg = (CPWScoreInfoDialog*)window;
	GAMEIN->SetPWScoreInfoDlg(pDlg);
	pDlg->Linking();
}
...
{
	cWindow * pShowEquipInfo = WINDOWMGR->GetDlgInfoFromFile("Data/Interface/Windows/inspect.bin", "rb");
	if (!pShowEquipInfo)
	{
		MessageBox(NULL, "Failed to load Inspect Dialog", "Warning", MB_OK);
		return;
	}
...
//Alemuri Server Time Update--------------------------------------------------------------------------------------
void cWindowManager::CreateServerTimeDialog()
{
	cWindow * pServerTime = WINDOWMGR->GetDlgInfoFromFile("Data/Interface/Windows/ServerTimeDlg.bin", "rb");
	if (!pServerTime)
	{
		MessageBox(NULL, "Failed to load Server Time Dialog", "Warning", MB_OK);
		return;
	}

	WINDOWMGR->AddWindow(pServerTime);
	GAMEIN->SetServerTimeDlg((CServerTimeDlg *)pServerTime);
	GAMEIN->GetServerTimeDlg()->Linking();
}
//-----------------------------------------------------------------------------------------------------------------