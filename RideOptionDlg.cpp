...
void CRideOptionDlg::Render()
{
	...
	{
		const MONEYTYPE money = min(_atoi64(mMoneyEditBox->GetEditText()), MAX_INVENTORY_MONEY);
		mMoneyEditBox->SetEditText(0 == money ? "0" : AddComma(money));
		mIsUpdateComma = FALSE;
	}
}
	...
	case TB_STATE_PUSHUP3:
		{
			...
			const DWORD chargedMoney = atoi(RemoveComma(mMoneyEditBox->GetEditText()));
			if (chargedMoney > MAXMONEY2B)
			{
				CHATMGR->AddMsg( CTC_HPWARNING, "Max Price is 2,000,000,000" ) ;
				break;
			}
