...
CInventoryExDialog::CInventoryExDialog()
{
	...
	//Alemuri Item Link To Chat-----------------
	itemChatLocked = FALSE;
	ITEMMGR->SetIsItemChat(itemChatLocked);
	//------------------------------------------
}
...
//	
	...
	{
		//Alemuri Gender Change (nothing happen if you are male and use a Male scroll)
		if (pItem->GetItemIdx() == Gender_Change_Scroll_Male && hero->GetGender() == 0)
		{
			CHATMGR->AddMsg( CTC_SYSMSG, "Your Character is already Male" );
			return;
		}

		if (pItem->GetItemIdx() == Gender_Change_Scroll_Female && hero->GetGender() == 1)
		{
			CHATMGR->AddMsg( CTC_SYSMSG, "Your Character is already Female" );
			return;
		}
		//----------------------------------------------------------------------------
			...
			case ITEM_KIND_CHANGE_CLASS:
				{
...

					{
						...
						if (shieldItemInfo && shieldItemInfo->WeaponType != 0) //Alemuri Dual Weapon without penalty Fix
						{
			...
			case ITEM_KIND_REBORN :
				{
					...
					//Alemuri Rebirth Minimum Level---------------------------------------------------------
					if (HERO->GetLevel() < 120)
					{
						CHATMGR->AddMsg( CTC_SYSMSG, "You need to be Level 120 to use Reborn Ticket!" );
						return;					
					}
					//--------------------------------------------------------------------------------------
			...
			case ITEM_KIND_MARRY_REQUEST:
				{
					...
					else
					{
						...
						MARRYMGR->RequestMarrySyn( pObject, pItem->GetItemIdx(), pItem->GetPosition() ) ; //Alemuri added Item to check Check and Item Position
			...
			case ITEM_KIND_MARRY_ABORT:
				{
					...
					MARRYMGR->AbortMarrySyn(pItem->GetItemIdx(), pItem->GetPosition());	//Alemuri added Item to check Check and Item Position							
				}
				break;
			...
			case ITEM_KIND_ALIAS:
				{
					...
					ALIASMGR->ItemKindAliasSyn(pInfo->SupplyValue, pInfo->ItemIdx, pItem->GetPosition()); //Alemuri Alias block Cheat
				}
				break;
#endif
			}
...
void CInventoryExDialog::OnActionEvnet(LONG lId, void * p, DWORD we)
{
	...
	//Alemuri Item Link To Chat intercept if left click + LShift is pushed-----------------------------------
	//if (we == WE_LBTNCLICK && g_UserInput.GetKeyboard()->GetKeyPressed( KEY_SHIFT ))
	//{
	//	CItem * pItem = NULL ;
	//	cIconGridDialog * gridDlg = NULL ;
	//	gridDlg = (cIconGridDialog *)GetTabSheet(GetCurTabNum()) ;
	...
	//	
	//	itemChatLocked = FALSE;
	//	itemChatLocked = ITEMMGR->GetIsItemChat();
	//	if (pItem->IsLocked() == FALSE && itemChatLocked == FALSE)
	//	{
	//		ITEMMGR->ResetItemChatLinkOption();
	//		ITEMMGR->SetItemLocked(pItem);
	//		pItem->SetLock(TRUE);
	//		itemChatLocked = TRUE;
	//		ITEMMGR->SetIsItemChat(itemChatLocked);
	//		ITEMMGR->SetItemChatLinkOption(ITEMMGR->GetOption(pItem->GetDBIdx()));
	//		ITEMMGR->CreateItemChatTooltip(ITEMMGR->GetOption(pItem->GetDBIdx()), pItem->GetItemIdx(), pItem->GetDBIdx()); 
	//	}
	//	else if (pItem->IsLocked() == TRUE && itemChatLocked == TRUE)
	//	{
	//		pItem->SetLock(FALSE);
	//		itemChatLocked = FALSE;
	//		ITEMMGR->SetIsItemChat(itemChatLocked);
...
BOOL CInventoryExDialog::CanBeMoved( CItem* pItem, POSTYPE toPos )
{
	...
	else if( TP_WEAR_START <= toPos && toPos < TP_WEAR_END )
	{
		...
		//Added Shield2--------------------------------------------------------------------------------------
		else if( TP_WEAR_START + eWearedItem_Shield2 == toPos )
		{
			CItem*				shield			= GetItemForPos( TP_WEAR_START + eWearedItem_Shield );
			const ITEM_INFO*	shieldInfo		= ( shield ? shield->GetItemInfo() : 0 );
			const BOOL			isEnableTwoBlade	= HERO->GetHeroTotalInfo()->bUsingTwoBlade;
			
			if( (! shieldInfo)  || (! movingItemInfo))
			{
				//main shield slot is empty, skins can only mirror slots
				CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 2487 ) );
				return FALSE;
			}
			if( shieldInfo->EquipType !=  movingItemInfo->EquipType)
			{
				//accessory shield can be combined only with shield accessory and weapon can be combined only with weapon accessory
				CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 2488 ) );
				return FALSE;
			}
			if(	ePartType_Shield2 != movingItemInfo->Part3DType)
			{
				//Wrong Slot
				return FALSE;
			}
		...
		//--------------------------------------------------------------------------------------------------------------
		//Added Weapon2-------------------------------------------------------------------------------------------------
		else if( TP_WEAR_START + eWearedItem_Weapon2 == toPos )
		{
			CItem*				weapon			= GetItemForPos( TP_WEAR_START + eWearedItem_Weapon );
			const ITEM_INFO*	weaponInfo		= ( weapon ? weapon->GetItemInfo() : 0 );
			const BOOL			isEnableTwoBlade	= HERO->GetHeroTotalInfo()->bUsingTwoBlade;
			
			if( (! weaponInfo)  || (! movingItemInfo))
			{
				//main weapon slot is empty, skins can only mirror slots
				CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 2516 ) );
				return FALSE;
			}
			if( weaponInfo->EquipType !=  movingItemInfo->EquipType)
			{
				//shield can be combined only with shield accessory and weapon can be combined only with weapon accessory
				CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 2517 ) );
				return FALSE;
			}
			if(	ePartType_Weapon2 != movingItemInfo->Part3DType)
			{
				//Wrong Slot
				return FALSE;
			}
			if(	weaponInfo->WeaponAnimation != movingItemInfo->WeaponAnimation)
			{
				//Cant use a 2 handed skin with a 1 handed weapon or viceversa
				CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 2517 ) );
				return FALSE;
			}
			if(	weaponInfo->WeaponType != movingItemInfo->WeaponType)
			{
				//Cant use for example a skin of Bow for a Sword weapon
				CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 2517 ) );
				return FALSE;
			}
	...
	//Check when you are removing the Shield that skin shield is not equipped
	else if( fromPos == TP_WEAR_START + eWearedItem_Shield )
	{		
		CItem*	item2 = GetItemForPos( TP_WEAR_START + eWearedItem_Shield2 );
		const ITEM_INFO*	item2Info	= ( item2 ? item2->GetItemInfo() : 0 );

		if( item2Info )
		{
			//Remove the shield before to remove the skin shield
			CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg(2515)) ;								
			return FALSE;
		}
	}
	//----------------------------------------------------------------------------
	else if( fromPos == TP_WEAR_START + eWearedItem_Weapon )
	{
		...
		//Check when you are removing weapons that skins are not equipped-------------
		CItem*	item2 = GetItemForPos( TP_WEAR_START + eWearedItem_Weapon2 );
		const ITEM_INFO* item2Info	= ( item2 ? item2->GetItemInfo() : 0 );

		if( item2Info )
		{
			//Remove the weapon before to remove the skin weapon
			CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg(2515)) ;								
			return FALSE;
		}
		//----------------------------------------------------------------------------
...
void CInventoryExDialog::InitExtended_Inventory( CHARACTER_TOTALINFO* pInfo )
{
	...
	else if (pInfo->wInventoryExpansion == 3)
	{
		ShowTab(2, TRUE);
		ShowTab(3, TRUE);
		ShowTab(4, TRUE);
	...
	else if (pInfo->wInventoryExpansion == 4)
	{
		ShowTab(2, TRUE);
		ShowTab(3, TRUE);
		ShowTab(4, TRUE);
...
void CInventoryExDialog::SortStackItems() //sort test fix stack item
{
	DWORD invSlotNum = TP_INVENTORY_END + (TABCELL_INVENTORY_NUM * HERO->Get_HeroExtendedInvenCount());
	DWORD item1Idx = 0;
	DWORD item2Idx = 0;
	for (int i = 0; i <= invSlotNum - 1; ++i)
	{
		POSTYPE position = i;
		CItem* itemForPos = GetItemForPos(position);
		if (itemForPos)
		{
			item1Idx = itemForPos->GetItemIdx();
			DWORD itemForPosDurability = itemForPos->GetDurability();
			DWORD itemForPosDurabilityMax = itemForPos->GetItemInfo()->Stack;
			if (itemForPos->GetDurability() > 0)
			{
				for (int i = 0; i <= invSlotNum - 1; ++i)
				{
					POSTYPE position2 = i;
					CItem* itemForPos2 = GetItemForPos(position2);
					if (itemForPos2)
					{
						item2Idx = itemForPos2->GetItemIdx();
						DWORD itemForPos2Durability = itemForPos2->GetDurability();
						DWORD itemForPos2DurabilityMax = itemForPos2->GetItemInfo()->Stack;
						if ((itemForPos2->GetItemIdx() == itemForPos->GetItemIdx()) && (itemForPos2->GetPosition() != itemForPos->GetPosition()))
						{
							if ((itemForPos2Durability < itemForPos2DurabilityMax) && (itemForPosDurability < itemForPosDurabilityMax))//if ((itemForPos2Durability < itemForPos2DurabilityMax) && (itemForPosDurability < itemForPosDurabilityMax) && (position2 > position))
							{
								FakeItemCombine(position2, itemForPos, itemForPos2);
								XSleep(50); //kecepatan sort
								break;
							}
						}
					}
