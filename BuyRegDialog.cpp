...
BOOL CBuyRegDialog::GetBuyRegInfo(BUY_REG_INFO& BuyInfo)
{
	...
	m_nMoney = (MONEYTYPE)_atoi64(RemoveComma(m_pMoneyEdit->GetEditText()));
...
void CBuyRegDialog::Render()
{
	if( m_bActive )
	{
		if( mMoney != (MONEYTYPE)_atoi64(RemoveComma(m_pMoneyEdit->GetEditText())) )
		{
			mMoney = (MONEYTYPE)_atoi64(RemoveComma(m_pMoneyEdit->GetEditText()));;

			// 091112 ONS 금액표시 색상 설정
			mColor = DWORD(GetMoneyColor(mMoney));
		}
	}
