...
void CMiniMapDlg::SetActive( BOOL val )
{
	...
	if( (SIEGEWARFAREMGR->IsSiegeWarfareKindMap(MAP->GetMapNum()) && ! SIEGEWARFAREMGR->Is_CastleMap()) ||
		DungeonMGR->IsDungeonMap( MAP->GetMapNum() ) || MAP->GetMapNum() == StreetTournament || MAP->GetMapNum() == PVP )
   	{
...
void CMiniMapDlg::SetChanelComboBox( MSG_CHANNEL_INFO* pInfo )
{
	...
	for( int i = 0; i < iChannelCount; ++i )
	{
		...
		{
			char buff[12] = {0}; 
			sprintf (buff, "%d", pInfo->PlayerNum[i]);
			m_pChannelComboBox->InsertChannelInfo( buff, m_pChannelComboBox->GetFreeColor() );
		}
		else if(pInfo->PlayerNum[i] < m_pChannelComboBox->GetBusyCount() )	//MAX USER PER CHANNEL
		{
			char buff[12] = {0}; 
			sprintf (buff, "%d", pInfo->PlayerNum[i]);
			m_pChannelComboBox->InsertChannelInfo( buff, m_pChannelComboBox->GetBusyColor() );
		}
		else 
		{
			char buff[12] = {0}; 
			sprintf (buff, "%d", pInfo->PlayerNum[i]);
			m_pChannelComboBox->InsertChannelInfo( buff, m_pChannelComboBox->GetExceedColor() );
		}
...
void CMiniMapDlg::ChangeChannel()
{
	...
	{
		CHATMGR->AddMsg(CTC_SYSMSG, CHATMGR->GetChatMsg(5125));
		m_pChannelComboBox->SelectComboText(gChannelNum);
		return;
	}
