...
//			( (eOO_EFFECT_ONLYHERO == dwEffectOption) && (pParam->m_pOperator == (CObject*)OBJECTMGR->GetHero()) ) )	
		//SW051026 수정
		//Alemuri Effect Exclusion Minimum Option-------------------------------------------- 
		CEffect* pEffect = EFFECTMGR->GetEffect( pParam->m_EffectID );
		int effectKind = pEffect->GetEffectKind();
		std::string effectName = "";
		std::string holyBarrier = "data/script/direct/BUFF_Invulnerable1.beff";
		std::string shieldBarrier = "data/script/direct/BUFF_Invulnerable2.beff";
		if (pDesc)
		{
			char* name = pDesc->m_EffFilename;
			std::string effName(name);
			effectName = effName;
		}
		//-----------------------------------------------------------------------------------
		if( ( eOO_EFFECT_DEFAULT == dwEffectOption ) || //기본 설정이거나
			(effectName == holyBarrier)				 ||	//Alemuri Effect Exclusion Minimum Option
			(effectName == shieldBarrier)			 || //Alemuri Effect Exclusion Minimum Option
			...
			) )
		{
			...
			// 080708 LUJ, 널 포인터 검사 추가
			if( ! strObjectName )
			{
				return;
			}
			else if( strcmp(strObjectName,OPERATOR) == 0)
			{
...
void CObjectEffectUnit::DoOn(EFFECTPARAM* pParam,TARGETSET* pObject)
{
	...
	// 유효하지 않은 대상이 있는 경우가 있다. 이때 무효한 포인터를 참조하여 강제 종료된다
	...
	MakeEffectUnit(
		&m_pEffectObjectArray[m_nCurTargetNum],
		&pos,
		angle,
		pDesc,
		pParam,
...
void CObjectEffectUnit::DoAttach(EFFECTPARAM* pParam,TARGETSET* pObject,char* AttachPartName,BOOL EnableScale/* = FALSE*/)
{
	// 090803 ShinJS --- Effect 옵션 Default가 아닌경우 MakeEffectUnit()를 정상적으로 실행하지 않은 상태에서 Detach시 Error 발생,
	//					 Effect 옵션이 Default가 아니고 HERO 가 아닌경우 Attach 하지 않는다.
	const DWORD dwEffectOption = OBJECTMGR->GetEffectOption();
	if( dwEffectOption != eOO_EFFECT_DEFAULT && pObject->pTarget != HERO )
	{
		return;
	}
	...
	pCurObject->Attach(pObject->pTarget->GetEngineObject(),AttachPartName,EnableScale);
...
void CObjectEffectUnit::DoLink(EFFECTPARAM* pParam,TARGETSET* pObject)
{
	...
	{
		pEffObject->m_Object.LinkTo(pObject->pTarget->GetEngineObject(), pParam->m_EffectID);
	}
}
...
void CObjectEffectUnit::SetHide( BOOL bHide) 
{
	EFFECTOBJECT* pEffObject;
	CEngineObject* pCurObject;
