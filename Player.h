...
public:
	...
	DWORD mJobEffect;
	DWORD mSelfEffect;
	DWORD mMarryPlayerEffect;
	DWORD mMarryEffectNum;
	const static DWORD MARRYEFFECT = 20; // 10384	eff_r_darkspin2.beff
...
public:
	...
	DWORD GetMarryPlayerID() const { return m_CharacterInfo.MarryPlayerID; }	
	void  SetMarryPlayerID(DWORD marryplayerid){ m_CharacterInfo.MarryPlayerID = marryplayerid; }
	void  SetMarryPlayerName(char* marryplayername){ SafeStrCpy(m_CharacterInfo.MarryName ,marryplayername,MAX_NAME_LENGTH + 1); }
	...
	char*					GetNickName();
	//ALIASTYPE				GetAliasType(){ return ALIASTYPE(rand()% ALIASTYPE_MAX);}
	DWORD					GetAliasType(){ return m_CharacterInfo.AliasType; }
	// 동맹 길드
...
public:
	...
	void StartJobEffect();
	void EndJobEffect();
	// 18 Feb 2014 , Death Add Marry Effect Function
	void StartMarryEffect(CPlayer* pMarriagePlayer);
	void EndMarryEffect();
...
private:
	...
	BOOL  m_registeredstreettournament;
...
public:
	...
	void SetRegisterStreetTournament(BOOL bRgeister)			{ m_registeredstreettournament = bRgeister; }
	BOOL IsRegisterStreetTournament()							{return m_registeredstreettournament;}

	void SetVeirfyCaptchaState(BOOL state)						{ m_verifystate = state; if(state == TRUE) m_verifyTimer = 60; m_verifyTime = gCurTime; }
	BOOL GetVeirfyCaptchaState()								{return m_verifystate;}
...
public:
	...
	//Switch Fly Walk---------------------------
	int GetFlyMode() const { return m_flyMode; }
	void SetFlyMode( int flyMode);
	//------------------------------------------

	//Alemuri Party War (Get Party War ID)
	DWORD GetPartyWarID(){ return m_CharacterInfo.PartyWarID ;};
	void  SetPartyWarID( DWORD pwarID ){ m_CharacterInfo.PartyWarID = pwarID;};
	//------------------------------------
