	...
	case MP_GUILD_WAR_START:		// guildfieldwar start
		{
			...
			for( DWORD i = 0; i < message->mSize; ++i )
			{
				...
				const MONEYTYPE money = data.MarkName;
...
void CGuildWarManager::SendDeclare( char* pName, char* pMoney )
{
	...
	const MONEYTYPE dwMoney = _atoi64( pMoney );
	...
	{		
		...
		{
			CHATMGR->AddMsg( CTC_SYSMSG, "Max Limit for guild war is 2.000.000.000" );
			return;
		}
