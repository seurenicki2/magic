...
mAutoRotataionDelayTime( 5000 )
{
	...
	//Alemuri Gender Change (Initialize it)
	genderBeforeTempChange = 0;
...
void CBodyChangeDialog::SetActive( BOOL isActive, CItem& item )
{
	...
	// 080414 LUJ, 이전 정보를 저장하고, 기본값을 현재 설정으로 해놓는다
	{
		...
		//Alemuri try change gender number 2 is Elf probably need to be added Racial too---------------
		genderBeforeTempChange = info->Gender;
		if (item.GetItemIdx() == Gender_Change_Scroll_Male)
		{
			if (hero->GetRace() == RaceType::RaceType_Human)
			{
				mCharacterChangeInfo.Gender = 0;
			}
			else if (hero->GetRace() == RaceType::RaceType_Elf)
			{
				mCharacterChangeInfo.Gender = 0;
			}
		}
		else if (item.GetItemIdx() == Gender_Change_Scroll_Female)
		{
			if (hero->GetRace() == RaceType::RaceType_Human)
			{
				mCharacterChangeInfo.Gender = 1;
			}
			else if (hero->GetRace() == RaceType::RaceType_Elf)
			{
				mCharacterChangeInfo.Gender = 1; //2 = Devil Male 3 = Devil Female 1 = Elf Female
			}
		}
		//---------------------------------------------------------------------------------------------
	...
	// 080414 LUJ, 종족/성별 표시
	{
		...
		if( titleStatic )
		{
			titleStatic->SetStaticText( ( race + " " + gender ).c_str() ); //Human Female for example
		}
	}
...
void CBodyChangeDialog::SetActive( BOOL isActive )
{
	...
	else if( ! isActive )
	{
		...
		//Alemuri Gender Change (restore normal gender if you push cancel)
		mCharacterChangeInfo.Gender = genderBeforeTempChange;
		//----------------------------------------------------------------
