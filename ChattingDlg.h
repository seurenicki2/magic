...
private:
	...
	const HYPERLINKNAME& GetHyperLink(LPCTSTR,DWORD);
...
public:
	...
	void AddMsgItemChat( BYTE byMessageType, BYTE byTabBtnType, char* msg, DWORD dwColor,std::vector<std::string>, std::vector<DWORD>, DWORD) ;
	
	void AddShoutMsg(char* msg, DWORD dwColor);
	
	void AddShoutMsgWithItem(const std::vector<std::string>&, std::vector<DWORD>, char* msg, DWORD dwColor, DWORD linkedItemIdx);
