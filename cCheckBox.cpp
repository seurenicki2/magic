...
void cCheckBox::Render()
{
	...
	if( IsMovedWnd() )
	{
		m_renderRect.left	= (LONG)m_absPos.x+15 ;
		m_renderRect.top	= (LONG)m_absPos.y+6 ;
	...
	CFONT_OBJ->RenderFont(m_wFontIdx, m_szCheckBoxText,lstrlen(m_szCheckBoxText),&m_renderRect,RGBA_MERGE(m_dwCheckBoxTextColor, m_alpha * m_dwOptionAlpha / 100));

	cWindow::Render();
}


void cCheckBox::Add( cWindow* )
{
