...
//aziz add PVP color name //battle arena
#define NAMECOLOR_PVPMODE	RGB_HALF(255, 0, 0)//johan set to normal (255, 0, 255)

#define NAMECOLOR_GM		RGB_HALF( 28, 233, 151 )
//Alemuri Party War----------------------------------------------
#define NAMECOLOR_PARTYWARALLY	RGB_HALF( 0, 255, 255 )
#define NAMECOLOR_PARTYWARENEMY RGB_HALF(255, 0, 94)
//---------------------------------------------------------------
...
protected:
	...
	//unsigned short m_hideLevelCheckChanged;
	////--------------------------------------------
...
public:
	...
	void SetFamilyMark();
	void SetAliasMark();
	void ShowObjectName( BOOL bShow, DWORD dwColor = NAMECOLOR_DEFAULT, BOOL isPkMode = FALSE );
	void ShowChatBalloon( BOOL bShow, char* chatMsg, DWORD dwColor = RGB_HALF(70,70,70), DWORD dwAliveTime = 5000 );
	//Alemuri Party War------------------------------
	DWORD GetPartyWarColor(DWORD originalColor, CPlayer* pPlayer, BOOL isPkMode);
	//-----------------------------------------------
