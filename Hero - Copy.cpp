...
DWORD CHero::DoGetAccuracyRate()
{
	...
	// 080313 LUJ, 아이템 스탯을 일반과 세트로 분리
	{
		...
		dwTotalAccuracy = (dwTotalAccuracy + mItemOptionStat.mAccuracy.mPlus + value.mPlus) * (1.0f + value.mPercent);
	}
...
DWORD CHero::DoGetEvasionRate()
{
	...
	// 080701 LYW, 버프로 늘어나는 스탯 수치도 추가한다.
	//float wDexterityAll	= ( mCharStat.mDexterity.mPlus + mItemStat.mDexterity.mPlus ) * ( 1.0f + mItemStat.mDexterity.mPercent );
	float wDexterityAll = (mCharStat.mDexterity.mPlus + mItemBaseStat.mDexterity.mPlus + mItemOptionStat.mEvade.mPercent + GetBuffStatus()->Dex) * (1.0f + mItemBaseStat.mDexterity.mPercent);
	...
	// 080313 LUJ, 아이템 스탯을 일반과 세트로 분리
	{
		...
		dwTotalEvasion = (dwTotalEvasion + mItemOptionStat.mEvade.mPlus + value.mPlus) * (1.0f + value.mPercent);
	}
