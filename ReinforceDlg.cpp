...
void CReinforceDlg::AddMaterial( CItem& item )
{
	...
	for( WORD i = 0; i < mMaterialIcon.size(); ++i )
	{
		...
		// 등록 개수 변경
		{
			...
			{
				//Alemuri Reinforce Commenting these lines-------------------------------------
		...
		// 재료 아이콘 등록		
		{
			...
			//Alemuri Reinforce with 1000 Gems--------------------
			if (material->GetDurability() > 100)
				material->SetDurability(100);
			//----------------------------------------------------
		...
		// 원본 아이템에도 등록되었음을 알리기 위해 개수를 0으로 하고 잠금 표시한다
		{
			if( ITEMMGR->IsDupItem( item.GetItemIdx() ) )
			{
				...
				item.SetDurability( item.GetDurability() - 100 );
				//----------------------------------------------------
				else
...
void CReinforceDlg::SetActive( BOOL isActive )
{
	...
	{
		CHATMGR->AddMsg( CTC_OPERATEITEM, "Reinforce is disable at this map" );
		isActive = FALSE;
	}
...
void CReinforceDlg::RemoveMaterial()
{
	...
	for( WORD i = 0; i < mMaterialIcon.size(); ++i )
	{
		...
		if( original )
		{
			...
			{
				//original->SetDurability( material->GetDurability() + original->GetDurability() ); //Alemuri Reinforce
			}
