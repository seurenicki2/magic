...
#include "../PartyWar/PWScoreInfoDialog.h" //TDEV/TJ UG PARTY WAR SCOREBOARD
...
// 24-05-2012 Death Add NewWeatherDialog
#include "NewWeatherDialog.h"

// 06 Feb 2014 Death Add Marry Invite Dialog
...
enum ENUM_DLGINFO_TABLES
{
	...
	eGDTVIEWDLG,
	eGTSCORETIMEDLG,
	ePWSCORETIMEDLG,//TDEV/TJ UG PARTY WAR SCOREBOARD
	eSEIGEWARTIMEREGDLG,				
...
// 080401 LYW --- cScriptManager : Add dialogs part for chatroom system.
	...
	eANTIBOTDLG,
	eMA_INVITEDLG,
	eTRAININGDLGJCT,//JobTree
	
	eSLOTDLG,	// 080313 NYJ --- 전체외치기 다이얼로그
	...
	while(1)
	{
		...
		case eGTSCORETIMEDLG :	// $GTSCORETIMEDLG
				if((fp.GetString())[0] == '{')
					...
					__asm int 3;			
				break;
		//TDEV/TJ UG PARTY WAR SCOREBOARD
		case ePWSCORETIMEDLG :
				if((fp.GetString())[0] == '{')
					dlg = (cDialog *)GetInfoFromFile(new CPWScoreInfoDialog, &fp);
				else
					__asm int 3;			
				break;
		...
		case eFISHINGPOINTDLG:
			if((fp.GetString())[0] == '{')
				...
				__asm int 3 ;
			break;
		// 24 - 05 - 2012 Death , Add Weather Dialog
		case eWEATHERDLG:
			if((fp.GetString())[0] == '{')
				dlg = GetInfoFromFile(new CNewWeatherDlg, &fp) ;
			else
				__asm int 3 ;
			break;
		...
		case eANTIBOTDLG:
			if((fp.GetString())[0] == '{')
				dlg = (CAntiBotDialog*)GetInfoFromFile(new CAntiBotDialog, &fp);
			else
				__asm int 3;
			break;
		case eMA_INVITEDLG :		// $PA_INVITEDLG
			if((fp.GetString())[0] == '{')
				dlg = (CMarryInviteDlg *)GetInfoFromFile(new CMarryInviteDlg, &fp);
			else
				__asm int 3;
			break;
		case eTRAININGDLGJCT :			// $TRAININGDLGJCT //JobTree
			if((fp.GetString())[0] == '{')
			{
				dlg = (cDialog*)GetInfoFromFile(new cSkillTrainingDlgJct, &fp);
			}
			else
				_asm int 3;
			break;
		...
		case eSLOTDLG:
			if((fp.GetString())[0] == '{')
				dlg = (CSlotDialog*)GetInfoFromFile(new CSlotDialog, &fp) ;
			else
				__asm int 3 ;
			break ;
		...
		case eREPORTDLG:
			if((fp.GetString())[0] == '{')
				dlg = (CReportDialog*)GetInfoFromFile(new CReportDialog, &fp) ;
			else
				__asm int 3 ;
			break ;
		...
		case eCHAPTCHADLG:
			if((fp.GetString())[0] == '{')
				dlg = (CChaptchaDlg*)GetInfoFromFile(new CChaptchaDlg, &fp) ;
			else
				__asm int 3 ;
			break ;
		//inspect
		case eOtherCharacterEquip: // 2014-08-11 Create Character Equip window! //$INSPECT
			if ((fp.GetString())[0] == '{')
		...
		//Alemuri Server Time Update-------------------------------------------------------
		case eServerTimeDialog: // 2020-11-03 Create Server Time Dialog! //$INSPECT
			if ((fp.GetString())[0] == '{')
				dlg = (cDialog *)GetInfoFromFile((cWindow*)(new CServerTimeDlg), &fp);
			else
				__asm int 3;
			break;
		//----------------------------------------------------------------------------------
...
//
	...
	while(1)
	{
		...
		if( dwSelect)
		{
			switch( dwItem)
			{
				...
				case 8:		// "#TOOLTIPMSG")
				{
					//i'm too lazy to put tip in client, so i do it here
					if (0 == strcmpi(IDName, "MNM_PCROOM_MALL_BTN"))
						strcpy(toolTipMsg, "Galaxy Shop");
					else if (0 == strcmpi(IDName, "MNM_PCROOM1_MALL_BTN"))
						strcpy(toolTipMsg, "Daily Reward");
					else if (0 == strcmpi(IDName, "MNM_PCROOM2_MALL_BTN"))
						strcpy(toolTipMsg, "Mount & Houses");
					else if (0 == strcmpi(IDName, "MNM_PCROOM3_MALL_BTN"))
						strcpy(toolTipMsg, "Trial Costumes");
					else if (0 == strcmpi(IDName, "MNM_PCROOM4_MALL_BTN"))
						strcpy(toolTipMsg, "Potion & Material");
					else if (0 == strcmpi(IDName, "MNM_PCROOM5_MALL_BTN"))
						strcpy(toolTipMsg, "Previews");
					else if (0 == strcmpi(IDName, "MNM_PCROOM6_MALL_BTN"))
						strcpy(toolTipMsg, "Vip Equipment");
					else if (0 == strcmpi(IDName, "MNM_PCROOM7_MALL_BTN"))
						strcpy(toolTipMsg, "Vip Costumes");
					else if (0 == strcmpi(IDName, "MNM_PCROOM8_MALL_BTN"))
						strcpy(toolTipMsg, "Vip Usable");
					else if (0 == strcmpi(IDName, "MNM_PCROOM9_MALL_BTN"))
						strcpy(toolTipMsg, "Vip Costumes");
					else if (0 == strcmpi(IDName, "MNM_PCROOM10_MALL_BTN"))
						strcpy(toolTipMsg, "Craft & Boxes");
					else if (0 == strcmpi(IDName, "MNM_PCROOM11_MALL_BTN"))
						strcpy(toolTipMsg, "Equipment");
					else
	...
	//KES 030916
				...
				case 74:	// "#SPINMINMAX")
					minValue = fp->GetLong();
					maxValue = fp->GetLongLong();//more than 4billion ULONGLONG
					break;
				case 75:	// "#SPINUNIT")
					spinUnit = fp->GetLong();
					break;
		...
		//			win->SetPieceImage(&GuagePieceImage);
				...
				case 126 : // "#SAVE_POSITION"
					{
						bSavePosition = fp->GetBool(); //set always false
					}
					break;
				...
				case 127 : // "#SAVE_ACTIVITY"
					{
						bSaveActivity = fp->GetBool(); //set always false
					}
					break;
	...
	CASE(WT_GTSCOREINFO_DLG)
		...
		if(-1 != FUNCSEARCH(funcName))
			win->SetcbFunc(g_mt_func[FUNCSEARCH(funcName)]._func);
	BREAK
	CASE(WT_PWSCOREINFO_DLG)//TDEV/TJ UG PARTY WAR SCOREBOARD
		CPWScoreInfoDialog* win = (CPWScoreInfoDialog*)wnd;
		win->Init(point.x, point.y, point.w, point.h, &BasicImage, uiID);		
		win->SetMovable(fMovable);
		win->SetCaptionRect(&captionRect);
		if(-1 != FUNCSEARCH(funcName))
	...
	CASE(WT_SLOT_DLG)
		CSlotDialog* win = (CSlotDialog*)wnd;
		win->Init(point.x, point.y, point.w, point.h, &BasicImage, uiID);
		win->SetMovable(fMovable);
		win->SetCaptionRect(&captionRect);
		if(-1 != FUNCSEARCH(funcName))
	...
	CASE(WT_REPORT_DLG)
		CReportDialog* win = (CReportDialog*)wnd;
		win->Init(point.x, point.y, point.w, point.h, &BasicImage, uiID);
		win->SetMovable(fMovable);
		win->SetCaptionRect(&captionRect);
		if(-1 != FUNCSEARCH(funcName))
	...
	CASE(WT_CHAPTCHA_DLG)
		CChaptchaDlg* win = (CChaptchaDlg*)wnd;
		win->Init(point.x, point.y, point.w, point.h, &BasicImage, uiID);
		win->SetMovable(fMovable);
		win->SetCaptionRect(&captionRect);
		if(-1 != FUNCSEARCH(funcName))
	...
	//Alemuri Server Time Update----------------------------------------------------------
	CASE(WT_SERVERTIMEDLG)       // the create equip 2014-08-11
		CServerTimeDlg * win = (CServerTimeDlg*)wnd;
		//win->SetValidXY(validXY.x, validXY.y);
		win->Init(point.x, point.y, point.w, point.h, &BasicImage, uiID);
		//win->Init(point.x, point.y, point.w, point.h, &BasicImage, IDSEARCH(IDName));
		win->SetMovable(fMovable);
