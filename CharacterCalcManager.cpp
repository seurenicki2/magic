...
void CCharacterCalcManager::NetworkMsgParse(BYTE Protocol,void* pMsg)
{
	...
	case MP_CHAR_LIFE_BROADCAST:
		...
		break;
	case MP_CHAR_POINTUP_NOTIFY:					Char_PointUp_Notify( pMsg ) ;			break;
	}
}
...
void CCharacterCalcManager::Char_Str_Notify( void* pMsg )
{
	...
	if(HERO)
	{
		HERO->SetStrength((DWORD)msg->dwData);
	}
}
...
void CCharacterCalcManager::Char_Dex_Notify( void* pMsg )
{
	...
	if(HERO)
	{
		HERO->SetDexterity((DWORD)msg->dwData);
	}
}
...
void CCharacterCalcManager::Char_Vit_Notify( void* pMsg )
{
	...
	if(HERO)
	{
		HERO->SetVitality((DWORD)msg->dwData);
	}
}
...
void CCharacterCalcManager::Char_Wis_Notify( void* pMsg )
{
	...
	if(HERO)
	{
		HERO->SetWisdom((DWORD)msg->dwData);
	}
}
...
void CCharacterCalcManager::Char_Int_Notify( void* pMsg )
{
	...
	if(HERO)
	{
		HERO->SetIntelligence((DWORD)msg->dwData);
	}
}
...
void CCharacterCalcManager::Char_PointUp_Notify( void* pMsg )
{
	MSG_POINT * pmsg = (MSG_POINT*)pMsg;
	CPlayer* pPlayer = (CPlayer*)OBJECTMGR->GetObject( pmsg->dwObjectID );
	if(pPlayer)
	{
		CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 2416 ),pmsg->PointUp,pmsg->CurrentPoint );
	}
}