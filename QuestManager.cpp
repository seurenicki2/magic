...
{
	MSG_DWORDEX* pmsg = (MSG_DWORDEX*)pMsg;																// 메시지를 받을 포인터를 선언하고 메시지를 받는다.

	CHATMGR->AddMsg( CTC_QUEST, CHATMGR->GetChatMsg(236), AddComma(pmsg->dweData) );					// 골드를 얻었다는 메시지를 출력한다.
...
void CQuestManager::Quest_TakeExp_Ack( void* pMsg )													// 퀘스트 경험치를 획득 수락 함수.
{
	...
	CHATMGR->AddMsg( CTC_QUEST, CHATMGR->GetChatMsg(237), AddComma(pmsg->dweData) );					// 경험치를 얻었다는 메시지를 출력한다.
...
void CQuestManager::Quest_TakesExp_Ack( void* pMsg )												// 퀘스트 sp 획득 수락 함수.
{
	...
	CHATMGR->AddMsg( CTC_QUEST, CHATMGR->GetChatMsg(238), pmsg->dweData );							// sp를 얻었다는 메시지를 출력한다.

	HERO->SetExpPoint(HERO->GetExpPoint() + pmsg->dweData) ;											// HERO의 경험치를 세팅한다.
}
...
void CQuestManager::Quest_GiveMoney_Ack( void* pMsg )												// 머니를 주는 함수.
{
	...
	CHATMGR->AddMsg( CTC_QUEST, CHATMGR->GetChatMsg(236), AddComma(pmsg->dweData) );					// 퀘스트 보상으로 골드를 얻었다는 메시지를 출력한다.
}
