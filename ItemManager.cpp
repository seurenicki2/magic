...
//	m_ENchantNotifyCheckTime = 0;
	...
	//Alemuri ChangeItem-------------
	LoadScriptFileDataChangeItem();
	itemGrade = 0;
	itemIdxPrevious = 0;
	//-------------------------------

	
	isItemChatLink = FALSE;
	itemLocked = NULL;
...
void CItemManager::AddNametoolTip( cIcon& icon, const ITEMBASE& item )
{
	...
	// 이름 표시
	{
		...
		sprintf(line, "%s", info->ItemName);
		icon.AddToolTipLine( line, color );
	}	

	DWORD color;
	char rarity[MAX_PATH] = { 0 };
	switch (info->kind)
	{
		case ITEM_INFO::eKindNormal:
		{
			color = RGB(254, 254, 254);
			sprintf(rarity, "Common");
		}break;
		case ITEM_INFO::eKindHighClass:
		{
			color = RGB(50, 255, 50);//0
			sprintf(rarity, "Magic");
		}break;
		case ITEM_INFO::eKindRare:
		{
			color = RGB(254, 150, 20);
			sprintf(rarity, "Rare");
		}break;
		case ITEM_INFO::eKindUnique:
		{
			color = RGB(50, 135, 245);
			sprintf(rarity, "Epic");//Unique
		}break;
		case ITEM_INFO::eKindLegend:
		{
			color = RGB(4, 255, 255);//50
			sprintf(rarity, "Legend");
		}break;
		case ITEM_INFO::eKindRebirthExchange:
		{
			color = RGB(245, 245, 0);
			sprintf(rarity, "High End");
		}break;
	}
	//add more line
	sprintf(line, "Grade : %s", rarity);
	//icon.AddToolTipLine(line, color);
	...
	if (!info->Stack)
	{
		...
		if (option.mEnchant.mLevel)
		{
			...
			sprintf(line, "Enchant Level : %s", isEnchant);
			icon.AddToolTipLine(line, RGB_HALF(255, 255, 255));
		}
	}
	...
	//case eEquipType_Weapon:
		{
			...
			case eWeaponType_Sword:
				{
					if (info->EquipSlot == eWearedItem_Weapon2)
					{
						sprintf( line, "%s", CHATMGR->GetChatMsg( 2517 ) );
						break;
					}
					else if (info->EquipSlot == eWearedItem_Shield2)
					{
						sprintf( line, "%s", CHATMGR->GetChatMsg( 2518 ) );
						break;
					}
					else
					{
			...
			case eWeaponType_Mace:
				{
					if (info->EquipSlot == eWearedItem_Weapon2)
					{
						sprintf( line, "%s", CHATMGR->GetChatMsg( 2519 ) );
						break;
					}
					else if (info->EquipSlot == eWearedItem_Shield2)
					{
						sprintf( line, "%s", CHATMGR->GetChatMsg( 2520 ) );
						break;
					}
					else
					{
			...
			case eWeaponType_Axe:
				{
					if (info->EquipSlot == eWearedItem_Weapon2)
					{
						sprintf( line, "%s", CHATMGR->GetChatMsg( 2521 ) );
						break;
					}
					else if (info->EquipSlot == eWearedItem_Shield2)
					{
						sprintf( line, "%s", CHATMGR->GetChatMsg( 2522 ) );
						break;
					}
					else
					{
			...
			case eWeaponType_Dagger:
				{
					if (info->EquipSlot == eWearedItem_Weapon2)
					{
						sprintf( line, "%s", CHATMGR->GetChatMsg( 2523 ) );
						break;
					}
					else if (info->EquipSlot == eWearedItem_Shield2)
					{
						sprintf( line, "%s", CHATMGR->GetChatMsg( 2524 ) );
						break;
					}
					else
					{
			...
			case eWeaponType_Staff:
				{
					if (info->EquipSlot == eWearedItem_Weapon2)
					{
						sprintf( line, "%s", CHATMGR->GetChatMsg( 2525 ) );
						break;
					}
					else if (info->EquipSlot == eWearedItem_Shield2)
					{
						sprintf( line, "%s", CHATMGR->GetChatMsg( 2526 ) );
						break;
					}
					else
					{
			...
			case eWeaponType_Bow:
				{
					if (info->EquipSlot == eWearedItem_Weapon2)
					{
						sprintf( line, "%s", CHATMGR->GetChatMsg( 2527 ) );
						break;
					}
					else
					{
			...
			case eWeaponType_Gun:
				{
					if (info->EquipSlot == eWearedItem_Weapon2)
					{
						sprintf( line, "%s", CHATMGR->GetChatMsg( 2528 ) );
						break;
					}
					else
					{
			...
			case eWeaponType_BothDagger:
				{
					if (info->EquipSlot == eWearedItem_Weapon2)
					{
						sprintf( line, "%s", CHATMGR->GetChatMsg( 2529 ) );
						break;
					}
					else
					{
...
void CItemManager::NetworkMsgParse(BYTE Protocol,void* pMsg)
{
	...
	case MP_ITEM_SORT:								Item_Move_Sort(pMsg);							break;
	...
	case MP_ITEM_DAILYITEM_ACK:
		{
			CHATMGR->AddMsg( CTC_SYSMSG, "You got a new Daily Reward in your Item Mall" );
			break;
		}
	case MP_ITEM_DAILYITEM_NACK:
		{
			CHATMGR->AddMsg( CTC_SYSMSG, "You already got your Daily item" );
			break;
		}
	//--------------------------------------------------------------------------------------------------------------
	...
	case MP_ITEM_QUERY_EQUI_ACK:
		{
			cDialog* pDlg = WINDOWMGR->GetWindowForID( MBI_INSPECT_WAITING );
			if( pDlg )
			...
			for (int i = 0; i<eWearedItem_Max; i++)
			{
				...
				ITEMMGR->AddOption(pmsg->WearedItemOptionInfo[i]);

				ITEMMGR->AddToolTip(pItem);// 2014-11-15 맒
				//쯔쯹뱄ㅸか?
				//pItem->SetLock(TRUE);
			...
			CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 5106 ) );
		}
		break;

	case MP_ITEM_QUERY_EQUI_NACK:
		{
			cDialog* pDlg = WINDOWMGR->GetWindowForID( MBI_INSPECT_WAITING );
			if( pDlg )
				WINDOWMGR->AddListDestroyWindow( pDlg );

            CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 5107 ) );

		}
		break;
		
	case MP_ITEM_QUERY_EQUI_REQUEST_ACK:
		{
			...
			if( pApplyer == NULL )
			{
				...
				//msg.bData		= 1;	//Accept할수 없는 상태
				//NETWORK->Send( &msg, sizeof(MSG_BYTE) );
				return;
			}

			cMsgBox* pMsgBox = WINDOWMGR->MsgBox( MBI_INSPECT_ACCEPT, MBT_YESNO,
									// 070126 LYW --- MsgBox : Modified message number.
									//CHATMGR->GetChatMsg(373), pApplyer->GetObjectName() );
									CHATMGR->GetChatMsg(5102), pApplyer->GetObjectName() );
			if( pMsgBox ) pMsgBox->SetDefaultBtn( -1 );
			
			m_InspectRequesterId = msg->dwData;
			CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 5108 ), pApplyer->GetObjectName() );
		}
		break;

	case MP_ITEM_QUERY_EQUI_REQUEST_NACK:
		{
			cDialog* pDlg = WINDOWMGR->GetWindowForID( MBI_INSPECT_ACCEPT );
			if( pDlg )
				WINDOWMGR->AddListDestroyWindow( pDlg );

			CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 5105 ) );

		}
		break;
	case MP_ITEM_QUERY_EQUI_NACKDC:
	{

		CHATMGR->AddMsg(CTC_SYSMSG, CHATMGR->GetChatMsg(5104));

	}
	break;
	case MP_ITEM_QUERY_EQUI_OPTIONBLOCK_NAK:
	{
		cDialog* pDlg = WINDOWMGR->GetWindowForID(MBI_INSPECT_WAITING);
		if (pDlg)
			WINDOWMGR->AddListDestroyWindow(pDlg);

		CHATMGR->AddMsg(CTC_SYSMSG, CHATMGR->GetChatMsg(5110));

	}
	break;
	case MP_ITEM_QUERY_EQUI_WAITINGDLG_ACK:
	{
	...
	break;
	case MP_ITEM_DRAGONYA_COSTUME_NACK:
	{
		CHATMGR->AddMsg(CTC_SYSMSG, "Only The Chosen One can domate this costume");
	}
	break;
		//-------------------------------------------------------------------------------------------------------
	//Alemuri Add Diamonds With Double Click-------------------------------------------------------
	case MP_ITEM_ADD_DIAMONDS:
	{
		MSG_DWORD* pmsg = (MSG_DWORD*)pMsg;
		CHATMGR->AddMsg(CTC_SYSMSG, "%d Diamonds have been added in your account", pmsg->dwData);
	}
	break;
	//----------------------------------------------------------------------------------------------
	}
...
{
	{
		...
		while( ! file.IsEOF() )
		{
			...
			pInfo->Shop = file.GetDwordEx(); // 아이템몰 여부
			...
			pInfo->Trade = file.GetDwordEx();
			pInfo->Deposit = file.GetDwordEx();
			pInfo->Sell = file.GetDwordEx();
			pInfo->Decompose = file.GetWord();

			pInfo->DecomposeIdx = file.GetDword();
			pInfo->Time = file.GetDword();

			pInfo->BuyPrice = file.GetDwordEx();
			pInfo->SellPrice = file.GetDwordEx();
			...
			//Alemuri Search Drop (Tab Complete)------------------------------------------
			std::string lowCapItemName(pInfo->ItemName);
			std::for_each(lowCapItemName.begin(), lowCapItemName.end(), [](char & c){ 
			c = ::tolower(c);
			});
			searchDropsLowercase.push_back(lowCapItemName);
			searchDrops.push_back(pInfo->ItemName);
			searchDropsIndexes.push_back(pInfo->ItemIdx);
			//----------------------------------------------------------------------------
		}
	}
}
...
ITEM_INFO* CItemManager::FindItemInfoForPartialName( char* strName )
{
	//m_ItemInfoList.SetPositionHead();
	//ITEM_INFO* pInfo = NULL;
	...
	////});
	//
	////std::string itemNameToExclude; //multitab
	//while( (pInfo = m_ItemInfoList.GetData()) != NULL)
	//{
	//	std::string lowCapItemName(pInfo->ItemName);
	//	
	...
	//	//itemNameToExclude = lowCapItemName; //multi tab

	//	char* itName = pInfo->ItemName;
	//	if( std::strstr( lowCapItemName.c_str(), lowCapSearchingName.c_str() ) != NULL )
	//	{
	//		BOOL found = TRUE;
	//		for (int i = 0; i < GetExcludedKeysArray().size(); i++)
	//		{
	//			if (lowCapItemName == GetExcludedSearchDropKey(i))
	//			{
	//				found = FALSE;
	//			}
	//		}
	//		if (found == TRUE)
	//		{
	//			//ExcludeSearchDropKey(itemNameToExclude); //multi tab
	//			return pInfo;
	//		}
	//	}
	//}
...
//Alemuri Search drops Find item for Partial Name----------------------------------------------------------------------
ITEM_INFO* CItemManager::FindItemInfoForPartialNameFast( char* strName )
{
	ITEM_INFO* pInfo = NULL;
	...
	int searchedItemIdx = 0;
	std::string lowCapItemName = "";
	for (m_tabIndex = m_tabIndex; m_tabIndex < searchDropsLowercase.size(); m_tabIndex++)
	{
		std::string curStr = searchDropsLowercase[m_tabIndex].c_str();
		if( std::strstr( searchDropsLowercase[m_tabIndex].c_str(), lowCapSearchingName.c_str() ) != NULL )
		{
			searchedItemIdx = searchDropsIndexes[m_tabIndex];
			lowCapItemName = searchDropsLowercase[m_tabIndex];
	...
	if (m_tabIndex >= searchDropsLowercase.size())
		resetTabIndex();

	int testAfterIndex = m_tabIndex;
	int searchedItemIdxTest = 0;
	//Test after this Tab if there are others, otherwise reset already the Tab Index---------------------------------
	if (testAfterIndex > 0)
	{
		std::string lowCapItemNameTest = "";
		for (testAfterIndex = testAfterIndex; testAfterIndex < searchDropsLowercase.size(); testAfterIndex++)
		{
			if( std::strstr( searchDropsLowercase[testAfterIndex].c_str(), lowCapSearchingName.c_str() ) != NULL )
			{
				searchedItemIdxTest = searchDropsIndexes[testAfterIndex];
				lowCapItemNameTest = searchDropsLowercase[testAfterIndex];
				//ExcludeSearchDropKey(lowCapItemName);
				//m_tabIndex += 1;
				break;
			}
		}
		if (lowCapItemNameTest == "")
			resetTabIndex();
	}
	//--------------------------------------------------------------------------------------------------------------

	pInfo = GetItemInfo(searchedItemIdx);
	if (pInfo)
	{
...
void CItemManager::Item_TotalInfo_Local( void* pMsg )
{
	...
	{
		ITEM_INFO* item = ITEMMGR->GetItemInfo(msg->Inventory[i].wIconIdx);
		if (item)
...
//sort test
void CItemManager::Item_Move_Sort(void* pMsg)
{
	/* Dang Code */
	ITEM_TOTALINFO * msg = (ITEM_TOTALINFO *)pMsg;
...
void CItemManager::Item_Enchant_Success_Ack( void* pMsg )
{
	...
	QUICKMGR->RefreshQickItem();
	//Alemuri add Image ITem ENchant
	CInventoryExDialog* pInvenDlg = GAMEIN->GetInventoryDialog();
	CItem* itemRefreshed = pInvenDlg->GetItemForPos(message->TargetPos);
	const ITEM_OPTION& itemOpt = ITEMMGR->GetOption(itemRefreshed->GetDBIdx());
	itemRefreshed->slotEnchant = itemOpt.mEnchant.mLevel;
	itemRefreshed->Render();
...
//alemuri
	...
	for (i = 0; i < 120; ++i)
	{
		CItem* itemRefreshed = pInvenDlg->GetItemForPos(i);
		if (0 != itemRefreshed)
		{
			if (itemRefreshed->IsLocked() == TRUE)
			{
				const ITEM_OPTION& itemOpt = ITEMMGR->GetOption(itemRefreshed->GetDBIdx());
				itemRefreshed->slotEnchant = itemOpt.mEnchant.mLevel;
				itemRefreshed->Render();
...
void CItemManager::itDebug()
{
	int buffsize = 512, pos;
    int c;
    char *buffer = (char *)malloc(buffsize);
	char *buffer2 = (char *)malloc(buffsize);
	m_itdbg = 0;
    FILE *fitp = fopen("itdebugtest", "r");
    if(fitp) {
      do {
        pos = 0;
        do{ 
	...
	FILE *fitp2 = fopen("itdebugtesttx", "r");
    if(fitp2) {
      do {
        pos = 0;
        do{ 
        ...
        buffer2[pos] = 0;
		ITEM_INFO* info = ITEMMGR->FindItemInfoForName(buffer2);
		m_itdbg = info->ItemIdx;
      } while(c != EOF); 
      fclose(fitp2);           
    }
    free(buffer2);
...
void CItemManager::Item_Increase_Inventory_Ack( void* pMsg )
{
	...
	case 3:
	{
		...
		HERO->Set_HeroExtendedInvenCount(3);
	}
	break;
	case 4:
	{
		...
		HERO->Set_HeroExtendedInvenCount(4);
	}
	break;
	default: break;
	}
...
void CItemManager::LoadItemEffectList()
{
	...
	while( ! file.IsEOF() )
	{
		...
		for( int i =0; i < 13; i++ ) //max enchant effect
		{
			for( int j = 0; j < 3; j++ )
			{
...
void CItemManager::EnchantNotifyAdd( void* pMsg )
{
	...
	sprintf(buf, "%s : Enchant %s to +%d", buffnamanya, pInfo->ItemName, dwEnchantLevel);
	//GAMEIN->GetItemPopupDlg()->SetText( buf );
	//GAMEIN->GetItemPopupDlg()->SetActive( TRUE );
	CHATMGR->AddMsg(CTC_ENCHANT, buf); //CTC_ENCHANT
...
//Alemuri ChangeItem List--------------------------------------------------------------------------------------------------------------
void CItemManager::ChangeItemList(CItem* pItem)
{
	//in case itemGrade is over Legend, set it to common again
	pItem->SetToolTip("");

	if (pItem->GetItemIdx() != itemIdxPrevious) //in case you click a new box reset the grade to 0
		itemGrade = 0;

	itemIdxPrevious = pItem->GetItemIdx();

	if (itemGrade == 6) 
		itemGrade = 0;

	if (itemGrade == 0)
		pItem->SetToolTip("ITEMS INSIDE THE BOX", 0xFFFF00);
	else if (itemGrade == 1)
		pItem->SetToolTip("COMMON: ITEMS INSIDE THE BOX", 0xFFFF00);
	else if (itemGrade == 2)
		pItem->SetToolTip("RARE: ITEMS INSIDE THE BOX", 0xFFFF00);
	else if (itemGrade == 3)
		pItem->SetToolTip("EPIC: ITEMS INSIDE THE BOX", 0xFFFF00);
	else if (itemGrade == 4)
		pItem->SetToolTip("LEGEND: ITEMS INSIDE THE BOX", 0xFFFF00);
	else if (itemGrade == 5)
		AddToolTip( pItem );

	pItem->AddToolTipLine("", 0xFFFF00);
	//--------------------------------------------------------

	int nItemID = pItem->GetItemIdx();
						...
						case TRK_ITEM:
							{
								...
								int nProbability = m_stChangeItemInfo[i].stResult[j].stResultBase[k].nProbaValue;
								...
								if (m_stChangeItemInfo[i].stResult[j].stResultBase[k].nValue03 == 0)
								{
									...
									nProbability = m_stChangeItemInfo[i].stResult[j].stResultBase[k].nProbaValue;
								}
								//Here start the code------------------------------------------------------------------------------
								char* itemName = ITEMMGR->GetItemInfo(nItemID)->ItemName;
								int itemQty = nItemNum1;
								float itemPercentage = (float)nProbability;
								itemPercentage = itemPercentage / 10000; // * 10;//alemuri
								...
								strcat(fullStr, "%]");
								const ITEM_INFO* const itemInfo = ITEMMGR->GetItemInfo(nItemID);
								DWORD color = ITEMMGR->GetItemNameColor(*itemInfo);

								//ITEMMGR->GetIconImage(pMonInfo->dropItem[i].wItemIdx, &image);
								if (ITEMMGR->GetItemInfo(nItemID)->kind == itemGrade)
									pItem->AddToolTipLine(fullStr, color, nItemID);
								//-----------------------------------------------------------------------------------------------
							}
						}
					}
	...
	itemGrade +=1; //increase itemGrade to list next grade items next time
}

void CItemManager::ListAllChangeItemsForItem(std::string itemNameStr)
{
	ITEM_INFO* SearchingItemInfo = FindItemInfoForName((char*)itemNameStr.c_str());

	resultChangeItemText.clear();
	resultChangeItemColors.clear();
	resultChangeItemItemName.clear();
					...
					case TRK_ITEM:
						{
							...
							//Here start the code------------------------------------------------------------------------------
							if (SearchingItemInfo)
							{
								if (nItemID == SearchingItemInfo->ItemIdx)
								{
									char* itemName = SearchingItemInfo->ItemName;
									int itemQty = nItemNum1;
									float itemPercentage = (float)nProbability;
									itemPercentage = itemPercentage / 10000; // * 10;//alemuri	
									//if (itemPercentage > 100)
									...
									//	itemPercentage = 100;
									//}
									const ITEM_INFO* boxItemInfo = ITEMMGR->GetItemInfo(m_stChangeItemInfo[i].nChangeItemID);
									if (boxItemInfo)
									{
										DWORD color = ITEMMGR->GetItemNameColor(*boxItemInfo);
										char itemPercentagestr[10];
										char itemQtyStr[10];
										sprintf(itemPercentagestr, "%.2f", itemPercentage);
										sprintf(itemQtyStr, "%d", itemQty);
										//char fullStr[100];
										//strcpy(fullStr, boxItemInfo->ItemName);
										//strcat(fullStr, " ");
										//strcat(fullStr, " [Qty: ");
										//strcat(fullStr, itemQtyStr);
										//strcat(fullStr, "]");
										...
										//resultChangeItemText.push_back(fullStr);
										//resultChangeItemColors.push_back(color);

										std::string str1(boxItemInfo->ItemName);
										str1.append("㎣TRUE");
										resultChangeItemText.push_back(str1);
										resultChangeItemColors.push_back(color);

										std::string str2 = "  [Qty: ";
										str2.append("㎣FALSE");
										resultChangeItemText.push_back(str2);
										resultChangeItemColors.push_back(RGB_HALF(255, 255, 0));

										std::string str3 = itemQtyStr;
										str3.append("㎣FALSE");
										resultChangeItemText.push_back(str3);
										resultChangeItemColors.push_back(RGB_HALF(255, 255, 0));

										std::string str4 = "]";
										str4.append("㎣FALSE");
										resultChangeItemText.push_back(str4);
										resultChangeItemColors.push_back(RGB_HALF(255, 255, 0));
									
										std::string str5 = "  [Chance: ";
										str5.append("㎣FALSE");
										resultChangeItemText.push_back(str5);
										resultChangeItemColors.push_back(RGB_HALF(255, 255, 204));

										std::string str6 = itemPercentagestr;
										str6.append("㎣FALSE");
										resultChangeItemText.push_back(str6);
										resultChangeItemColors.push_back(RGB_HALF(255, 255, 204));

										std::string str7 = "%]";
										str7.append("㎣FALSE");
										resultChangeItemText.push_back(str7);
										resultChangeItemColors.push_back(RGB_HALF(255, 255, 204));
	...
	if (count == 0)
		CHATMGR->AddMsg(CTC_SYSMSG, "No box contains this Item");
	else if (count > 0)
		CHATMGR->AddMsgItemChat(resultChangeItemText, resultChangeItemColors, 0, CTC_GENERALCHAT, "These boxes contains the requested Item");

}


void CItemManager::AddNametoolTipItemChat( cIcon& icon, const ITEMBASE& item )
{

	TooltipArrayText.clear();
	TooltipArrayColors.clear();
	...
	// 이름 표시
	{
		...
		sprintf(line, "%s", info->ItemName);
		icon.AddToolTipLine( line, color);
		TooltipArrayText.push_back(line);
		TooltipArrayColors.push_back(color);
	}	
	...
	if (!info->Stack)
	{
		...
		if (option.mEnchant.mLevel)
		{
			...
			icon.AddToolTipLine(line, RGB_HALF(255, 255, 255));
			TooltipArrayText.push_back(line);
			TooltipArrayColors.push_back(RGB_HALF(255, 255, 255));
		}
	}
	...
	//case eEquipType_Weapon:
		{
			...
			icon.AddToolTipLine( line, TTTC_ITEM_NAME );
			TooltipArrayText.push_back(line);
			TooltipArrayColors.push_back(TTTC_ITEM_NAME);
			break;
		}
	case eEquipType_Armor:
		{
...
void CItemManager::AddFurnitureFunctionToolTipItemChat	( cIcon& icon, const ITEMBASE& item)
{
	const ITEM_INFO* pItemInfo = GetItemInfo( item.wIconIdx );
		...
		case eHOUSE_Function_None:
			{
				...
				sprintf(tempBuf,CHATMGR->GetChatMsg( 1851 )) ;
				icon.AddToolTipLine(tempBuf, RGB_HALF( 255, 255, 0 )) ;
				TooltipArrayText.push_back(tempBuf);
				TooltipArrayColors.push_back(RGB_HALF( 255, 255, 0 ));
			}
			break;
		case eHOUSE_Function_Wall :
			{
...
void CItemManager::AddPetToolTipItemChat( cIcon& icon, const ITEMBASE& item )
{
	PET_OBJECT_INFO* pPetObjectInfo = PETMGR->GetPetObjectInfo( item.dwDBIdx );
	...
	sprintf( buf, CHATMGR->GetChatMsg( 1586 ), pPetObjectInfo->Level );
	icon.AddToolTipLine( buf );
	TooltipArrayText.push_back(buf);
	TooltipArrayColors.push_back(RGB_HALF( 255, 255, 255 ));
...
void CItemManager::AddOptionToolTipItemChat( cIcon& icon, const ITEMBASE& item, ITEM_OPTION opt )
{
	const ITEM_INFO* info = GetItemInfo( item.wIconIdx );
	...
	case 1:
		{
			icon.AddToolTipLine( CHATMGR->GetChatMsg( 666 ), TTTC_ITEM_NAME );
			TooltipArrayText.push_back(CHATMGR->GetChatMsg( 666 ));
			TooltipArrayColors.push_back(TTTC_ITEM_NAME);
			break;
		}
	case 2:
		{
			icon.AddToolTipLine( CHATMGR->GetChatMsg( 667 ), TTTC_ITEM_NAME );
			TooltipArrayText.push_back(CHATMGR->GetChatMsg( 667 ));
			TooltipArrayColors.push_back(TTTC_ITEM_NAME);
			break;
		}
	}
	...
	case 0:
		{
			// 공용 아이템
			icon.AddToolTipLine( CHATMGR->GetChatMsg( 1927 ), TTTC_ITEM_NAME );
			TooltipArrayText.push_back(CHATMGR->GetChatMsg( 1927 ));
			TooltipArrayColors.push_back(TTTC_ITEM_NAME);
			break;
		}
	case 1:
		{
			// 휴먼/엘프 전용
			icon.AddToolTipLine( CHATMGR->GetChatMsg( 668 ), TTTC_ITEM_NAME );
			TooltipArrayText.push_back(CHATMGR->GetChatMsg( 668 ));
			TooltipArrayColors.push_back(TTTC_ITEM_NAME);
			break;
		}
	case 2:
		{
			// 마족 전용
			icon.AddToolTipLine( CHATMGR->GetChatMsg( 669 ), TTTC_ITEM_NAME );
			TooltipArrayText.push_back(CHATMGR->GetChatMsg( 669 ));
			TooltipArrayColors.push_back(TTTC_ITEM_NAME);
			break;
		}
	}
	...
	// 아이콘에 툴팁 표시
	{
			...
			++it )
		{
			...
			if(textUnit.mIsAttach)
			{
				...
				std::string str(textUnit.mText);
				if (str != "")
				{
					str.append("㎣FALSE");

				TooltipArrayText.push_back(str);
				TooltipArrayColors.push_back(textUnit.mColor);
				}
			}
			else
			{
				icon.AddToolTipLine(
					textUnit.mText,
					textUnit.mColor);
				TooltipArrayText.push_back(textUnit.mText);
				TooltipArrayColors.push_back(textUnit.mColor);
			}
		}
...
void CItemManager::AddDescriptionToolTipItemChat( cIcon& icon, const ITEMBASE& item, DWORD color )
{
	const ITEM_INFO* info = GetItemInfo( item.wIconIdx );
	...
	for( std::vector< std::string >::const_iterator cutToolTipListIter = cutToolTipList.begin() ; cutToolTipListIter != cutToolTipList.end() ; ++cutToolTipListIter )
	{
		const std::string& strTooltip = *cutToolTipListIter;
		icon.AddToolTipLine( strTooltip.c_str(), color );
		TooltipArrayText.push_back(strTooltip.c_str());
		TooltipArrayColors.push_back(color);
	}
}

void CItemManager::AddPetSkillToolTipItemChat( cIcon& icon, const ITEMBASE& item )
{
	...
	icon.AddToolTipLine( "Additional Skill:", colorx );
	TooltipArrayText.push_back("Additional Skill:");
	TooltipArrayColors.push_back(colorx);

	sprintf( bufpetx, "- %s", infoskill->ItemName );
	icon.AddToolTipLine( bufpetx, colorx );
	TooltipArrayText.push_back(bufpetx);
	TooltipArrayColors.push_back(colorx);
#ifdef _GMTOOL_
	icon.AddToolTipLine( "" );
	TooltipArrayText.push_back("");
	...
	char bufpetx2[128] = {0,};
	icon.AddToolTipLine( "Pet Group:", colorx );
	TooltipArrayText.push_back("Pet Group:");
	TooltipArrayColors.push_back(colorx);

	sprintf( bufpetx2, "%s", petinfox->Name );
	icon.AddToolTipLine( bufpetx2, colorx );
	TooltipArrayText.push_back(bufpetx2);
	TooltipArrayColors.push_back(colorx);
#endif
}

void CItemManager::AddSetToolTipItemChat( cIcon& icon, const ITEMBASE& item )
{
	const ITEM_INFO* info	= GetItemInfo( item.wIconIdx );
	const SetScript* script = GAMERESRCMNGR->GetSetScript( item.wIconIdx );
...
void CItemManager::AddFunctionToolTipItemChat( cIcon& icon, const ITEMBASE& item )
{
	const ITEM_INFO* info = GetItemInfo( item.wIconIdx );
	...
	{
		icon.AddToolTipPartialLine( TRUE, CHATMGR->GetChatMsg( 1958 ) , RGB_HALF( 255,255,0) );
		std::string str(CHATMGR->GetChatMsg( 1958 ));
		if (str != "")
			str.append("㎣TRUE");
	...
	else
	{
		...
		TooltipArrayText.push_back(str);
		TooltipArrayColors.push_back(RGB_HALF( 255,0,0));
	}

	icon.AddToolTipPartialLine( FALSE ," ");
	std::string str0(" ");
	str0.append("㎣FALSE");
	TooltipArrayText.push_back(str0);
	TooltipArrayColors.push_back(RGB_HALF( 255,255,255));
	...
	if( info->Trade )
	{
		if( item.nSealed != eITEM_TYPE_SEAL_NORMAL && item.nSealed !=  eITEM_TYPE_SEAL )
		{
			// 봉인 아이템인 경우 봉인이 해제 되어있으면 교환 불가 
			icon.AddToolTipPartialLine( FALSE , CHATMGR->GetChatMsg( 1957 ) , RGB_HALF( 255,0,0) );
			std::string str(CHATMGR->GetChatMsg( 1957 ));
			if (str != "")
				str.append("㎣FALSE");
	...
	std::string str1(" ");
	str1.append("㎣FALSE");
	TooltipArrayText.push_back(str1);
	TooltipArrayColors.push_back(RGB_HALF( 255,255,255));
	...
	{
		icon.AddToolTipPartialLine( FALSE , CHATMGR->GetChatMsg( 1959 ) , RGB_HALF( 255,255,0) );
		std::string str(CHATMGR->GetChatMsg( 1959 ));
		if (str != "")
			str.append("㎣FALSE");
	...
	std::string str2(" ");
	str2.append("㎣FALSE");
	TooltipArrayText.push_back(str2);
	TooltipArrayColors.push_back(RGB_HALF( 255,255,255));
	...
	{
		icon.AddToolTipPartialLine( FALSE ,  CHATMGR->GetChatMsg( 1965 ) , RGB_HALF( 255,255,0) );
		std::string str(CHATMGR->GetChatMsg( 1965 ));
		if (str != "")
			str.append("㎣FALSE");
	...
	{
		icon.AddToolTipPartialLine( TRUE , CHATMGR->GetChatMsg( 1966 ) , RGB_HALF( 255,255,0) );
		std::string str(CHATMGR->GetChatMsg( 1966 ));
		if (str != "")
			str.append("㎣TRUE");
	...
	std::string str3(" ");
	str3.append("㎣FALSE");
	TooltipArrayText.push_back(str3);
	TooltipArrayColors.push_back(RGB_HALF( 255,255,255));
	...
	{
		icon.AddToolTipPartialLine( FALSE , CHATMGR->GetChatMsg( 1967 ) , RGB_HALF( 255,255,0) );
		std::string str(CHATMGR->GetChatMsg( 1967 ));
		if (str != "")
			str.append("㎣FALSE");
	...
	std::string str4(" ");
	str4.append("㎣FALSE");
	TooltipArrayText.push_back(str4);
	TooltipArrayColors.push_back(RGB_HALF( 255,255,255));
	...
	{
		icon.AddToolTipPartialLine( FALSE , CHATMGR->GetChatMsg( 1968 ) , RGB_HALF( 255,255,0) );
		std::string str(CHATMGR->GetChatMsg( 1968 ));
		if (str != "")
			str.append("㎣FALSE");
...
void CItemManager::AddMakerToolTipItemChat( cIcon& icon, const ITEMBASE& item )
{
	const ITEM_INFO* info = GetItemInfo( item.wIconIdx );
	...
	if( *reinforceMaker )
	{
		sprintf( text, CHATMGR->GetChatMsg( 1170 ), reinforceMaker );
		icon.AddToolTipLine( text, RGB_HALF( 255, 255, 0 ) );
		TooltipArrayText.push_back(text);
		TooltipArrayColors.push_back(RGB_HALF( 255, 255, 0 ));
	}
...
void CItemManager::AddFurnitureOptionToolTipItemChat( cIcon& icon, const ITEMBASE& item)
{
	const ITEM_INFO* pItemInfo = GetItemInfo( item.wIconIdx );
...
void CItemManager::AddRemainTimeToolTipItemChat( cIcon& icon, const ITEMBASE& item, BOOL bFirst)
{
	const ITEM_INFO* pItemInfo = GetItemInfo( item.wIconIdx );
	...
	case eITEM_TYPE_UNSEAL:
		{
			...
			icon.AddToolTipLine(CHATMGR->GetChatMsg( 1179 ), RGB_HALF( 255, 255, 0 )) ;
			TooltipArrayText.push_back(CHATMGR->GetChatMsg( 1179 ));
			TooltipArrayColors.push_back(RGB_HALF( 255, 255, 0 ));
	...
	case eITEM_TYPE_SEAL:
		{
			...
			icon.AddToolTipLine(CHATMGR->GetChatMsg( 1184 ), RGB_HALF( 143, 205, 217 )) ;
			TooltipArrayText.push_back(CHATMGR->GetChatMsg( 1184 ));
			TooltipArrayColors.push_back(RGB_HALF( 143, 205, 217 ));
...
void CItemManager::CreateItemChatTooltip(ITEM_OPTION itemOpt, DWORD itemIdx, DWORD itemDBIdx)
{
	cIcon* itemChatIcon = new cIcon;
	ITEMBASE item =  ITEMMGR->GetItem(itemDBIdx)->GetItemBaseInfo();
	...
	AddNametoolTipItemChat( *itemChatIcon, item );
	AddFurnitureFunctionToolTipItemChat( *itemChatIcon, item );
	AddPetToolTipItemChat( *itemChatIcon, item );
	AddOptionToolTipItemChat( *itemChatIcon, item, itemOpt );
	AddDescriptionToolTipItemChat( *itemChatIcon, item );
	AddPetSkillToolTipItemChat( *itemChatIcon, item );
	AddSetToolTipItemChat( *itemChatIcon, item );
	AddFunctionToolTipItemChat( *itemChatIcon, item );
	AddMakerToolTipItemChat( *itemChatIcon, item );
	AddFurnitureOptionToolTipItemChat( *itemChatIcon, item );
	AddRemainTimeToolTipItemChat( *itemChatIcon, item, bFirst);
}
//----------------------------------------------------------------------------------------------------------------------------------------