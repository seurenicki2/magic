...
void CStorageDialog::LoadStorageListInfo()
{
	...
	for(int n = 0;n< MAX_BASESTORAGELIST_NUM; ++n)
	{
		...
		m_StorageListInfo.Storage[n].MaxMoney = file.GetDwordEx();
		m_StorageListInfo.Storage[n].BuyPrice = file.GetDwordEx();
	}
...
void CStorageDialog::SetStorageMoney(MONEYTYPE Money)
{
