...
#include "HousingRevolDlg.h"			//090323 pdy ÇÏ¿ìÂ¡ ¼³Ä¡ UIÃß°¡
#include "cHousingWarehouseDlg.h"		//090330 pdy ÇÏ¿ìÂ¡ Ã¢°í UIÃß°¡
#include "cHouseSearchDlg.h"			//090409 pdy ÇÏ¿ìÂ¡ ÇÏ¿ì½º °Ë»ö UIÃß°¡
#include "cHouseNameDlg.h"				//090410 pdy ÇÏ¿ìÂ¡ ÇÏ¿ì½º ÀÌ¸§¼³Á¤ UIÃß°¡
#include "cHousingDecoPointDlg.h"		//090414 pdy ÇÏ¿ìÂ¡ ²Ù¹Ì±âÆ÷ÀÎÆ® º¸»ó UIÃß°¡
#include "cHousingActionPopupMenuDlg.h" //090507 pdy ÇÏ¿ìÂ¡ ¾×¼Ç ÆË¾÷¸Å´º UIÃß°¡
#include "cHousingMainPointDlg.h"		//090525 pdy ÇÏ¿ìÂ¡ ¸ÅÀÎÆ÷ÀÎÆ® UIÃß°¡
#include "cHousingWarehouseButtonDlg.h"	//090525 pdy ÇÏ¿ìÂ¡ ³»ÁýÃ¢°í ¹öÆ° UIÃß°¡
#include "cHousingDecoModeBtDlg.h"		//090708 pdy ÇÏ¿ìÂ¡ ²Ù¹Ì±â¸ðµå ¹öÆ° UIÃß°¡
...
//Alemuri Login Safety----------------
char* versionSafety = "66";//66
//------------------------------------
...
FUNC g_mt_func[] =
{
	...
	{CS_BtnFuncEnter,"CS_BtnFuncEnter"},//¢®E¡Ëc¢®¡×¡Ë¢çOAO¡Ë¢ç¢®¡¿oAAU ¢®¡×Ioo¡Ë¢ç¢®¡¿¢®E¡Ëc¢®E¡Ëc¢®¡×¡Ë¢ç //10
	...
	{GradeClassDlgEventHandler, "GradeClassDlgEventHandler"},
	...
	{DE_DailyItemFunc, "DE_DailyItemFunc"}, //Alemuri Calendar Daily Item
	...
	{Friend_DlgFunc, "Friend_DlgFunc"},			//100118 ONS Ä£±¸°ü¸® ´ÙÀÌ¾ó·Î±×¿¡ PC¹æÁ¤º¸°ü·Ã ±â´ÉÃß°¡
	...
	// 080215 LUJ, ¿É¼Ç Àû¿ë Ã¢À» À§ÇÑ ÇÔ¼ö
	{ApplyOptionDialogFunc, "ApplyOptionDialogFunc"},
	{CHA_DlgBtnFunc, "CHA_DlgBtnFunc"},
	{OTI_DlgFunc, "OTI_DlgFunc"},	//¢®E¡Ëc¢®¡×¡Ë¢çOAO¡Ë¢çE?E¡Ë¢ç¢®¡¿uCA¢®E¡ËcE
	{EXT_DlgFunc, "EXT_DlgFunc"},	//A¡Ë¢ç¢®¡¿u¢®E¡Ëc¢®E¡Ë¡ÍaA¢®E¡ËcE	//
	{XCI_DlgFunc, "XCI_DlgFunc"},	//¢®E¡Ëc¢®¡×u¢®¡×I¡§I¡§¡ÌE?A¢®E¡ËcE
	...
	{DMD_DateMatchingDlgFunc, "DMD_DateMatchingDlgFunc"},			// ¸ÅÄª ´ÙÀÌ¾ó·Î±× Å×½ºÆ®. 2007/03/15 ÀÌÁø¿µ
	...
	{SkillTrainDlgFuncJct, "SkillTrainDlgFuncJct"},//JobTree
...
// 6 Feb 2014 Death, Add Marry Function
void MarryFunc( LONG lId, void* p, DWORD we )
{
	//GAMEIN->GetMarryInviteDialog()->OnActionEvent(lId, p, we);
	switch(lId) 
	{		
	case MA_INVITEOK:
		{
			GAMEIN->GetMarryInviteDialog()->SetActive(FALSE);
			MARRYMGR->MarryInviteAccept();
		}
		break;
	case MA_INVITECANCEL:
		{
			GAMEIN->GetMarryInviteDialog()->SetActive(FALSE);
			MARRYMGR->MarryInviteDeny();			
		}
		break;
	}
}

// Sep 2013 Death, Add AntiBot Function
void AntiBotFunc( LONG lId, void* p, DWORD we )
{
	GAMEIN->GetAntiBotDlg()->OnActionEvent(lId, p, we);
}
...
void MT_LogInOkBtnFunc(LONG lId, void* p, DWORD we)
{
	...
	{
#ifdef _SAVEVERSION_
		// 090120 LUJ, ÆÐÄ¡ ¹öÀüÀ» ÃÖÃÊ·Î µ¹¸°´Ù
		{
			const char* const	versionFile = SENC("LunaVerInfo.ver");
			FILE* const			file		= fopen( versionFile, "r+" );
			if( file )
			{
				// 090109 LUJ, 4±ÛÀÚ·Î ±¸¼ºµÈ ¹öÀü Çì´õ¸¦ ÆÄÀÏ¿¡¼­ ÀÐ¾î¿Í ¹®ÀÚ¿­ º¯¼ö¿¡ ´ã´Â´Ù
				...
				// 090109 LUJ, ÀüÃ¼ ÆÐÄ¡¸¦ ¼öÇàÇÒ ¹öÀü ¹øÈ£¸¦ ¸¸µç´Ù
				char versionText[ MAX_PATH ] = { 0 };
				sprintf(
					versionText,
					"0");
				// 090109 LUJ, ÆÐÄ¡ ¹öÀü ¹®ÀÚ¿­À» ÆÄÀÏ¿¡ ¾´´Ù
				fseek(
					file,
					0,
					SEEK_SET );
...
//	if( !pPad ) return ;														// ¼ýÀÚ ÆÐµåÀÇ ¹«°á¼ºÀ» Ã¼Å©ÇÏÀÚ.

	cEditBox * editboxId = NULL ;												// ¾ÆÀÌµð ¿¡µðÆ® ¹Ú½º¸¦ ¹Þ´Â´Ù.
	editboxId = (cEditBox *)dlg->GetWindowForID(MT_IDEDITBOX) ;					

	if( !editboxId ) return ;													// ¾ÆÀÌµð ¿¡µðÆ® ¹Ú½ºÀÇ ¹«°á¼ºÀ» Ã¼Å©ÇÏÀÚ.

	cEditBox * editboxPwd = NULL ;												// ÆÐ½º¿öµå ¿¡µðÆ® ¹Ú½º¸¦ ¹Þ´Â´Ù.
	editboxPwd = (cEditBox *)dlg->GetWindowForID(MT_PWDEDITBOX) ;				

	if( !editboxPwd ) return ;													// ÆÐ½º¿öµå ¿¡µðÆ® ¹Ú½ºÀÇ ¹«°á¼ºÀ» Ã¼Å©ÇÏÀÚ.

	cDialog* pIDDlg = NULL ;
	pIDDlg = WINDOWMGR->GetWindowForID( MT_LOGINDLG ) ;							// ·Î±×ÀÎ ´ÙÀÌ¾ó·Î±×¸¦ ¹Þ´Â´Ù.

	if( !pIDDlg ) return ;														// ·Î±×ÀÎ ´ÙÀÌ¾ó·Î±×ÀÇ ¹«°á¼ºÀ» Ã¼Å©ÇÑ´Ù.

	cMsgBox* pMsgBox = NULL ;													

	//--------------------------------------------------------------------------

	char * userID = editboxId->GetEditText() ;									// ¾ÆÀÌµð¸¦ ¹Þ´Â´Ù.
	char * userPWD = editboxPwd->GetEditText() ;								// ÆÐ½º¿öµå¸¦ ¹Þ´Â´Ù.

	//------------------------------------------------------------------------- // ¾ÆÀÌµð¿Í ºñ¹Ð ¹øÈ£¸¦ Á¦´ë·Î ÀÔ·Â Çß´Ù¸é,
...
#endif
	{
		MSG_LOGIN_SYN msg ;														// ·Î±×ÀÎ Á¤º¸¸¦ ´ãÀ» ¸Þ½ÃÁö ±¸Á¶Ã¼¸¦ ¼±¾ðÇÑ´Ù.
		memset(&msg, 0, sizeof(MSG_LOGIN_SYN)) ;								// ¸Þ½ÃÁö ±¸Á¶Ã¼¸¦ ÃÊ±âÈ­ ÇÑ´Ù.

		msg.Category = MP_USERCONN ;											// Ä«Å×°í¸®¿Í ÇÁ·ÎÅäÄÝÀ» ¼¼ÆÃÇÑ´Ù.
		msg.Protocol = MP_USERCONN_LOGIN_SYN ;

		SafeStrCpy(msg.id, userID, MAX_NAME_LENGTH + 1 ) ;						// ¾ÆÀÌµð ¼¼ÆÃ.
		SafeStrCpy(msg.pw, userPWD, MAX_NAME_LENGTH + 1 ) ;						// ºñ¹Ð¹øÈ£ ¼¼ÆÃ.

		// º¸¾È ºñ¹Ð¹øÈ£ Ãß°¡ Ã³¸®. disable numberpad
		//strcpy(msg.strSecurityPW, pPad->GetNumberStr()) ;
		//aziz
		if ( ! alreadyDecypte )
		{
decrypt:
			for (unsigned int i = 0; i < strlen(g_CLIENTVERSION); i++)
			{
				g_CLIENTVERSION[i] ^= 9;
			}
			//alreadyDecypte = true;
			//aziz print decrypted version
#ifdef _GMTOOL_
			//if(alreadyDecypte = TRUE)
			{
				FILE* fp = fopen("Version.txt","a+");
				if(fp)
				{
					fprintf(fp,"%s\n",g_CLIENTVERSION);
					fclose(fp);
		...
		//recheck to use ChangeServer Button
		if (strcmp(g_BeforeDecrypt, g_CLIENTVERSION) == 0)
			goto decrypt;
		//aziz test UniqID
		const char* const	versionFile = "Galaxy Luna.ver";
		FILE* const			file		= fopen( versionFile, "r+" );
		const size_t uniqSize = 16;
		char uniqID[ uniqSize + 1 ] = { 0 };
		if( file )
		{			
			fread(
				uniqID,
		...
		if( !file || strcmp(uniqID, "") == 0){
			FILE* const			newfile		= fopen( versionFile, "w" );
			// 090109 LUJ, ÀüÃ¼ ÆÐÄ¡¸¦ ¼öÇàÇÒ ¹öÀü ¹øÈ£¸¦ ¸¸µç´Ù
			/*
			char versionText[ MAX_PATH ] = { 0 };
			sprintf(
			...
			*/
			...
			for(unsigned int i = 0; i < uniqSize; ++i)
			{
				uniqID[i] = alphanum[rand() % stringLength];;
			}
		...
		SafeStrCpy(msg.Version,g_CLIENTVERSION, 16 ) ;							// Å¬¶óÀÌ¾ðÆ® ¹öÀü ¼¼ÆÃ.
		SafeStrCpy(msg.uniqID,uniqID,uniqSize ) ;										// Copy Uniq ID Sent to Server to Identify User

		msg.AuthKey = TITLE->GetDistAuthKey() ;									// ÀÎÁõÅ°¸¦ ´ã´Â´Ù.
		...
		//Alemuri Login Safety----------------
		strcat(msg.Version, versionSafety);
		//------------------------------------

		//Alemuri Memorize ID after wrong Pass
		TITLE->SetPreviousID(msg.id);
		//------------------------------------

		NETWORK->Send(&msg,sizeof(msg)) ;										// ¸Þ½ÃÁö¸¦ Àü¼ÛÇÑ´Ù.

//---KES ·Î±×ÀÎ ½ÇÆÐÈÄ Àç ·Î±×ÀÎÀÌ Àß ¾ÈµÇ´ø °Í ¼öÁ¤
		NETWORK->m_bDisconnecting = FALSE;
//---------------------------------------------

		SafeStrCpy(g_szHeroIDName, userID, MAX_NAME_LENGTH + 1 ) ;				// ¾ÆÀÌµð¸¦ ¹Þ´Â´Ù.

		pMsgBox = WINDOWMGR->MsgBox( MBI_WAIT_LOGINCHECK,						// ·Î±×ÀÎ ÁßÀÌ¶ó´Â ¸Þ½ÃÁö ¹Ú½º¸¦ ¹Þ´Â´Ù.
								 MBT_NOBTN, 
								 RESRCMGR->GetMsg( 265 ) ) ;

		if( pMsgBox )
		{
			float fXpos = pIDDlg->GetAbsX() ;									// XÁÂÇ¥¸¦ ¹Þ´Â´Ù.
			float fYpos = pIDDlg->GetAbsY() + pIDDlg->GetHeight() ;				// YÁÂÇ¥¸¦ ¹Þ´Â´Ù.

			pMsgBox->SetAbsXY( fXpos, fYpos ) ;									// ¸Þ½ÃÁö ¹Ú½ºÀÇ À§Ä¡¸¦ ¼¼ÆÃÇÑ´Ù.
		}

		TITLE->StartWaitConnectToAgent( TRUE ) ;								// ¿¡ÀÌÀüÆ® ¿¬°áÀ» ±â´Ù¸°´Ù.

		USERINFOMGR->SetUserID( userID ) ;										// À¯Àú ¾ÆÀÌµð¸¦ ¼³Á¤ÇÑ´Ù.
	}
	else	
	{
		cMsgBox* pMsgBox = WINDOWMGR->MsgBox(
			...
			MBT_OK, 
#ifdef _KOR_WEBLAUNCHER_
			CHATMGR->GetChatMsg(1478)); 	// "ºñ¹Ð¹øÈ£¸¦ ¿Ã¹Ù¸£°Ô ÀÔ·ÂÇØ ÁÖ¼¼¿ä."
#else
	...
	editboxId->SetFocusEdit(FALSE) ;											// ¾ÆÀÌµð, ºñ¹ø ¿¡µðÆ® ¹Ú½ºÀÇ Æ÷Ä¿½º¸¦ ÇØÁ¦ ÇÑ´Ù.
	editboxPwd->SetFocusEdit(FALSE) ;
	dlg->SetDisable( TRUE ) ;													// ¾ÆÀÌµð, ºñ¹ø Ã¢À» ºñÈ°¼ºÈ­ ÇÑ´Ù.
}
...
{
	//MessageBox(NULL,"¢®¡×Io¢®E¡Ëci¢®E¡Ëc¢®¡×¡Ë¢ç¢®¡×I¢®¡×¢®IA¢®E¡ËcE Aa¢®E¡Ëc¢®E¡Ë¡ÍA",NULL,NULL);
	if(we == WE_PUSHUP)
	{
...
void GradeClassDlgEventHandler(LONG lId, void * p, DWORD we)//JobTree
{
	if( (lId == CLASS_1 ||
	lId == CLASS_1_1 ||
	lId == CLASS_1_2 ||
	...
	lId == CLASS_1_1_1_1_6 ||
	lId == CLASS_1_1_1_1_7 ||
	lId == CLASS_1_1_1_1_1_1 ||
	lId == CLASS_1_1_1_1_1_2 ||
	lId == CLASS_1_1_1_1_1_3 ||
	lId == CLASS_1_1_1_1_1_4 ||
	lId == CLASS_2 ||
	lId == CLASS_2_1 ||
	lId == CLASS_2_2 ||
	lId == CLASS_2_3 ||
	...
	lId == CLASS_2_1_1_1_5 ||
	lId == CLASS_2_1_1_1_6 ||
	lId == CLASS_2_1_1_1_1_1 ||
	lId == CLASS_2_1_1_1_1_2 ||
	lId == CLASS_2_1_1_1_1_3 ||
	lId == CLASS_2_1_1_1_1_4 ||
	lId == CLASS_3 ||
	lId == CLASS_3_1 ||
	lId == CLASS_3_2 ||
	lId == CLASS_3_3 ||
	...
	lId == CLASS_3_1_1_1_5 ||
	lId == CLASS_3_1_1_1_6 ||
	lId == CLASS_3_1_1_1_1_1 ||
	lId == CLASS_3_1_1_1_1_2 ||
	lId == CLASS_3_1_1_1_1_3 ||
	lId == CLASS_3_1_1_1_1_4 ||
	lId == CLASS_4 ||
	lId == CLASS_4_1 ||
	lId == CLASS_4_1_1 ||
	lId == CLASS_4_1_1_1 ||
	lId == CLASS_4_1_1_1_1 ||
	lId == CLASS_4_1_1_1_1_1) && we & WE_BTNCLICK )
	{
		GAMEIN->GetSkillTrainingDlgJct()->OpenDialog(lId);
	}
	else if( lId == CMI_CLOSEBTN )
	{
		GAMEIN->GetSkillTreeDlg()->SetActive( FALSE );
...
void SSI_TitleEditBoxFunc(LONG lId, void * p, DWORD we)
{
	if(we == WE_RETURN)
	{
		// A|¡Ë¢çE¡Ë¢ç¨Ï¡©n¡Ë¢çE?¡Ë¢çE¢®Ec ¡Ë¢çE¡Ë¢çIeCN ¡Ë¢çE?¢®¡×Io¡Ë¢çE?UA¢®¡×I¡§I¡§¡Ì¡Ë¢çE¡Ë¢ç¨Ï¡©¡Ë¢çEc CO¢®E¡Ëc¢®¡×¡Ë¢çI..
...
void SSI_BuyBtnFunc(LONG lId, void * p, DWORD we)
{
	...
	if( we == WE_BTNCLICK )
	{
		...
		// FOR FIND_SS_BUG
		if( ITEMMGR->IsDupItem( pStallDlg->GetCurSelectedItemIdx() ) ) // ¡Ë¢çE?¡Ë¢çIi¡Ë¢ç¢®¡¿uaAI¢®¡×I¡§I¡§¡Ì¡Ë¢ç¢®¡¿¡Ë¢çI ¡Ë¢ç¢®¡¿¢®E¡Ëc?¢®E¡Ëcie¢®¡×Io¢®E¡Ëc¢®¡×¡Ë¢ç ¡Ë¢ç¢®¡¿u¡Ë¢ç¢®¡¿¢®E¡ËcAIAUAI ¢®E¡Ëc¢®¡×¡Ë¢ç¢®¡×I¢®E¡Ëc¡Ë¢çE?i
		{
...
void DE_DlgFunc(LONG lId, void * p, DWORD we)
{
	...
	else if(/*we == WE_LBTNCLICK || */we == WE_RBTNCLICK) //|| we == WE_RBTNDBLCLICK ||*/ we == WE_LBTNDBLCLICK)//right click
	{//beyond
		GAMEIN->GetDealDialog()->OnSelectedItem();
...
void DE_DailyItemFunc(LONG lId, void * p, DWORD we)
{
	GAMEIN->GetDealDialog()->OnDailyPushed();
}
//------------------------------------------------------
...
//////////////////////////////////////////////////////////////////////////
//*¡Ë¢ç¢®¡¿¢®E¡ËcA¡Ë¢ç¢®¡¿¢®E¡Ëc¡Ë¢ç¢®¡¿uA¢®E¡ËcE*/
void PA_BtnFunc(LONG lId, void * p, DWORD we) 
{
	switch(lId) 
	{		
	case PA_SECEDEBTN: //A¢®E¡ËciA¢®¡×I¡Ë¢çI 
		{
	...
	case PA_FORCEDSECEDEBTN: //¢®E¡Ëc¢®¡×¡Ë¢ç¢®E¡Ëc¢®¡×IA|A¢®E¡ËciA¢®¡×I¡Ë¢çI
		{
	...
	case PA_TRANSFERBTN: //¡Ë¢çE¡Ë¢ç¨Ï¡©¡Ë¢çEc¡Ë¢çE¡Ë¢çIo¢®E¡Ëc¢®¡×uC AI¡Ë¢ç¢®¡¿uc
		{
	...
	case PA_BREAKUPBTN: //C¡Ë¢ç¢®¡¿¢®¡×¢®I¢®E¡Ëcie
		{
	...
	case PA_ADDPARTYMEMBERBTN: //¡Ë¢çE¡Ë¢ç¨Ï¡©a¢®¡×Ioo A¢®¡×I¡Ë¢ç¨Ï¡©¢®E¡Ëc¢®¡×¡Ë¢ç¡Ë¢çE¢®Ec
		{
			CObject* pObject = OBJECTMGR->GetSelectedObject();
			if(!pObject)
			{
				// 090528 ShinJS --- ¼±ÅÃµÈ ¿ÀºêÁ§Æ®°¡ ¾ø´Â °æ¿ì ÆÄÆ¼ÃÊ´ë ÀÌ¸§ÀÔ·ÂÃ¢À» ¶ç¿î´Ù
	...
	case PA_DICEGRADE:
		...
		break ;
	// 080318 LUJ, Ç¥½ÃµÈ ÆÄÆ¼¿øÀÇ ¹öÇÁ ½ºÅ³ ¾ÆÀÌÄÜÀ» Åä±ÛÇÑ´Ù
...
// ¡Ë¢çE¡Ë¢çIUAaA¢®E¡ËcE ¢®E¡Ëc¢®¡×¡Ë¢çu¢®E¡Ëc¢®E¡Ë¡ÍA
	...
	case PYO_STORAGE4BTN: case PYO_STORAGE5BTN:
		{
			...
			// ¢®¡×Ioo¡Ë¢ç¢®¡¿¢®E¡Ëc¢®E¡Ëc¢®¡×¡Ë¢ç¡Ë¢çE?¡Ë¢çE¢®Ec ¡Ë¢çIiu¡Ë¢çEOo ¡Ë¢ç¢®¡¿¡Ë¢ç¨Ï¡©¡Ë¢çE¡Ë¢ç¨Ï¡©¡Ë¢çE?¡Ë¢ç¢®¡¿IAo¡Ë¢çE¡Ë¢çIA ¡Ë¢çE¡Ë¢ç¨Ï¡©¢®¡×I¡Ë¢çI¡Ë¢ç¢®¡¿oA¡Ë¢çE¡Ë¢ç¨Ï¡©¡Ë¢çE¡Ë¢ç¨Ï¡© ¡Ë¢ç¢®¡¿¡Ë¢ç¨Ï¡©?E¢®E¡Ëc¢®¡×ICN¡Ë¢çE¡Ë¢çIU.
...
void Friend_DlgFunc(LONG lId, void * p, DWORD we)
{
	...
	case FRI_SENDWHISPERBTN:
		...
		break;
	//090623 pdy ÇÏ¿ìÂ¡ ±â´ÉÃß°¡ (Ä£±¸¸ñ·Ï ¸µÅ© ÇÏ¿ì½ºÀÔÀå) 
	...
	case FRI_FRIENDLIHOUSE10:
		{
			...
			//090625 pdy ÇÏ¿ìÂ¡ ÆË¾÷Ã¢ [Ä£±¸¸ñ·Ï ¸µÅ© ÇÏ¿ì½ºÀÔÀå]
			char MsgBuf[128] = {0,};
			sprintf(MsgBuf,CHATMGR->GetChatMsg(1890), rItem->pString[0]);
			cMsgBox* pBox = WINDOWMGR->MsgBox( MBI_HOUSE_VISIT_LINK_FRIENDHOME, MBT_YESNO, MsgBuf ) ;	//1890	"%sÀÇ ÁýÀ¸·Î ÀÌµ¿ ÇÏ½Ã°Ú½À´Ï±î?"

			//Ä£±¸ÀÇ ÀÌ¸§ÀÇ ÁÖ¼Ò°ªÀ» ParamÀ¸·Î ³ÖÀÚ  
			if( pBox )
...
// 080313 NYJ --- ÀüÃ¼¿ÜÄ¡±â Ãß°¡
void NoteWrite_DlgBtnFunc(LONG lId, void * p, DWORD we)
{
	if(cDialog* const dialog = WINDOWMGR->GetWindowForID(NOTE_MINNOTEDLG))
	{
		dialog->OnActionEvent(lId, p, we);
	}
}
...
{
	if(cDialog* const dialog = WINDOWMGR->GetWindowForID(ISD_DLG))
	{
		dialog->OnActionEvent(lId, p, we);
	}
}
...
{
	if( EXITMGR->IsExitWaiting() )	//Á¾·áÁß¿£ ºÎÈ° ¾ÈµÊ
	{
		...
		case CR_PRESENTSPOT:
			{
				if( SIEGEDUNGEONMGR->IsSiegeDungeon(MAP->GetMapNum()) )
  				{
  					WINDOWMGR->MsgBox(MBI_REVIVE_PRESENTSPOT, MBT_YESNO, CHATMGR->GetChatMsg( 1687 ) );//"1~5%ÀÇ °æÇèÄ¡°¡ ÇÏ¶ôÇÕ´Ï´Ù, Á¦ÀÚ¸®¿¡¼­ ºÎÈ° ÇÏ½Ã°Ú½À´Ï±î?") ;
...
{
	//A¢®E¡ËcAA¢®E¡ËcE¢®E¡Ëc¢®¡×u¡Ë¢çE¡Ë¢ç¨Ï¡©Co
	if( we == WE_BTNCLICK )
	{
		switch( lId )
		{
...
void MessageBox_Func( LONG lId, void * p, DWORD we )
{
			...
			if( we == MBI_YES )
			{
				...
				else
				{
					...
					case eEXITCODE_LOOTING:
						...
						break;
					default:
						// 070622 LYW --- GlobalEventFunc : ¸Þ½ÃÁö ¹øÈ£ ¼öÁ¤.
	...
	// 080121 LYW --- GlobalEventFunc : Ä³¸¯ÅÍ ¼±ÅÃ È­¸éÀ¸·Î ÀÌµ¿È®ÀÎ ¸Þ½ÃÁö Ã³¸®.
	...
	case MBI_SELECTLOGINPOINT:
		{
			if(we == MBI_YES)
			{
				...
				{
					//error ¢®E¡Ëc¢®E¡Ë¡ÍI¢®E¡Ëc¢®¡×u¢®E¡Ëc¢®E?AI¡Ë¢ç¢®¡¿¢®E¡Ëc¢®E¡ËcAAI¡Ë¢ç¢®¡¿¢®E¡Ëc¡Ë¢çEc¡Ë¢çE¡Ë¢ç¨Ï¡©| AuAaCO ¡Ë¢ç¢®¡¿uo ¡Ë¢ç¢®¡¿u¢®¡×I¢®¡×¢®I¡Ë¢ç¢®¡¿oA¡Ë¢çE¡Ë¢çII¡Ë¢çE¡Ë¢çIU.
				}
	...
	case MBI_LOGINEDITCHECK:
		{
			...
			if( we == MBI_OK )
			{
				cEditBox * pIdEdit =(cEditBox *) WINDOWMGR->GetWindowForIDEx(MT_IDEDITBOX); // pjs ID¡Ë¢çE?¡Ë¢çE¢®Ec ¡Ë¢ç¢®¡¿¢®E¡Ëc¢®E¡ËcAA¡Ë¢çE?¡Ë¢ç¢®¡¿o¡Ë¢ç¢®¡¿¡Ë¢ç¨Ï¡© [5/20/2003]
				cEditBox * pPwEdit =(cEditBox *) WINDOWMGR->GetWindowForIDEx(MT_PWDEDITBOX); // pjs ID¡Ë¢çE?¡Ë¢çE¢®Ec ¡Ë¢ç¢®¡¿¢®E¡Ëc¢®E¡ËcAA¡Ë¢çE?¡Ë¢ç¢®¡¿o¡Ë¢ç¢®¡¿¡Ë¢ç¨Ï¡© [5/20/2003]
	...
	case MBI_SHOWDOWNAPPLY:
		...
		break;
	//Alemuri Party War-----------------------------------
		case MBI_SHOWDOWNPARTYAPPLY:
		{
			if(we == MBI_YES)
			{
				SHOWDOWNMGR->AcceptShowdownParty( TRUE );
			}
			else if( we == MBI_NO )
			{
				SHOWDOWNMGR->AcceptShowdownParty( FALSE );
			}
		}
		break;
	...
	case MBI_CHARMAKEERROR:
		{
			...
			// ¼ºº°À» ¼±ÅÃÇÒ ¼ö ¾øµµ·Ï ÇÏ±â À§ÇØ Disable·Î ¼¼ÆÃÇØ ÁØ´Ù.
	...
	case MBI_PKMODE:
		{
			if( we == MBI_YES && MAP->GetMapNum() !=PVP)
			{
				PKMGR->SendMsgPKOn();
			}
			else
	...
	case MBI_WAIT_LOGINCHECK:
		{
			//¡Ë¢çE?¡Ë¢ç¢®¡¿I¢®E¡Ëc¢®¡×ua¡Ë¢ç¢®¡¿u¢®E¡Ëc¢®¡×I ¡Ë¢çE¡Ë¢ç¨Ï¡©¡Ë¢ç¢®¡¿¡Ë¢ç¡§I¡Ë¢ç¢®¡¿u¡Ë¢ç¢®¡¿uAo¡Ë¢çE¡Ë¢ç¨Ï¡©| ¡Ë¢ç¢®¡¿¡Ë¢ç¨Ï¡©¡Ë¢çE¡Ë¢ç¨Ï¡©¢®¡×I¡§I¡§¡Ì¢®E¡Ëci¢®E¡Ëc¢®¡×¡Ë¢çi ¢®¡×Ii¡§Io¡Ë¢çEi¢®¡×o¡Ë¢ç¢®¡¿u¡Ë¢ç¢®¡¿¢®E¡Ëc¡Ë¢ç¢®¡¿u¢®E¡Ëc¢®¡×I A¢®¡×I¡§I¡§¡Ì¡Ë¢çE¡Ë¢ç¨Ï¡©¡Ë¢çEc? ¢®E¡Ëc¢®E¡Ë¡Í¢®E¡ËcEAI¡Ë¢çEO¢®E¡Ëc¡Ë¢ç¡Ë?¡Ë¢çE¡Ë¢çIA AI¢®E¡Ëc¢®¡×¡Ë¢çI¡Ë¢çIi¡Ë¢çIi A¢®¡×I¡§I¡§¡Ì¡Ë¢çE¡Ë¢ç¨Ï¡©¡Ë¢çEc ¡Ë¢ç¢®¡¿uE¡Ë¢çIiE¢®E¡Ëc¢®¡×¡Ë¢çI ¡Ë¢ç¢®¡¿u¡Ë¢ç¢®¡¿¢®E¡Ëc¡Ë¢çE¡Ë¢çIN¢®E¡Ëc¢®¡×¡Ë¢ç¡Ë¢çE¢®Ec?
			...
			TITLE->NoDisconMsg();		//¡Ë¢ç¢®¡¿u¡Ë¢ç¢®¡¿¢®E¡Ëc¢®E¡Ëc¢®E¡Ë¡Í¡Ë¢çE¢®Ec showserverlist¡Ë¢çE?¡Ë¢çE¢®Ec¡Ë¢ç¢®¡¿u¢®E¡Ëc¢®¡×I A¢®E¡ËcE¡Ë¢ç¢®¡¿uO ¢®¡×I¡Ë¢ç"¡ËcAA¢®E¡Ëci¡Ë¢çEO¢®E¡Ëc¡Ë¢ç¡Ë? ¢®¡×I¡Ë¢çAo¢®E¡Ëc¢®¡×ue ¡Ë¢çE¡Ë¢ç¨Ï¡©¡Ë¢ç¢®¡¿¡Ë¢ç¡§I¡Ë¢ç¢®¡¿u¡Ë¢ç¢®¡¿uAo ¡Ë¢ç¢®¡¿u¢®¡×I¢®¡×¢®I¡Ë¢çIi¡Ë¢çIi¢®E¡Ëc¢®E¡Ë¡ÍI.
			TITLE->ShowServerList();
	...
	case MBI_GUILD_BAN:
		{
			...
			// 080417 LUJ, ¸Þ½ÃÁö Ã¢ Ç¥½Ã Àü¿¡ Àá°¬À¸¹Ç·Î ¹Ýµå½Ã ´Ù½Ã Ç®¾îÁÖÀÚ
			dialog->SetDisable( FALSE );
	...
	case MBI_GUILD_RETIRE_REQUEST:
		...
		break;
		// ¾ÆÀÌÅÛ Á¶ÇÕÀ» º¹¼ö·Î ÇÒ¶§ ½ÇÇà ¿©ºÎ¿¡ ´ëÇÑ È®ÀÎ Ã¢À» Ç¥½ÃÇÑ´Ù.
	case MBI_ITEM_MIX_CONFIRM:
		{
			// ÁøÇà Ç¥½Ã°¡ ³¡³­ ÈÄ ¼­¹ö¿¡ ÀÛ¾÷À» ¿äÃ»ÇÒ °ÍÀÌ´Ù
			if( we == MBI_YES )
			{
		...
		break;
		// ¾ÆÀÌÅÛ °­È­ ¿É¼ÇÀÌ ÀÖ´Â °æ¿ì ¿É¼ÇÀÌ ³·¾ÆÁú ¼ö ÀÖÀ½¿¡ ´ëÇÑ °æ°í¸¦ Ç¥½ÃÇÑ´Ù
	case MBI_ITEM_REINFORCE_CONFIRM:
		...
		break;
	// desc_hseos_ÆÐ¹Ð¸®01
	// S ÆÐ¹Ð¸® Ãß°¡ added by hseos 2007.10.16
	// ..Å»Åð
	...
	case MBI_SKILLTRAIN:
		...
		break;
	// E ÆÐ¹Ð¸® Ãß°¡ added by hseos 2007.10.16
	...
	case MBI_MAPMOVE_AREYOUSURE :
		{
			...
			if( !pDlg ) return ;
			if (!pDlg->IsActive() && we == MBI_YES)//johan cek masuk portal
	...
	// 071203 LYW --- GlovalEventFunc : ÇöÀç ¸¶³ª °¨¼Ò È¿°ú¸¦ Ãë¼ÒÇÏ°í ´Ù½Ã Àû¿ëÇÒ °ÍÀÎÁö ¹°¾îº»´Ù.
	...
	case MBI_USEBUFFITEM_AREYOUSURE :
		{
			...
			// 080728 LUJ, ÀÎº¥Åä¸® Àá±ÝÀ» ÇØÁ¦ÇÑ´Ù
			pDlg->SetDisable( FALSE );

			if( we == MBI_YES )
			{
				CItem* pItem = NULL ;
				// ÀÎº¥¿¡ ¾ÆÀÌÅÛ µî·Ï ÇÏ°í »ç¿ëÇÏ¸é, ÇöÀç ¼±ÅÃ µÈ ¾ÆÀÌÅÛÀÌ ¾øÀ» ¶§ Ã³¸®¸¦ ¸øÇÏ±â ¶§¹®¿¡
				// GetCurSelectedItemÀº »ç¿ëÇÏÁö ¾Ê´Â´Ù.
	...
	case MBI_REGISTSTREETTOURNAMENT_AREYOUSURE :
		{
			if( we == MBI_YES )
			{
				MSGBASE msg;
	...
	case MBI_GETNPCBUFF_AREYOUSURE :
		{
			if( we == MBI_YES )
			{
				MSGBASE msg;
	...
	// 071204 LYW --- GlovalEventFunc : ½ºÅÈ ÃÊ±âÈ­ È®ÀÎ ¸Þ½ÃÁö Ã³¸®.
	...
	case MBI_RESETLEVEL_AREYOUSURE :
		{
			CInventoryExDialog* pDlg = NULL ;
			pDlg = GAMEIN->GetInventoryDialog() ;
			...
			if( we == MBI_YES )
			{
				ITEMMGR->UseItem_ResetLevel(pItem) ;
		...
		break ;
	// 071204 LYW --- GlovalEventFunc : ½ºÅ³ ÃÊ±âÈ­ È®ÀÎ ¸Þ½ÃÁö Ã³¸®.
	...
	// 071205 LYW --- GlovalEventFunc : °æÇèÄ¡ Áõ°¡ È®ÀÎ ¸Þ½ÃÁö Ã³¸®.
	...
	// 071205 LYW --- GlovalEventFunc : µå¶ø·ü Áõ°¡ È®ÀÎ ¸Þ½ÃÁö Ã³¸®.
	case MBI_INCDROPRATE_AREYOUSURE :
		...
		break ;
	// 071210 LYW --- GlobalEventFunc : ÀÎº¥Åä¸® È®Àå »ç¿ë ¸Þ½ÃÁö Ã³¸®.
	case MBI_EXTENDINVEN_AREYOUSURE :
		...
		break ;
	// S ³óÀå½Ã½ºÅÛ Ãß°¡ added by hseos 2008.01.17
	case MBI_FARM_TAX_PAY_REQUEST:
		{
			if( we == MBI_YES )
			{
				// 091204 ONS ÆÐ¹Ð¸® ´ÙÀÌ¾ó·Î±×¿¡ ³óÀå¼¼±Ý ³³ºÎ ±â´É ¹öÆ° Ãß°¡
				// ³óÀå¸Ê¿¡¼­¸¸ ³óÀåÁ¤º¸°¡ °ü¸®µÇ°í ÀÖ±â ¶§¹®¿¡ ³óÀå¸Ê¿¡¼­ÀÇ ³³ºÎ¿Í 
				// Å¸¸Ê¿¡¼­ÀÇ ³³ºÎ¸¦ ±¸ºÐÇÏ¿© Ã³¸®ÇÑ´Ù.
		...
		break;
	// E ³óÀå½Ã½ºÅÛ Ãß°¡ added by hseos 2008.01.17
		// 080414 LUJ, ¿Ü¾ç º¯°æ ¾ÆÀÌÅÛ »ç¿ë
	case MBI_USEBODYCHANGEITEM:
		{
			cDialog* dialog = WINDOWMGR->GetWindowForID( BODY_CHANGE_DIALOG );

			// 080930 LUJ, ¸Þ½ÃÁö ¹Ú½º Ã³¸®µµ ÇØ´ç Ã¢¿¡¼­ ÇÏµµ·Ï ÇÑ´Ù
			if( ! dialog )
			{	
				break;
			}
	...
	case MBI_CR_SECRETCODE_ERR :
 		{
 			...
 			// È¤¿© Ã¤ÆÃÃ¢¿¡ Æ÷Ä¿½º°¡ ÀÖÀ¸¸é ¤Ñ¤Ñ; ÇØÀç ÇÏÀÚ.
 		}
 		break ;

	case MBI_CR_ENDCHAT :														// Ã¤ÆÃ¹æ Á¾·á ¹öÆ° Ã³¸®.
		{
			if( we == MBI_YES )
			{
				CChatRoomDlg* pDlg = NULL ;										// Ã¤ÆÃ¹æ Á¤º¸¸¦ ¹Þ´Â´Ù.
				...
				MSG_DWORD msg ;													// Á¾·á ¿äÃ» ¸Þ½ÃÁö¸¦ º¸³½´Ù.
	...
	case MBI_GTENTER_SYN:
		...
		// 080821 LYW --- GlobalEventFunc : ¹°¾îº¸´Â ¸Ê ÀÌµ¿ Æ÷Å» Ã³¸® Ãß°¡.
  	case MBI_RECALLPORTAL_CHANGEMAP :
  		{
  			if( we == MBI_YES ) 
  			{
  				...
  				if( !pObject )
  				{
  					CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 1689 ) );//"´ë»óÀ» ¼±ÅÃÇØ ÁÖ¼¼¿ä." ) ;
  				...
  				{
  					CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 1690 ) ); //"¿Ã¹Ù¸¥ ´ë»óÀÌ ¾Æ´Õ´Ï´Ù." ) ; 
  					return ;
  				}
  		...
  		// 080922 LYW --- GlobalEventFunc : ±æµå ¾ÆÁöÆ®·Î ÀÌµ¿ÇÏ°Ú³Ä´Â Ã³¸® Ãß°¡.
	...
	case MBI_RECALL_ASK:
		...
		break;
		// 090422 ShinJS --- Å»°Í Å¾½Â ¿äÃ»½Ã ¼ö¶ô/°ÅºÎ 
	case MBI_VEHICLE_ALLOW_GETON:
		{
			// ÀÚ½ÅÀÇ Å»°Í Á¤º¸¸¦ ¾ò¾î¿Â´Ù
	...
	// 090422 ShinJS --- Å»°Í Å¾½Â½Ã ¿ä±ÝÀÌ ÇÊ¿äÇÑ °æ¿ì Å¾½Â ½Ãµµ ÆÇ´Ü
	case MBI_VEHICLE_GETON_ASK_PAY_TOLL:
		...
		break;
	// 090415 ÇÏ¿ìÂ¡ ÀÌ¸§Áþ±â ½ÇÇà ¿©ºÎ¸¦ Ã³¸®ÇÑ´Ù 
	case MBI_HOUSENAME_AREYOUSURE:
		{
			...
			//090420 ÇÏ¿ì½º»ý¼º ÆÐÅ¶À» º¸³×ÀÚ MP_HOUSE_CREATE_SYN
		...
		break;
	// 090415 ÇÏ¿ìÂ¡ ÁýÃ£±â ½ÇÇà ¿©ºÎ¸¦ Ã³¸®ÇÑ´Ù . 
	...
	case MBI_VISIT_RANDOM_AREYOUSURE:
		...
		break;
		// 090415 ÇÏ¿ìÂ¡ ²Ù¹Ì±â º¸³Ê½º »ç¿ë ¿©ºÎ¸¦ Ã³¸®ÇÑ´Ù . 
	case MBI_USEDECOBONUS_AREYOUSURE:
		{
			//Ã³¸®ÇÏÀÚ Ã³¸®ÇÏÀÚ Ã³¸®ÇÏÀÚ 
	...
	case MBI_HOUSE_VISIT_MYHOME_AREYOUSURE:
		...
		break;
	case MBI_VISIT_LINK_PLAYER_RCLICK:						//090615 pdy ÇÏ¿ìÂ¡ Ä³¸¯ÅÍ ¿ìÅ¬¸¯ ¸µÅ© ÇÏ¿ì½ºÀÔÀå ±â´É Ãß°¡
		{
			if( MBI_YES == we )
			{
		...
		break;
	case MBI_HOUSE_VISIT_LINK_MYHOME_AREYOUSURE:			//090604 pdy static Npc¸¦ ÅëÇØ ³»Áý°¡±â Ãß°¡ 
		{
			if( MBI_YES == we )
			{
				...
				if( pNpc )
				{
					...
					msg.Protocol = MP_HOUSE_ENTRANCE_SYN;
					msg.dwObjectID = HEROID;
					msg.cKind	=  eHouseVisitByLink ;			//Npc¸¦ ÅëÇØ µé¾î°¡´Â°ÍÀº eHouseVisitByLink
					msg.dwValue1 = pNpc->GetID();				
					msg.dwValue2 = TITLE->GetUserIdx();			//Npc¸¦ ÅëÇØ ³»ÁýÀ» µé¾î°¥¶§´Â À¯ÀúÀÎµ¦½º¸¦ º¸³×¾ßÇÑ´Ù.
					NETWORK->Send(&msg,sizeof(msg));
				}
				else
				{
	...
	case MBI_HOUSE_VISIT_LINK_AREYOUSURE:
		{
			if( MBI_YES == we )
			{
				...
				{
					CHATMGR->AddMsg(CTC_SYSMSG,  CHATMGR->GetChatMsg(1839) ); //1839	"Å¾½Â µµ±¸¸¦ ÀÌ¿ë ½Ã¿¡´Â ÇØ´ç ±â´ÉÀ» »ç¿ëÇÏ½Ç ¼ö ¾ø½À´Ï´Ù."
					OBJECTSTATEMGR->EndObjectState(HERO, eObjectState_Deal) ;
					return;
				}
	...
	case MBI_HOUSE_DESTROY_FUTNITURE_AREYOUSURE:
		...
		break;
	// 091125 ONS Ä³¸¯ÅÍ¸í º¯°æ½Ã ÆË¾÷ ¸Þ¼¼Áö Ãâ·Â
	case MBI_CHANGENAME_ACK:
		...
		break;
	// 091126 ONS ÆÐ¹Ð¸® ´ÙÀÌ¾ó·Î±×¿¡ ÆÐ¹Ð¸®Àå ÀÌ¾ç ±â´É Ãß°¡
	case MBI_FAMILY_TRANSFER:
		...
		break;
	// 100105 ShinJS --- PC¹æ ¾ÆÀÌÅÛ Áö±Þ ¿äÃ» È®ÀÎ
	...
	case MBI_RESURRECT_ASK:
		{
			...
			else
			{
				...
				// ºÎÈ°À» °ÅºÎÇÏ¸é ±âÁ¸ÀÇ ºÎÈ°Áö¿ª ¼±ÅÃ ´ÙÀÌ¾ó·Î±×¸¦ È°¼ºÈ­ÇÑ´Ù.
	...
	case MBI_CONSIGNMENT_BUY_CONFIRM:
		{
			if(MBI_YES == we)
			{
				...
				if(pDlg && pDlg->GetMode()==CConsignmentDlg::eConsignment_Mode_Buy)
				{
					...
					DWORDEX dwBuyTotalPrice = pDlg->GetBuyPrice(dwConsignmentIndex) * wBuyDurbility;
	...
	case MBI_INSPECT_ACCEPT:
		{
			if(we == MBI_YES)
			{
				MSG_DWORD msg;
				msg.Category = MP_ITEM;
				msg.Protocol = MP_ITEM_QUERY_EQUI_ACCEPT_ACK;
				msg.dwObjectID = HERO->GetID();
				msg.dwData = ITEMMGR->m_InspectRequesterId;
				NETWORK->Send(&msg, sizeof(msg));
				ITEMMGR->m_InspectRequesterId = 0;
			}
			else if( we == MBI_NO )
			{
				...
				MSG_DWORD msg;
				msg.Category = MP_ITEM;
				msg.Protocol = MP_ITEM_QUERY_EQUI_ACCEPT_NACK;
				msg.dwObjectID = HERO->GetID();
				msg.dwData = ITEMMGR->m_InspectRequesterId;
				NETWORK->Send(&msg, sizeof(msg));
				ITEMMGR->m_InspectRequesterId = 0;
	...
	case MBI_INSPECT_WAITING:
		{
			MSG_DWORD msg;
			msg.Category = MP_ITEM;
			msg.Protocol = MP_ITEM_QUERY_EQUI_REQUEST_NACK;
			msg.dwObjectID = HERO->GetID();
...
void MNM_DlgFunc(LONG lId, void * p, DWORD we)
{
	...
	case MNM_PATHFIND_BTN:
		...
		// 091209 ShinJS --- ¾ÆÀÌÅÛ¸ô ¹öÆ°
	case MNM_ITEMMALL_BTN:
		{
			if(GAMEIN->GetItemShopDialog()->IsActive())
			{
				GAMEIN->GetItemShopDialog()->SetActive( FALSE );
				// 100223 ONS ÀÎº¥Åä¸®µµ ÇÔ²² ´Ý´Â´Ù.
				GAMEIN->GetInventoryDialog()->SetActive( FALSE );
				return;
			}

			// 091105 pdy Á¶ÇÕ/ÀÎÃ¦Æ®/°­È­/ºÐÇØ ÁßÀÎ°æ¿ì ¾ÆÀÌÅÛ¸ô ¿­±â ºÒ°¡´É 
			BOOL isOpen = FALSE;
			{
		...
		//Alemuri Map Move Scroll MiniMap--------------------------------------------
	case MNM_MAPMOVE_BTN:
		{
			cMapMoveDialog* pDlg = NULL ;
		...
		break;
		//---------------------------------------------------------------------------
		//Alemuri Warehouse Scroll MiniMap--------------------------------------------
	case MNM_WAREHOUSE_BTN:
		{
				if( GAMEIN->GetDealDialog()->IsActive() )
				{
					...
					CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 1077 ) );		// »óÁ¡À» ¸ÕÀú ´ÝÀ¸¼¼¿ä
					break;
				}
				...
				//{
				//	CHATMGR->AddMsg( CTC_SYSMSG, "Minimum required VIP Level 1, for use this shop"  );		// »óÁ¡À» ¸ÕÀú ´ÝÀ¸¼¼¿ä
				//	break;
				//}

				if(GAMEIN->GetStorageDialog()->IsItemInit())									// Ã¢°í ¾ÆÀÌÅÛÀÇ ¾ÆÀÌÅÛ ÃÊ±âÈ­°¡ µÇ¾úÀ¸¸é,
				{
					if( !GAMEIN->GetInventoryDialog()->IsActive() )								// ÀÎº¥Åä¸® ´ÙÀÌ¾ó·Î±×°¡ È°¼ºÈ­ ÁßÀÌ¸é,
					{
						GAMEIN->GetStorageDialog()->ShowStorageDlg(TRUE) ;							// Ã¢°í ´ÙÀÌ¾ó·Î±×¸¦ È°¼ºÈ­ ÇÑ´Ù.
					}
					else
					{
						GAMEIN->GetStorageDialog()->SetActive(TRUE) ;							// Ã¢°í ´ÙÀÌ¾ó·Î±×¸¦ È°¼ºÈ­ ÇÑ´Ù.
					}

					if(OBJECTMGR->GetHero()->GetStorageNum() == 0)												// °³¼³ÇÑ Ã¢°í°¡ ¾ø´Ù¸é,
					{

						GAMEIN->GetStorageDialog()->ShowStorageMode(eStorageMode_NoWare);		// Ã¢°í°¡ ¾ø´Ù´Â ¸ðµå·Î ¼¼ÆÃÇÑ´Ù.
					}
					else																		// °³¼³ÇÑ Ã¢°í°¡ ÀÖ´Ù¸é,
					{
						GAMEIN->GetStorageDialog()->ShowStorageMode(eStorageMode_StorageWare1);	// Ã¢°í 1À» È°¼ºÈ­ ÇÑ ¸ðµå·Î ÇÑ´Ù.
						GAMEIN->GetStorageDialog()->AddStorageMode(eStorageMode_StorageWare1);	// Ã¢°í ¸ðµå¸¦ Ãß°¡ÇÑ´Ù.
					}
				}
				else																			// Ã¢°íÀÇ ¾ÆÀÌÅÛÀÇ ¾ÆÀÌÅÛ ÃÊ±âÈ­°¡ µÇÁö ¾Ê¾Ò´Ù¸é,
				{
					if(HERO->GetStorageNum() == 0)												// °³¼³ÇÑ Ã¢°í°¡ ¾øÀ¸¸é,
					{
						return;
					}
					else																		// °³¼³ÇÑ Ã¢°í°¡ ÀÖ´Ù¸é,
					{					
						MSG_WORD2 msg;														// ¸Þ½ÃÁö ±¸Á¶Ã¼¸¦ ¼±¾ðÇÑ´Ù.
						msg.Category = MP_ITEM;												// Ä«Å×°í¸®¸¦ ¾ÆÀÌÅÛÀ¸·Î ¼¼ÆÃÇÑ´Ù.
						msg.Protocol = MP_ITEM_STORAGE_ITEM_INFO_SYN;						// ÇÁ·ÎÅäÄÝÀ» ¾ÆÀÌÅÛ Ã¢°í, ¾ÆÀÌÅÛ Á¤º¸ ½ÌÅ©·Î ¼¼ÆÃÇÑ´Ù.
						msg.dwObjectID = HEROID;											// HERO ¾ÆÀÌµð¸¦ ¼¼ÆÃÇÑ´Ù.
						msg.wData1=79 ;													// µ¥ÀÌÅÍ¸¦ 79·Î ¼¼ÆÃÇÑ´Ù.
						msg.wData2 = eNpcParam_ShowpStorage;								// µ¥ÀÌÅÍ 2¸¦ NPC µ¥ÀÌÅÍ·Î ¼¼ÆÃÇÑ´Ù.
	...
	case MNM_NOTE_BTN:
		{
			...
			//if( cekvip->GetVIPstatus() < 1)
			//{
			//	CHATMGR->AddMsg( CTC_SYSMSG, "Minimum required VIP Level 1, for use this menu"  );		// »óÁ¡À» ¸ÕÀú ´ÝÀ¸¼¼¿ä
			//	break;
			//}
		...
		break;
		//Alemuri Friends in Minimap--------------------------------------------------
	case MNM_FRIENDS_BTN:
		{
	...
	case MNM_DD_BTN:
		{
			...
			else																			
			{
				...
				if( pDlg )																	
				{
					if( pDlg->IsActive() )													
					{
						CDateMatchingChatDlg* pChatDlg = NULL ;								

						pChatDlg = GAMEIN->GetDateMatchingDlg()->GetChatingDlg() ;			
	...
	case MNM_CHATROOM_BTN:
		...
		// 091209 ShinJS --- ³ìÈ­ ½ÃÀÛ(³ìÈ­ Dialog ¶ç¿ì±â) ¹öÆ°
	...
	case MNM_VIDEO_REC_STOP_BTN:
		...
		// 091209 ShinJS --- PC¹æ »óÁ¤
	case MNM_PCROOM_MALL_BTN:
{
    CHero* hero = OBJECTMGR->GetHero();
    ASSERT(hero);

    //Zera Obtener el número del mapa actual
    int currentMap = hero->GetCharacterTotalInfo()->CurMapNum;

    //Zera Restringir acceso a la tienda en el mapa 55
    if (currentMap == 55)
    {
        CHATMGR->AddMsg(CTC_SYSMSG, "You cannot open this shop in this map.");
        return;
    }

    //Zera Bloquear la apertura de la tienda si el intercambio está activo
    if (GAMEIN->GetExchangeDialog()->IsActive())
    {
        CHATMGR->AddMsg(CTC_SYSMSG, "You cannot open the shop while trading.");
        return;
    }
	...
	case MNM_PCROOM1_MALL_BTN:
		{
			...
			ZeroMemory( &msg, sizeof(msg) );
			msg.Category = MP_PCROOM;
			msg.Protocol = MP_PCROOM1_OPEN_MALL_SYN;
			msg.dwObjectID = HEROID;
			NETWORK->Send( &msg, sizeof(msg) );
		}
		break;
	case MNM_PCROOM2_MALL_BTN:
		{
			if( GAMEIN->GetDealDialog()->IsActive() )
			{
				if( GAMEIN->GetDealDialog()->IsActivePointInfo() )
			...
			ZeroMemory( &msg, sizeof(msg) );
			msg.Category = MP_PCROOM;
			msg.Protocol = MP_PCROOM2_OPEN_MALL_SYN;
			msg.dwObjectID = HEROID;
			NETWORK->Send( &msg, sizeof(msg) );
		}
		break;
	case MNM_PCROOM3_MALL_BTN:
		{
			if( GAMEIN->GetDealDialog()->IsActive() )
			{
				if( GAMEIN->GetDealDialog()->IsActivePointInfo() )
			...
			ZeroMemory( &msg, sizeof(msg) );
			msg.Category = MP_PCROOM;
			msg.Protocol = MP_PCROOM3_OPEN_MALL_SYN;
			msg.dwObjectID = HEROID;
			NETWORK->Send( &msg, sizeof(msg) );
		}
		break;
	case MNM_PCROOM4_MALL_BTN:
		{
			if( GAMEIN->GetDealDialog()->IsActive() )
			{
				if( GAMEIN->GetDealDialog()->IsActivePointInfo() )
			...
			ZeroMemory( &msg, sizeof(msg) );
			msg.Category = MP_PCROOM;
			msg.Protocol = MP_PCROOM4_OPEN_MALL_SYN;
			msg.dwObjectID = HEROID;
			NETWORK->Send( &msg, sizeof(msg) );
		}
		break;
	case MNM_PCROOM5_MALL_BTN:
		{
			if( GAMEIN->GetDealDialog()->IsActive() )
			{
				if( GAMEIN->GetDealDialog()->IsActivePointInfo() )
			...
			ZeroMemory( &msg, sizeof(msg) );
			msg.Category = MP_PCROOM;
			msg.Protocol = MP_PCROOM5_OPEN_MALL_SYN;
			msg.dwObjectID = HEROID;
			NETWORK->Send( &msg, sizeof(msg) );
		}
		break;
	case MNM_PCROOM6_MALL_BTN:
		{
			if( GAMEIN->GetDealDialog()->IsActive() )
			{
				if( GAMEIN->GetDealDialog()->IsActivePointInfo() )
			...
			ZeroMemory( &msg, sizeof(msg) );
			msg.Category = MP_PCROOM;
			msg.Protocol = MP_PCROOM6_OPEN_MALL_SYN;
			msg.dwObjectID = HEROID;
			NETWORK->Send( &msg, sizeof(msg) );
		}
		break;
	case MNM_PCROOM7_MALL_BTN:
		{
			if( GAMEIN->GetDealDialog()->IsActive() )
			{
				if( GAMEIN->GetDealDialog()->IsActivePointInfo() )
			...
			ZeroMemory( &msg, sizeof(msg) );
			msg.Category = MP_PCROOM;
			msg.Protocol = MP_PCROOM7_OPEN_MALL_SYN;
			msg.dwObjectID = HEROID;
			NETWORK->Send( &msg, sizeof(msg) );
		}
		break;
	case MNM_PCROOM8_MALL_BTN:
		{
			if( GAMEIN->GetDealDialog()->IsActive() )
			{
				if( GAMEIN->GetDealDialog()->IsActivePointInfo() )
			...
			ZeroMemory( &msg, sizeof(msg) );
			msg.Category = MP_PCROOM;
			msg.Protocol = MP_PCROOM8_OPEN_MALL_SYN;
			msg.dwObjectID = HEROID;
			NETWORK->Send( &msg, sizeof(msg) );
		}
		break;
	case MNM_PCROOM9_MALL_BTN:
		{
			if( GAMEIN->GetDealDialog()->IsActive() )
			{
				if( GAMEIN->GetDealDialog()->IsActivePointInfo() )
			...
			ZeroMemory( &msg, sizeof(msg) );
			msg.Category = MP_PCROOM;
			msg.Protocol = MP_PCROOM9_OPEN_MALL_SYN;
			msg.dwObjectID = HEROID;
			NETWORK->Send( &msg, sizeof(msg) );
		}
		break;
	case MNM_PCROOM10_MALL_BTN:
		{
			if( GAMEIN->GetDealDialog()->IsActive() )
			{
				if( GAMEIN->GetDealDialog()->IsActivePointInfo() )
			...
			ZeroMemory( &msg, sizeof(msg) );
			msg.Category = MP_PCROOM;
			msg.Protocol = MP_PCROOM10_OPEN_MALL_SYN;
			msg.dwObjectID = HEROID;
			NETWORK->Send( &msg, sizeof(msg) );
		}
		break;
	case MNM_PCROOM11_MALL_BTN:
		{
			if( GAMEIN->GetDealDialog()->IsActive() )
			{
				if( GAMEIN->GetDealDialog()->IsActivePointInfo() )
			...
			ZeroMemory( &msg, sizeof(msg) );
			msg.Category = MP_PCROOM;
			msg.Protocol = MP_PCROOM11_OPEN_MALL_SYN;
			msg.dwObjectID = HEROID;
			NETWORK->Send( &msg, sizeof(msg) );
		}
		break;
	case MNM_PCROOM_SLOT_MALL_BTN:
		{
			if( GAMEIN->GetDealDialog()->IsActive() )
			{
				if( GAMEIN->GetDealDialog()->IsActivePointInfo() )
			...
			const BOOL isOpen = mixDialog->IsActive() || enchantDialog->IsActive() || reinforceDialog->IsActive() ||dissoloveDialog->IsActive();
			...
			msg.Protocol = MP_NOTE_SLOT_OPEN_SYN;
			msg.dwObjectID = HEROID;
			NETWORK->Send( &msg, sizeof(msg) );
		}
		break;
	
		// 100105 ShinJS --- PC¹æ ¾ÆÀÌÅÛ Áö±Þ ¿äÃ»
	case MNM_PCROOM_PROVIDE_ITEM_BTN:
		{
			// ¾ÆÀÌÅÛ Áö±Þ ¿©ºÎ¸¦ ¹¯´Â MsgBox ½ÇÇà
...
void QUE_QuestDlgFunc( LONG lId, void* p, DWORD we )
{
	...
	case QUE_NOTICEBTN :
		{
			//QUE_RegistQuestNotice(TRUE);	// 080305 NYJ --- Äù½ºÆ®ÀÚµ¿¾Ë¸®¹Ì »ç¿ëÀ» À§ÇØ ¾Æ·¡ÄÚµå¸¦ ÇÔ¼ö·Î Ã³¸®ÇÔ. (ÇöÀç »ç¿ëµÇÁö ¾ÊÀ½)
			/**/
			DWORD QuestIdx = 0 ;																			// Äù½ºÆ® ÀÎµ¦½º¸¦ ´ãÀ» º¯¼ö¸¦ ¼±¾ðÇÏ°í 0À¸·Î ¼¼ÆÃÇÑ´Ù.

			CQuestDialog* pDlg = NULL ;																		// Äù½ºÆ® ´ÙÀÌ¾ó·Î±× Á¤º¸¸¦ ¹ÞÀ» Æ÷ÀÎÅÍ¸¦ ¼±¾ðÇÏ°í nullÃ³¸®¸¦ ÇÑ´Ù.
			pDlg = GAMEIN->GetQuestTotalDialog()->GetQuestDlg() ;											// Äù½ºÆ® ´ÙÀÌ¾ó·Î±× Á¤º¸¸¦ ¹Þ´Â´Ù.

			if( pDlg )																						// Äù½ºÆ® ´ÙÀÌ¾ó·Î±× Á¤º¸°¡ À¯È¿ÇÏ´Ù¸é,
			{
				QuestIdx = pDlg->GetSelectedQuestID() ;														// ¼±ÅÃ µÈ Äù½ºÆ® ÀÎµ¦½º¸¦ ¹Þ´Â´Ù.
			}

			if( QuestIdx < 1 )																				// Äù½ºÆ® ÀÎµ¦½º°¡ 1º¸´Ù ÀÛÀ¸¸é,
			{
			...
			else
			{
				...
				if( pQuickDlg )
				{
					...
					// 100601 ONS Äù½ºÆ® ¾Ë¸®¹Ì µî·ÏÃ¢À» ¾÷µ¥ÀÌÆ®ÇÑ´Ù.
	...
	case QUE_GIVEUPBTN :
		...
		break ;
		// 100414 ONS ÀÏ¹Ýº¸»ó/¼±ÅÃº¸»ó ¹öÆ°Ã³¸®Ãß°¡
	...
	case FD_CCREATEOKBTN:
		{
			...
			{
				// 100309 ONS ÆÐ¹Ð¸® Áßº¹½ÅÃ»½Ã ¸Þ¼¼Áö ¼öÁ¤
...
// S ³óÀå½Ã½ºÅÛ Ãß°¡ added by hseos 2007.08.24	2007.09.10	2007.10.23
...
// E ³óÀå½Ã½ºÅÛ Ãß°¡ added by hseos 2007.08.24	2007.09.10	2007.10.23

// desc_hseos_µ¥ÀÌÆ® Á¸_01
// S µ¥ÀÌÆ® Á¸ Ãß°¡ added by hseos 2007.11.14  2007.11.15	2008.01.24
...
// E µ¥ÀÌÆ® Á¸ Ãß°¡ added by hseos 2007.11.14  2007.11.15	2008.01.24
	...
	case GFW_GDU_REMOVE_OKBTN:	// guildunion remove
		{
			...
			// 080128 LYW --- GlobalEventFunc : [MPÈ¸º¹] Ãâ·Â »èÁ¦.
			/*else
			{
...
void GDT_DlgFunc(LONG lId, void * p, DWORD we)
{
	...
	case GTENTRYCHANGE:
	// 090824 ONS ¿£Æ®¸® µî·Ï´ÙÀÌ¾ó·Î±×¿¡ X¹öÆ° ±â´É Ãß°¡.
...
{
	GAMEIN->GetSkillTrainingDlgJct()->OnActionEvent(lId, p, we);
}

// desc_hseos_ÆÐ¹Ð¸®01
// S ÆÐ¹Ð¸® Ãß°¡ added by hseos 2007.07.02
void FD_GuildDlg(LONG lId, void* p, DWORD we)
{
	GAMEIN->GetFamilyDlg()->OnActionEvent(lId, p, we);
}
// E ÆÐ¹Ð¸® Ãß°¡ added by hseos 2007.07.02
...
void HD_DlgFunc(LONG lId, void* p, DWORD we )
{
	...
	if( we & WE_CHECKED )													// Ã¼Å© ¹Ú½º Ã¼Å© ÀÌº¥Æ®°¡ ¹ß»ýÇß´Ù¸é,
	{
		if( lId != HD_SHOWTUTORIAL ) return ;								// Ã¼Å© ¹Ú½º ¾ÆÀÌµð¿Í ÀÏÄ¡ÇÏÁö ¾ÊÀ¸¸é, return Ã³¸®¸¦ ÇÑ´Ù.

		TUTORIALMGR->Set_ActiveTutorial(TRUE) ;								// Æ©Åä¸®¾óÀ» È°¼ºÈ­ »óÅÂ·Î ¼¼ÆÃÇÑ´Ù.
		TUTORIALMGR->Get_TutorialBtnDlg()->SetActive(TRUE) ;				// Æ©Åä¸®¾ó ¹öÆ°À» È°¼ºÈ­ ÇÑ´Ù.

		OPTIONMGR->Set_ShowTutorial(TRUE) ;									// Æ©Åä¸®¾ó º¸ÀÌ±â¸¦ true·Î ¼¼ÆÃÇÑ´Ù.
		OPTIONMGR->SaveGameOption();
	}
	else if( we & WE_NOTCHECKED )											// Ã¼Å© ¹Ú½º Ã¼Å© Ç®±â ÀÌº¥Æ®°¡ ¹ß»ýÇß´Ù¸é,
	{
		if( lId != HD_SHOWTUTORIAL ) return ;								// Ã¼Å© ¹Ú½º ¾ÆÀÌµð¿Í ÀÏÄ¡ÇÏÁö ¾ÊÀ¸¸é, return Ã³¸®¸¦ ÇÑ´Ù.

		TUTORIALMGR->Set_ActiveTutorial(FALSE) ;							// Æ©Åä¸®¾óÀ» ºñÈ°¼ºÈ­ »óÅÂ·Î ¼¼ÆÃÇÑ´Ù.
		TUTORIALMGR->Get_TutorialBtnDlg()->SetActive(FALSE) ;				// Æ©Åä¸®¾ó ¹öÆ°À» ºñÈ°¼ºÈ­ ÇÑ´Ù.

		OPTIONMGR->Set_ShowTutorial(FALSE) ;								// Æ©Åä¸®¾ó º¸ÀÌ±â¸¦ false·Î ¼¼ÆÃÇÑ´Ù.
		OPTIONMGR->SaveGameOption();
	}
	...
	case VIDEOCAPTURE_START_BTN:
		{
			...
			// 100111 ONS ÇØ´ç¸Ê¿¡ ¹Ì´Ï¸ÊÀÌ ¾øÀ»°æ¿ì È­¸é ÁÂ»ó´Ü¿¡ [³ìÈ­Áß]ÀÌ¹ÌÁö¸¦ Ãâ·ÂÇÑ´Ù.
...
{
	if(cDialog* const dialog = WINDOWMGR->GetWindowForID(SLOT_SLOTDLG))
	{
		dialog->OnActionEvent(lId, p, we);
	}
}
...
{
	if(cDialog* const dialog = WINDOWMGR->GetWindowForID(REPORT_REPORTDLG))
	{
		dialog->OnActionEvent(lId, p, we);
	}
}
...
{
	if(cDialog* const dialog = WINDOWMGR->GetWindowForID(CHAPTCHA_CHAPTCHADLG))
	{
		dialog->OnActionEvent(lId, p, we);
	}
}
...
{
	if (cDialog* const dialog = WINDOWMGR->GetWindowForID(IN_INSPECTDLG))
	{
		switch (lId)
		{
			//×°±¸
			case CG_VIEWTARGETEQ:
			{
				...
				else
				{
					...
					pDlg->SetActive(FALSE);
					//#ifndef _GMTOOL_
					//WINDOWMGR->MsgBox( MBI_INSPECT_WAITING, MBT_CANCEL, CHATMGR->GetChatMsg(5103) ); //Window while waiting with Cancel Option
					//CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 5109 ) );
					//#endif
				}
			}
			break;
