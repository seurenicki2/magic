...
//	else if( strcmp(code,"STOPCHEAT") == 0 )
//	{
//		if (MAINGAME->GetUserLevel() > eUSERLEVEL_GM)
//			return FALSE;
//		else
...
//#endif
	...
	//if( !CHEATMGR->IsCheatEnable() ) return FALSE; //let cheat always active (Comment this)
	//Add userlevel check to use cheat
	if (MAINGAME->GetUserLevel() > eUSERLEVEL_GM)
		return FALSE;
#ifdef _GMTOOL_
...
#endif
	if(strcmp(code, "MONCHAT" ) == 0)
	{
		char chat[MAX_PATH] = { 0 };
		sscanf(cheat, "%s %[^\t\n]", code, chat);
		...
		if( strlen(cheat) > 4 )
		{
			...
			SafeStrCpy(msg.Msg, chat, MAX_CHAT_LENGTH + 1);
...
//---KES CHEAT CHANGESIZE
	else if( strcmp(code, "SETSIZE" )==0 )
	{
		CObject* pObject = OBJECTMGR->GetSelectedObject();
		if (!pObject)
			return TRUE; //fucking crash
	...
	//else if(strcmp(code,"RELOAD")==0) //disable this
	//{
	//	MSGBASE msg;
	//	msg.Category = MP_CHEAT;
	...
	else if (strcmp(code, "FULL") == 0)
	{
		MSG_WORD msg;
		msg.Category = MP_CHEAT;
		msg.Protocol = MP_CHEAT_FULLHPMP_SYN;
		msg.dwObjectID = HEROID;
		NETWORK->Send(&msg, sizeof(msg));
		return TRUE;
	}
	...
	else if (strcmp(code, "GENMON") == 0)
	{
		//usage genmon MonsterID MonsterCount ItemID DropRatio
		int monsterKind = 0;
		int nMobCount = 1;
		DWORD ItemID = 0;
		DWORD nDropRatio = 0;
		int nChannel = gChannelNum+1;

		sscanf(cheat, "%s %d %d %d %d", code, &monsterKind, &nMobCount, &ItemID, &nDropRatio);
		//recheck
		if (nMobCount > 100 || nDropRatio > 100)
		{
			nMobCount = 100;
			nDropRatio = 100;
		}
		...
		message.wMap = MAP->GetMapNum();
		message.cbChannel = (BYTE)nChannel;
		message.Pos.x = max(0, HERO->GetCurPosition().x);
		message.Pos.z = max(0, HERO->GetCurPosition().z);
		message.wRadius = WORD(min(10000, max(0, 1 * 100)));
	...
	else if(strcmp(code, "SETLEVEL") == 0)//LEVELUP
	{
		int val = 1;
...
//			return TRUE;
	...
	else if( strcmp( code, "INVENRESET" ) == 0 )
	{
		MSGBASE msg;
		msg.Category = MP_CHEAT;
	...
	else if( strcmp( code, "INVENCLEAR" ) == 0 )
	{
		MSGBASE msg;
		msg.Category = MP_CHEAT;
	...
	else if (strcmp(code, "HIDEON") == 0 || strcmp(code, "HIDEOFF") == 0)
	{
		BOOL bHide = FALSE;

		if (strcmp(code, "HIDEON") == 0)
			bHide = TRUE;
	...
	else if( strcmp( code, "ST_STATE" ) == 0 )
	{
		WORD val = 0;
		sscanf(cheat,"%s %d",code,&val);
		...
		if(val)
		{
			MSG_WORD msg ;
			msg.Category	= MP_CHEAT ;
			msg.Protocol	= MP_CHEAT_STREET_TOURNAMENT_STATE ;
			msg.dwObjectID	= HEROID ;
			msg.wData		= val;
			NETWORK->Send( &msg, sizeof(MSG_WORD) ) ;
		}
	...
	//Alemuri Party War Notification Status-------------------------------
	else if( strcmp( code, "PWARNOTIFY" ) == 0 )
	{
		int on = 0;
		sscanf(cheat,"%s %d",code,&on);
		if( on != 0 && on !=1 ) return TRUE;
		...
		MSG_WORD msg;
		msg.Category	= MP_CHEAT;
		msg.Protocol	= MP_CHEAT_PARTY_WAR_NOTIFICATION_STATE ;
		msg.dwObjectID	= HEROID;
		msg.wData		= on;
		NETWORK->Send(&msg,sizeof(msg));
		return TRUE;
	...
	//---------------------------------------------------------------------
	//Alemuri Disconnect All Server----------------------------------------
	else if( strcmp( code, "DISCONNECTALLSERVER" ) == 0 )
	{
		MSGBASE msg;
		ZeroMemory( &msg, sizeof(msg) );
		msg.Category = MP_CHEAT;
		msg.Protocol = MP_CHEAT_DISCONNECT_SERVER;
		msg.dwObjectID = HEROID;
		NETWORK->Send( &msg, sizeof(msg) );
		return TRUE;
	}
...
void CCheatMsgParser::Cheat_Move_Ack( void* pMsg )
{
	...
	{
		CObject* const movedobject = OBJECTMGR->GetObject(pmsg->dwObjectID);
		if(!movedobject) return;

		MOVEMGR->InitMove(movedobject,&info);
	} else
	{
