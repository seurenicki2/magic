...
void CGameIn::UserConn_GameIn_Ack( void* pMsg )
{
	...
	if(GTMAPNUM == MAP->GetMapNum() || MAP->GetMapNum() == PVP)//aziz pvp normal size
	{
...
void CGameIn::UserConn_Character_Add(void* pMsg )
{
	...
	else if(GTMAPNUM == MAP->GetMapNum() || MAP->GetMapNum() == PVP)//aziz normal size pvp
	{
		pPlayer->SetCharacterSize(
			1.0f);
	}
...
void CGameIn::Signal_UserAddItem( void* pMsg )
{
	...
	CHATMGR->AddMsg(CTC_ENCHANT, "%s : Obtained %s", msg->CharacterName, szItemName);
}
