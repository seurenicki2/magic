...
void CMoveManager::Move_OneTarget( void* pMsg )
{
	...
	if( pObject->GetObjectKind() == eObjectKind_Player )
	{
		...
		//Switch Fly Walk--------------------------
		playerObject->SetFlyMode(pmsg->fly);
		//-----------------------------------------
...
void CMoveManager::Move_FadeInOut( void* pMsg )
{
	...
	int curMapNum = MAP->GetMapNum(); //Alemuri FadeInOut Remove in CS
	if (curMapNum != The_Castle_of_Rushen && curMapNum != The_Castle_of_Zevyn && curMapNum != The_Culvert  ) //Alemuri FadeInOut Remove in CS
	{
		pDlg->FadeIn() ;
	}
...
//wasd movement //beyond
BOOL CMoveManager::GoTarget(VECTOR3* pPos,BOOL bMousePointDisplay)
{
	CHero* pHero = OBJECTMGR->GetHero();
	...
	// 090907 ShinJS --- 길찾기를 이용한 이동처리 (5팀 Source 적용)
	// 091207 ShinJS --- 지형 클릭 이동시 이동불가지역인 경우 이동처리 추가
	return HERO->Move_WASDPath( pPos, TRUE, bMousePointDisplay, TRUE );//TRUE
}