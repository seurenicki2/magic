...
void cQuickItem::PutSkillTip()
{
	...
	if(cActiveSkillInfo* const skillInfo = (cActiveSkillInfo*)SKILLMGR->GetSkillInfo(GetData()))
	{
		if(skillInfo->GetCoolTime() > gCurTime)
		{
			...
			//Alemuri cooltime skill Text-----------------------------------------------
			float coolTime = float(skillInfo->GetCoolTime() - gCurTime);
			coolTime /= 1000;
			DWORD temp = coolTime;
			int digitCounter = 0;
			while(temp != 0)
			...
			{
				char cooldown[10];
				sprintf(cooldown, "%s%.2f", " ", coolTime);
				RECT rect={m_absPos.x + 3, m_absPos.y + m_height * (1 - 0.6f)};
				CFONT_OBJ->RenderFont(cFont::FONT13,cooldown,4,&rect,0xffffffff);
			}
			else if (digitCounter == 2)
			{
				char cooldown[10];
				sprintf(cooldown, "%.2f", coolTime);
				RECT rect={m_absPos.x + 3, m_absPos.y + m_height * (1 - 0.6f)};
				CFONT_OBJ->RenderFont(cFont::FONT13,cooldown,4,&rect,0xffffffff);			
			}
			else if (digitCounter > 2)
			{
				char cooldown[10];
				sprintf(cooldown, "%d", (DWORD)coolTime);
				RECT rect={m_absPos.x + 4, m_absPos.y + m_height * (1 - 0.6f)};
				CFONT_OBJ->RenderFont(cFont::FONT13,cooldown,4,&rect,0xffffffff);			
			}
			//--------------------------------------------------------------------------
		}
