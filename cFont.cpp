...
#else
	m_pFont[idx] = g_pExecutive->GetRenderer()->CreateFontObject(pFont, D3DX_FONT);
#endif
...
void cFont::RenderFont(WORD wFontIdx, char * text, int size, RECT * pRect, DWORD color)
{
	...
	pRect->bottom = pRect->top + GetTextHeight( wFontIdx ) + 10;	//+1안하면 밑줄이 안나온다.
...
void cFont::RenderFontShadow(WORD wFontIdx, int nShadowDistance, char* text, int size, RECT* pRect, DWORD dwColor)
{
	...
	pRect->bottom = pRect->top + GetTextHeight( wFontIdx ) + 10;	
