					...
					*/
					if( m_bStyle == LDS_NORMAL )
					{
						...
						if( (m_ClickSelected-m_lTopListItemIdx) == line && (m_bShowSelect == TRUE))
							...
							CFONT_OBJ->RenderFont(m_wFontIdx,item->string,lstrlen(item->string),&rect,RGBA_MERGE(item->rgb, m_alpha * m_dwOptionAlpha / 100)); //Alemuri Normal Chat Render Font
					}
					else
					{
...
void cListDialog::AddItemChatLink( const char* text, DWORD color, std::vector<std::string>ToolTipText, std::vector<DWORD> ToolTipColor, DWORD itemIdx, int line, DWORD dwData, WORD wImageIdx )
{
	//ITEM* pItem = new ITEM;
	//strcpy( pItem->string, text);
	//pItem->rgb = color;
	//pItem->line = line;
	//pItem->nFontIdx = cFont::FONT14 ;
	//cListItem::AddItem(pItem);

	//AddItemWithFont(pItem->string, color, pItem->nFontIdx, line);
	AddItem( text, color-70 ,line, dwData );

	cListDialog::ToolTipTextList toolTipList;

	for (unsigned int i = 0; i < ToolTipText.size(); ++i)
	{
		toolTipList.push_back(ToolTipText[i]);
	}

	// 080225 LUJ, 툴팁이 있다면 추가한다
	if( ! toolTipList.empty() )
	{
		ToolTip toolTip;
		toolTip.nPos = m_ListItem.GetCount() - 1;
		...
		toolTip.mIcon = new cIcon;
		{
			...
			for(
				ToolTipTextList::const_iterator it = toolTipList.begin();
				toolTipList.end() != it;
			++it )
			{
				std::string str(it->c_str());
				//str.substr(0, str.size() - 7);
				//toolTip.mIcon->AddToolTipLine( it->c_str() );
				
				if (str.length() > 7 && str.substr(str.size() - 7) == "㎣FALSE")
				{
					toolTip.mIcon->AddToolTipPartialLine(FALSE, str.substr(0, str.size() - 7).c_str(), ToolTipColor[colorIndex]);
					++colorIndex;
				}
				else if (str.length() > 6 && str.substr(str.size() - 6) == "㎣TRUE")
				{
					toolTip.mIcon->AddToolTipPartialLine(TRUE, str.substr(0, str.size() - 6).c_str(), ToolTipColor[colorIndex]);
					++colorIndex;
				}
				else
				{
					toolTip.mIcon->AddToolTipLine( it->c_str(), ToolTipColor[colorIndex], itemIdx, 0 );
					++colorIndex;
				}
			}
		}
