...
const DWORD dwChatTextColor[] = {
	...
	RGBA_MAKE(200, 200, 200, 255),			//CTC_SYSMSG
	...
	RGBA_MAKE(255, 255, 255, 255),		//ENCHANT//johan cek (0, 230, 255, 255),
	RGBA_MAKE(255, 255, 255, 255),		//EVENT
};
...
public:
	...
	void AddMsgItemChat(std::vector<std::string>, std::vector<DWORD>, DWORD itemIdx, eTextClass, LPCTSTR, ...);
	...
	__time64_t GetForbidChatTime();

	void resetSearDropPartialText() {m_tempSearchDropText = "";}
...
private:
	...
	void Chat_Normal_ItemChat(LPVOID);
	void Chat_Party_ItamChat(LPVOID);
	void Chat_Family_ItemChat(LPVOID);
	void Chat_Alliance_ItemChat(LPVOID);
	void Chat_Guild_ItemChat( LPVOID );
	void Chat_Trade_ItemChat(LPVOID);
	void Chat_Shout_Item_Ack(LPVOID);
	void Chat_Whisper_Item_Ack(LPVOID);
	void Chat_WhisperItem(LPVOID);
	

	//Alemuri Search Drop From Monsters (Function to split a string in a vector)------------------------------------
	std::vector<std::string>	splitStringOnVector(char *phrase, string delimiter, string delimiterBetweenWords);
	DWORD m_dwProgressTimeSearchDrop;
	std::string currentChatEditBoxText;
	std::string m_tempSearchDropText;
	//--------------------------------------------------------------------------------------------------------------
};