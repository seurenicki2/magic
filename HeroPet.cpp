...
void CHeroPet::SetExp( WORD exp )
{
	...
	((CObjectGuagen*)GAMEIN->GetPetUIDlg()->GetWindowForID(PET_UI_FRIENDLY_GUAGE))->SetValue(
		(float)exp/(float)maxexp, 0 );
	char szValue[50];
	//RL3 FIX PET BAR EXP
	//sprintf( szValue, "%d%%", exp, maxexp );
	WORD wMaxExp = PETMGR->GetExp( mGrade );
	sprintf( szValue, "%.1f%%", (float) exp / (float)wMaxExp * 100.0f );
	cStatic* ps = (cStatic *)GAMEIN->GetPetUIDlg()->GetWindowForID(PET_UI_FRIENDLY_STATIC);
