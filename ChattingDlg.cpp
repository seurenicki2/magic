...
void CChattingDlg::Linking()
{
	...
	// E ¿É¼Ç Ãß°¡ added by hseos 2007.06.19
}
...
void CChattingDlg::InspectMsg(char* str)
{
	...
	// 0901128 LYW --- ChattingDlg :Ä³¸¯ÅÍ°¡ »óÁ¡À» È°¼ºÈ­ ÇÑ »óÅÂÀÎÁ¦ È®ÀÎÇÑ´Ù.
	// 091209 ONS Ä³¸¯ÅÍ »ç¸ÁÈÄ¿¡µµ Ã¤ÆÃÀÌ °¡´ÉÇÏµµ·Ï ¼öÁ¤
...
BOOL CChattingDlg::Check_LifeAction( char* str )
{
	...
	while(pos)
	{
		...
		// 091022 ShinJS --- Å»°Í Å¾½Â½Ã »ç¿ëÇÒ¼ö ¾ø´Â Action ÀÎ °æ¿ì
	...
	case 3 :	// ´ë»ó
		{
			...
			// 081210 LYW --- ChattingDlg : Å¸°Ù ¸í·É¾î¿¡¼­, ½ºÅÚ½º¸¦ »ç¿ëÇÏ´Â À¯Àú´Â Å¸°ÙÀÌ ¾ÈÀâÈ÷µµ·Ï ÇÑ´Ù.
			if( pObject->GetObjectKind() == eObjectKind_Player )
			{
				...
				{
					// HERO°¡ ºñÈ¦´õ ½ºÅ³ÀÌ È°¼ºÈ­ µÇ¾îÀÖÀ¸¸é, ´ë»óÀ» ÀâÀ» ¼ö ÀÖµµ·Ï ÇÑ´Ù.
			...
			float fDistance = 0.0f ;																// HERO¿ÍÀÇ °Å¸®¸¦ ´ãÀ» º¯¼ö¸¦ ¼±¾ðÇÑ´Ù.

			VECTOR3 vHeroPos;																		// HEROÀÇ À§Ä¡¸¦ ´ãÀ» º¤ÅÍ¸¦ ¼±¾ðÇÑ´Ù.
			HERO->GetPosition(&vHeroPos) ;															// HEROÀÇ À§Ä¡¸¦ ¹Þ´Â´Ù.
			fDistance = CalcDistanceXZ(&vHeroPos, &pObject->GetBaseMoveInfo()->CurPosition) ;				// HERO¿Í ¸ó½ºÅÍÀÇ °Å¸®¸¦ ±¸ÇÑ´Ù.
	...
	case 20 :	// ¼­±â
		{
			...
			// S ³óÀå½Ã½ºÅÛ Ãß°¡ added by hseos 2007.09.20
			if (g_csFarmManager.CLI_GetIngCharMotion()) return FALSE;
			// E ³óÀå½Ã½ºÅÛ Ãß°¡ added by hseos 2007.09.20
	...
	case 24 :	// ÀÎÃ¦Æ®
		...
		break;
//---KES »óÁ¡°Ë»ö 2008.3.11
	case 26:
		{
			//pNameÀº ÀÌ¸§ÀÌ ¾Æ´Ï°í °Ë»öÇÒ ´Ü¾îÀÌ´Ù.
...
void CChattingDlg::Msg_Guild()
{
	if (ITEMMGR->GetIsItemChat())
	{
		...
		data.dwObjectID = HEROID;								// 090317 ONS ¿¡ÀÌÀüÆ®¼­¹öÀÇ ÆÐÅ¶Á¶ÀÛ ¹æÁöÃ³¸®¿¡ µû¸¥ ¸Þ¼¼Áö º¯°æ(GuildIdx->HEROID)
		data.dwGuildIdx = HERO->GetGuildIdx();					// 090317 ONS ¿¡ÀÌÀüÆ®¼­¹öÀÇ ÆÐÅ¶Á¶ÀÛ ¹æÁöÃ³¸®¿¡ µû¸¥ ¸Þ¼¼Áö º¯°æ(HEROID->GuildIdx)
		data.itemIdx = ITEMMGR->GetItemLocked()->GetItemIdx();
		SafeStrCpy(data.Msg, m_szSendMsg, MAX_CHAT_LENGTH+1 );
	...
	else
	{
		...
		MSG_GUILD_CHAT data;									// 090317 ONS ¿¡ÀÌÀüÆ®¼­¹öÀÇ ÆÐÅ¶Á¶ÀÛ ¹æÁöÃ³¸®¿¡ µû¸¥ ¸Þ¼¼Áö º¯°æ(TESTMSGID->MSG_GUILD_UNION_CHAT)
		data.Category = MP_CHAT;
...
void CChattingDlg::Msg_Alliance()
{
	...
	if (ITEMMGR->GetIsItemChat())
	{
		...
		{
			//MSG_GUILDUNION_CHAT message;							// 090317 ONS ¿¡ÀÌÀüÆ®¼­¹öÀÇ ÆÐÅ¶Á¶ÀÛ ¹æÁöÃ³¸®¿¡ µû¸¥ ¸Þ¼¼Áö º¯°æ(MSG_CHAT_WITH_SENDERID->MSG_GUILD_UNION_CHAT)
			ITEM_CHAT_LINK message;
			message.Category	= MP_CHAT;
			message.Protocol	= MP_CHAT_GUILDUNION_ITEMCHAT;
			message.dwObjectID	= gHeroID;					// 090317 ONS ¿¡ÀÌÀüÆ®¼­¹öÀÇ ÆÐÅ¶Á¶ÀÛ ¹æÁöÃ³¸®¿¡ µû¸¥ ¸Þ¼¼Áö º¯°æ(GuildUnionIdx->HEROID)
			message.dwGuildUnionIdx	= hero->GetGuildUnionIdx();		// 090317 ONS ¿¡ÀÌÀüÆ®¼­¹öÀÇ ÆÐÅ¶Á¶ÀÛ ¹æÁöÃ³¸®¿¡ µû¸¥ ¸Þ¼¼Áö º¯°æ(HEROID->GuildUnionIdx)
...
void CChattingDlg::Msg_Whisper() 
{
	...
	//{
		...
		//	std::replace( replaceApo.begin(), replaceApo.end(), '\'', 'ƒ'); //Replace Apo with another symbol so that the chat can process it
		...
		// desc_hseos_¹®ÀÚÇÊÅÍ¸µ01
		// S ¹®ÀÚÇÊÅÍ¸µ Ãß°¡ added by hseos 2007.06.25
		// ..ÇÊÅÍ¿¡¼­ Á¦¿ÜÇÒ ¹®ÀÚ Ã³¸®¸¦ À§ÇØ Å¸ÀÔÀ» ¼³Á¤ÇÑ´Ù.
		FILTERTABLE->SetExceptionInvalidCharIncludeType(FET_WHISPER_CHARNAME);
		// E ¹®ÀÚÇÊÅÍ¸µ Ãß°¡ added by hseos 2007.06.25
		if( (FILTERTABLE->IsInvalidCharInclude((unsigned char*)m_szWhisperName)) == TRUE )
		{
			CHATMGR->AddMsg(CTC_SYSMSG, CHATMGR->GetChatMsg(368), RGBA_MAKE(94, 94, 94, 255));
			return;
...
void CChattingDlg::AddMsg( BYTE byMessageType, BYTE byTabBtnType, char* msg, DWORD dwColor )
{
	...
	for( int i = 0 ; i < nBrkNum ; ++i )
	{
		...
		{
			if( nCount == 1 )	//ÇÑ´Ü¾î¸¸ ½è´Âµ¥ ³Ñ¾î°¡¹ö¸®¸é °­Á¦·Î ²÷´Â´Ù.
			{
				for( int k = 1 ; k <= nCutPos ; ++k )
				{
					if( CFONT_OBJ->GetTextExtentEx(0, msg, k ) > CHATLIST_TEXTEXTENT )
					{
						nCutPos = nCutPos - brk[i] + k-1; //¾´ ±ÛÀÚ±îÁö¸¸
						strncpy( buf, msg, nCutPos );	//Àü´Ü°è±îÁö
						buf[nCutPos] = 0;
						...
						{
							// ½Ã½ºÅÛ ¸Þ½ÃÁö´Â ¸ðµÎ, ´ÜÃà Ã¢¿¡¸¸ Ãâ·ÂÇÏµµ·Ï ÇÑ´Ù.
							if( byTabBtnType == e_CONVERSATION_SYSTEM )
							{
								// ±Ó¼Ó¸»/°ÔÀÓ ÆÁÀº ¸ÞÀÎ Ã¤ÆÃÃ¢¿¡ Ãâ·ÂÇÑ´Ù.
			...
			else
			{
				...
				// 080917 LYW --- ChattingDlg : ½Ã½ºÅÛ ÅÇÀÌ È°¼ºÈ­ µÇ¾î ÀÖÀ» ¶§, ±Ó¼Ó¸»/ÆÁÀº ¸ÞÀÎÃ¢¿¡ Ãâ·ÂÇÏ°í,
				// ³ª¸ÓÁö ¸Þ½ÃÁöµéÀº ½Ã½ºÅÛ ´ÜÃà Ã¢¿¡¸¸ Ãâ·ÂÇÏµµ·Ï Ã³¸® - (¿ø»ó¹Ì).
				if( m_pTabButton[e_CONVERSATION_SYSTEM]->IsPushed() 
				&& m_byCurAssistance == e_CONVERSATION_SYSTEM 
				&& m_pAssistList->IsActive() )	// ½Ã½ºÅÛ ´ÜÃàÃ¢ÀÌ È°¼ºÈ­ µÇ¾î ÀÖÀ¸¸é,
				{
					// ½Ã½ºÅÛ ¸Þ½ÃÁö´Â ¸ðµÎ, ´ÜÃà Ã¢¿¡¸¸ Ãâ·ÂÇÏµµ·Ï ÇÑ´Ù.
					if( byTabBtnType == e_CONVERSATION_SYSTEM )
					{
	...
	// 091201 ShinJS --- Dialog¿¡ ¼³Á¤µÈ TextRect ±æÀÌ·Î Text¸¦ ÀÚ¸¥´Ù.
	int nLen = strlen( msg );
...
BOOL CChattingDlg::Event_LButtonClick(CMouse* mouseInfo)
{
	...
	else if( m_pCloseBtn->PtInWindow( xPos, yPos ) )
	{
		...
		// 080312 NYJ --- Ã¤ÆÃº¸Á¶Ã¢(=ÆË¾÷Ã¢) ´ÝÀ»¶§ ¿É¼Ç ÀúÀåÇÏ´Â ÄÚµå Ãß°¡.
...
void CChattingDlg::Event_PushDown(CMouse* mouseInfo)
{
	...
	// desc_hseos_¿É¼Ç01
	// S ¿É¼Ç Ãß°¡ added by hseos 2007.06.19
	// ..Ã¤ÆÃ ¸ðµå¸¦ ÀúÀå. ¸ÊÀÌµ¿½Ã ºÒ·¯¿È
	sGAMEOPTION stOption = *OPTIONMGR->GetGameOption();
...
void CChattingDlg::ExtendDlg()
{
	...
	// desc_hseos_¿É¼Ç01
	// S ¿É¼Ç Ãß°¡ added by hseos 2007.06.20
	// ..Ã¤ÆÃÃ¢ ¶óÀÎ ¼ö¸¦ ÀúÀå. ¸ÊÀÌµ¿½Ã ºÒ·¯¿È
	sGAMEOPTION stOption = *OPTIONMGR->GetGameOption();
	stOption.nChatLineNum = middleCount;
	OPTIONMGR->SetGameOption(&stOption);
	// E ¿É¼Ç Ãß°¡ added by hseos 2007.06.20
}
...
void CChattingDlg::CallAssistanceDlg(CONVERSATION byClass)
{
	...
	// 080918 LYW --- ChattingDlg : ½Ã½ºÅÛ ¸Þ½ÃÁö ÀÛ¾÷ - (¿ø»ó¹Ì)
...
void CChattingDlg::SetRaceActive(WORD wRace, BOOL bActive)
{
	...
	// Ã¢ÀÇ À§Ä¡ Á¶Á¤ ¶§¹®¿¡... ´Ù½Ã CallAssistanceDlg¸¦ È£ÃâÇÏ¿©ÁØ´Ù.
