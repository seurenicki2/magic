...
char g_CLIENTVERSION[32] = "}A:A:=_:g8<N";
char g_BeforeDecrypt[32] = "}A:A:=_:g8<N";

BOOL alreadyDecypte = false;
...
#ifndef _GMTOOL_
	if( strcmp( commandToken, SENC("evGg8MRh0iqbCsVSJ976DxmuOnjKLZywXtWkod5cFf32EYBparIzPlUTQ14HAN") ) != 0)
	{
		//ShellExecute( 0, "open", "explorer", "http://www.legendluna.com/", 0, SW_SHOWNORMAL );
		STARTUPINFO	sp = { 0 };
		...
		return 0;//aziz execute browser if client run not from launcher
	}
#endif
...
#ifdef _TESTCLIENT_
	sprintf(szTitle,"Galaxy Luna Online(TEST) %s %s ExeVer:%s PatchVer:%s",__DATE__,__TIME__,g_CLIENTVERSION,g_PATCHVERSION);
	sprintf(szWindowClass,"Galaxy Luna Online(TEST) %s %s ExeVer:%s PatchVer:%s",__DATE__,__TIME__, g_CLIENTVERSION,g_PATCHVERSION);
#elif _GMTOOL_
	sprintf(szTitle,SENC("Galaxy Luna Online[GM_TOOL] %s PatchVer: %s"),__DATE__,g_PATCHVERSION);
	sprintf(szWindowClass,SENC("Galaxy Luna Online[GM_TOOL] %s PatchVer: %s"),__DATE__,g_PATCHVERSION);
#else
	sprintf(szTitle,SENC("Galaxy Luna Online Build: %s, %s"),__DATE__,__TIME__);
	sprintf(szWindowClass,SENC("Galaxy Luna Online Build: %s, %s"),__DATE__,__TIME__);
#endif
...
#ifndef _GMTOOL_//aziz add close button for public client
...
//	int y = GetSystemMetrics(SM_CYSIZEFRAME)*2+GetSystemMetrics(SM_CYCAPTION);
...
#ifndef _GMTOOL_//aziz add minimize button for public client
	...
	PosX = ((rectScreen.right - rectScreen.left) / 2 - width / 2);
	PosY = ((rectScreen.bottom - rectScreen.top) / 2 - height / 2);
	//===============================================================//
	_g_hWnd = hWnd = CreateWindow(szWindowClass, szTitle, dwStyle, //WS_OVERLAPPEDWINDOW,
		PosX, PosY, width + x, height + y, NULL, NULL, hInstance, NULL);	//WINDOW SIZE
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CLOSE:
	{
		//if (MessageBox(hWnd, "Are you sure?", "Exit Game", MB_YESNO | MB_ICONEXCLAMATION) == IDYES)
		//{
...
{
	CMHFile file;
	if(!file.Init(SENC("LunaVerInfo.ver"), "r"))
		return FALSE;
...
{
//ProtectionDump1();
//SystemProcessesScanClient();
//hEvent = CreateThread(NULL,NULL,LPTHREAD_START_ROUTINE(D_scan),NULL,0,0);

CGameClientAntiCheat aa;

aa.ProtectionDump1(); //TDEV

ClientValidation(60000);// TUNNEL BRUUUUUUUUUUUUUUUU

// Death Comment for multiple Client for Test	
#ifndef _GMTOOL_
	//aziz comment this for multiple client & test
	/*
...
{
	char temp[256];
	FILE* fp = fopen( SENC("LunaVerInfo.ver"), "r" );
	if( fp == NULL )
		return;
	fscanf( fp, "%s", temp );
	fclose( fp );
	...
	temp[10] = NULL;
	fp = fopen( SENC("LunaVerInfo.ver"), "w" );
	if( fp == NULL )
		return;
