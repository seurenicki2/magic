...
void cConductManager::RegistConduct()
{
		...
		info = m_ConductInfoTable.GetData())
	{
		...
		/* [ 7 Jan 2013 ] Death , Fix to Dynamic Tab Sheet For new Emotion */
		int Tabsheet = info->GetConductPos() / 36; // 6 Rows * 6 Columns
		int IconPosition = info->GetConductPos() % 36; // 6 Rows * 6 Columns
	...
	case 64:
		{
			if(MAP->GetMapNum() == StreetTournament || MAP->GetMapNum() == PVP_MAP )
			{
				CHATMGR->AddMsg( CTC_OPERATEITEM, "PK Mode is disable at this map" );
				return;
			}
	...
	case 70 : 
		{
			...
			SHOWDOWNMGR->ApplyShowdownParty() ;
		}
	break ;
	//--------------------------------------------------------------------------------------------------------------------
