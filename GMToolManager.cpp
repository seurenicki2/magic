...
BOOL CGMToolManager::CreateGMSubDialog()
{
	...
	m_hWndSub[eMenu_Mapjackpot]= CreateDialog( g_hInst, MAKEINTRESOURCE(IDD_GM_MAPJACKPOT), m_hWndDlg, GMMapjackpotDlgProc );
	return TRUE;
}
...
//							if( pInfo->Grade == 0 )
			...
			SendDlgItemMessage( m_hWndSub[eMenu_Mapjackpot], IDC_GMJACKPOTVALUE, CB_SETCURSEL, 0, 0 );
	...
	case eMenu_ForbidChat:
		{
			...
			case eGM_POWER_PATROLLER:	return;	break;
			case eGM_POWER_AUDITOR:		break;
			case eGM_POWER_EVENTER:		return; break;
			case eGM_POWER_QA:			return;	break;
			}
		}
		break;
	...
	case eMenu_Event:
		{
			switch( m_nPower )
			{
			case eGM_POWER_MASTER:		return; break;
			case eGM_POWER_MONITOR:		return;	break;
			case eGM_POWER_PATROLLER:	return;	break;
			case eGM_POWER_AUDITOR:		return;	break;
			case eGM_POWER_EVENTER:		return; break;
...
BOOL CGMToolManager::OnItemCommand()
{
	...
	if( SendMessage( GetDlgItem( m_hWndSub[eMenu_Item], IDC_GMITEM_BTN_WEAPON ), BM_GETCHECK, 0, 0 ) == BST_CHECKED )
	{
		...
		//Alemuri Fix the Crash in case you push Get with an incomplete line
		std::string bufCheck(buf);
		if (bufCheck.find('(') != std::string::npos)
		{
			//Found
		}
		else
	...
	else if( SendMessage( GetDlgItem( m_hWndSub[eMenu_Item], IDC_GMITEM_BTN_POTION ), BM_GETCHECK, 0, 0 ) == BST_CHECKED )
	{
		...
		if( rt )
		{
			...
			else if( nCount > 1000 )		nCount = 1000;
	...
	{
		//BOOL rt;
		//MONEYTYPE dwMoney = (ULONGLONG)GetDlgItemInt(m_hWndSub[eMenu_Item], IDC_GMITEM_EDT_MONEY, &rt, TRUE);
		//if( !rt )	return FALSE;
		//aziz, hot fix get money from gmtools //johan cek
...
BOOL CGMToolManager::OnSetJackpot()
{
	if( !m_hWndSub[eMenu_Mapjackpot] ) return FALSE;
	SetFocus( _g_hWnd );

	WORD jackAllmap = 0;
	if( SendMessage( GetDlgItem( m_hWndSub[eMenu_Mapjackpot], IDC_GMJACKPOTALLMAP ), BM_GETCHECK, 0, 0 ) == BST_CHECKED )
	jackAllmap = 1;

	BOOL rt;

	MONEYTYPE dwMoney = GetDlgItemInt( m_hWndSub[eMenu_Mapjackpot], IDC_GMJACKPOTVALUE, &rt, TRUE );
	
	if(MAINGAME->GetUserLevel() > eUSERLEVEL_PROGRAMMER)
	{
		CHATMGR->AddMsg( CTC_TOGM, "You'r GM access can't Add Jackpot" );
		return FALSE ;
	}

	MSG_DWORDEX msgslotsyn;
	memset(&msgslotsyn, 0, sizeof(MSG_DWORDEX));
	msgslotsyn.Category	= MP_CHEAT;
	msgslotsyn.Protocol	= MP_CHEAT_ADDJACKPOT_SYN;
	msgslotsyn.dwObjectID	= gHeroID;
	msgslotsyn.dweData		= dwMoney;
	//msg.dweData2	= jackAllmap;

	NETWORK->Send( &msgslotsyn, sizeof(msgslotsyn) );

	CHATMGR->AddMsg( CTC_TOGM, "sent jackpot msg %s - %d", AddComma(dwMoney), jackAllmap );
...
INT_PTR CALLBACK GMMapjackpotDlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch( message )
	{
	case WM_COMMAND:
		{
			if( HIWORD( wParam ) == BN_CLICKED )
			{
				switch( LOWORD( wParam ) )
				{
				case IDC_GMJACKPOTSEND:
					{
						GMTOOLMGR->OnSetJackpot();
					}
					break;
				}
			}
...
void CGMToolManager::AddChatMsg( char* pName, char* pMsg, int nKind, WORD type )
{
	if( !m_hWndChatList && type != 1 ) return;
