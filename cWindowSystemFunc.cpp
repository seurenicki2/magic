...
void SendDeleteIconMsg(cIcon * pIcon)
{
	...
	{
		//inspect
		if (GAMEIN->GetCharacterEquipInfoDlg()->IsActive()) return;
		...
		{
			CItem*	targetItem			= ( CItem* )pIcon;
			const ITEM_INFO*	info	= targetItem->GetItemInfo();
			const ITEM_OPTION& option	= ITEMMGR->GetOption(targetItem->GetItemBaseInfo());
			
			if(info->EquipType == eEquipType_Weapon || info->EquipType == eEquipType_Armor || info->EquipType == eEquipType_Accessary || (targetItem->GetItemBaseInfo().Position >= TP_WEAR_START && targetItem->GetItemBaseInfo().Position <= TP_WEAR_END))
			{
				char itemnya[128] = "";
				
				if(option.mEnchant.mLevel >= 12)
				{
					sprintf(itemnya, "+%d %s", option.mEnchant.mLevel, info->ItemName);
					CChaptchaDlg* ChaptchaDlg = (CChaptchaDlg*)GAMEIN->GetChaptchaDlg();
					if(!ChaptchaDlg)
					{
						ITEMMGR->FakeDeleteItem((CItem *)pIcon);
						ChaptchaDlg->SetTypeCaptcha(3);
						ChaptchaDlg->SetItemnameTodelete(itemnya);
						ChaptchaDlg->SetTypeGame(FALSE);
						ChaptchaDlg->SetActive(TRUE);
						CHATMGR->AddMsg( CTC_OPERATEITEM, "Are you sure delete this item? we are detect this item enchant level 12+. Please fill captcha!" );
					} else {
						ITEMMGR->FakeDeleteItem((CItem *)pIcon);
						ChaptchaDlg->SetTypeCaptcha(3);
						ChaptchaDlg->SetItemnameTodelete(itemnya);
				...
				} else if (targetItem->GetItemBaseInfo().Position >= TP_WEAR_START && targetItem->GetItemBaseInfo().Position <= TP_WEAR_END)
				{
					sprintf(itemnya, "%s", info->ItemName);
					CChaptchaDlg* ChaptchaDlg = (CChaptchaDlg*)GAMEIN->GetChaptchaDlg();
					if(!ChaptchaDlg)
					{
						ITEMMGR->FakeDeleteItem((CItem *)pIcon);
						...
						ChaptchaDlg->SetTypeGame(FALSE);
						ChaptchaDlg->SetActive(TRUE);
						CHATMGR->AddMsg( CTC_OPERATEITEM, "Are you sure delete this item? we are detect this item is equiped. Please fill captcha, for delete the item!" );
					} else {
						ITEMMGR->FakeDeleteItem((CItem *)pIcon);
						ChaptchaDlg->SetTypeCaptcha(3);
						ChaptchaDlg->SetItemnameTodelete(itemnya);
