...
DWORD cComboBox::ActionEvent(CMouse * mouseInfo)
{
	...
	if(m_pComboBtn->IsPushed())
	{
		...
		if( m_bComboBoxSelected )
		{
			we |= WE_COMBOBOXSELECT;
			return we;
		}
	}
...
void cComboBox::ListMouseCheck( CMouse * mouseInfo )
{
	...
	if( LeftDown )
	{
		...
		if(m_nCurSelectedIdx > GetItemCount() )
		{
		...
		else if( !WINDOWMGR->IsMouseDownUsed() )
		{
			...
			// 100615 콤보 리스트가 선택 되었는지 아닌지 판단
			m_bComboBoxSelected	=	TRUE;
		}
	}
}
...
void cComboBox::ForceListFold()
{
	m_pComboBtn->SetPush(FALSE);
	m_nCurSelectedIdx = -1;
}

void cComboBox::InsertComboItem( char* strTest )
{
	WORD	nMaxLine	=	GetMaxLine() + 1;
	
	SetMaxLine( nMaxLine );

	ITEM* pItem = new ITEM;
	strcpy(pItem->string, strTest );
