...
//////////////////////////////////////////////////////////////////////////
//판매 아이템 등록 함수
void CStreetStallManager::RegistItemEx( CItem* pItem , MONEYTYPE money )
{
...
void CStreetStallManager::LinkBuyItem( STREETSTALL_INFO* pStallInfo )
{
	...
	for( i=0;i<SLOT_STREETSTALL_NUM;++i)
	{
		...
		if( stallItem.Fill )
		{
			...
			MONEYTYPE money = stallItem.money;
			...
			{
				DWORD color = 0xffffffff;
				MONEYTYPE money = stallItem.Durability * stallItem.money;
			...
			//Alemuri Locke the item if the price is less than 2 gold
			if (money < 2)
...
void CStreetStallManager::LinkSellItem( STREETSTALL_INFO* pStallInfo )
{
	...
	for(int i=0;i<SLOT_STREETSTALL_NUM;++i)
	{
		if( pStallInfo->Item[i].Fill )
		{
			...
			DWORD color = 0xffffffff;
			MONEYTYPE money = pStallInfo->Item[i].money;
			...
			{
				DWORD color = 0xffffffff;
				MONEYTYPE money = pStallInfo->Item[i].money * pStallInfo->Item[i].wVolume;
...
void CStreetStallManager::StreetStall_Open_Nack() 
{
	...
	{
		GAMEIN->GetStreetBuyStallDialog()->RegistTitle(NULL, FALSE);
		//Shop Buy is Deactivated Notice
		CHATMGR->AddMsg(CTC_SYSMSG, CHATMGR->GetChatMsg(5116));
	}
