...
enum eMONSTERMOTIONIDLE
{
	eMonsterMotionIdle_Stand,
	eMonsterMotionIdle_Stand2,
	eMonsterMotionIdle_Battle,

	eMonsterMotionIdle_Max,
};

enum eNPCMOTIONIDLE
{
	eNpcMotionIdle_None,
	eNpcMotionIdle_Stand,
	eNpcMotionIdle_Stand2,
...
public:
	...
	void SetStandRate(BYTE byRate)
	{
		if(byRate < 0)
			...
			standmotionRate = byRate;
	}
	BYTE GetStandRate()
	{
		return standmotionRate;
	}
private:
	DWORD		motionID;
	CMOTIONBASE motionBase[eMonsterMotionN_Max];
	BYTE		standmotionRate;	// 080321 NYJ --- IDLE2 출력확률
};
