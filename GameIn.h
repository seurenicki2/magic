...
#define BATTLE_SHOWDOWN_READYTIME	6000
#define BATTLE_SHOWDOWN_FIGHTTIME	60000
#define BATTLE_SHOWDOWN_FIGHTTIME_PARTY	300000
...
class CPWScoreInfoDialog;// tdev/tj party war scoreboard
class CGuageDialog;
...
//	CMacroModeDialog	* m_pMacroModeDlg;
	...
	*/
	...
	CPWScoreInfoDialog*			m_pPWScoreInfoDlg;	//TDEV/TJ UG PARTY WAR SCOREBOARD
	// 06. 02. 강화 재료 안내 인터페이스 추가 - 이영준
	...
	//Alemuri Server Time Update-----
	CServerTimeDlg*	m_pServerTimeDlg;
	//-------------------------------
	...
	cImage*	m_WinnerShowImage;
	int m_battleState;
	CImageNumber m_ImageNumberTeam1;
	CImageNumber m_ImageNumberTeam2;
#endif
	//------------------------------
private:
	std::vector<std::vector<DWORD>> preloadMonsterInMap;
	std::vector<DWORD> preloadMonsterInMapList;
	
	//Alemuri Search Mobs From Partial Name------------
	DWORD m_tabIndex;
	//-------------------------------------------------
public:	
	//Alemuri Search Drops From Mobs-------------------------------------------------------
	void SearchDropFromMobs(char*);
	std::vector<std::string> MonsterArrayForDrop;
	std::vector<DWORD> MonsterArrayForDropColors;
	void resetTabIndex() {m_tabIndex = 0;}
	//-------------------------------------------------------------------------------------
	//Alemuri Mob Stats-------------------------------------
	void MobStats(char*);
	std::vector<std::string> MobStatsArray;
	std::vector<DWORD> MobStatsColorArray;
	//------------------------------------------------------
	//Alemuri Find item for Partial Name----------------------------------------------
	std::string FindMobInfoForPartialName( char* strName );
	//--------------------------------------------------------------------------------
	...
	void AfterRender();
	//Alemuri Party War-------------------------------------------------------------
	void setPartyWarResult(int resultType) { m_WinnerTeam = resultType; } //resultType 1 = winner, 2 = loser, 3= draw
	void renderPartyWar();
	void setCurrentRenderState(int battlestate);
	void partyWarImagesInit();
	void partyWarStateFight();
	void partyWarStateWinner();
	void partyWarResetAll();
	void PartyWarScoreBoardUpdater1(int nTeam1);//scoreboard updater for client
	void PartyWarScoreBoardUpdater2(int nTeam2);//scoreboard updater for client
	int nTeam1Score;
	int nTeam2Score;
	//------------------------------------------------------------------------------
...
//	void SetMacroModeDialog(CMacroModeDialog * dlg){ m_pMacroModeDlg=dlg; }
	...
	*/
	...
	CPWScoreInfoDialog*	GetPWScoreInfoDlg()					{	return m_pPWScoreInfoDlg;		}
	void SetPWScoreInfoDlg(CPWScoreInfoDialog* dlg)			{	m_pPWScoreInfoDlg = dlg;		}
	//
	// 06. 02. 강화 재료 안내 인터페이스 추가 - 이영준
...
public :
	...
	//Alemuri Server Time Update--------------------------------------------------
	CServerTimeDlg*	GetServerTimeDlg(){ return m_pServerTimeDlg; }
	void SetServerTimeDlg(CServerTimeDlg *dlg) { m_pServerTimeDlg = dlg; }
	//----------------------------------------------------------------------------

	//Switch fly Walk-------------------------------------
	void switchFlyWalk( int flyMode);
	int heroFlyMode;
	//----------------------------------------------------
