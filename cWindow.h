...
//	void SetToolTip(char * msg, DWORD color=RGB_HALF(255,255,0), cImage * image=NULL, DWORD imgColor = 0x00ffffff );
	...
	void SetToolTipWithFont(int fontIdx, char * msg, DWORD color=RGB_HALF(255,255,0), cImage * image=NULL, DWORD imgColor = TTCLR_DEFAULT ); //Alemuri
	void SetModifiedToolTip(char * msg, DWORD color = RGB_HALF(255, 255, 0), cImage * image = NULL, DWORD imgColor = TTCLR_DEFAULT);
	// 070803 웅주, 줄바꿈되는 툴팁 추가
	void SetAutoLineToolTip( const char*, DWORD color, cImage*, DWORD lineSize );

	void AddToolTipLine(const char* msg, DWORD color = TTTC_DEFAULT, DWORD wItemIdx = 0, int imageIndexArray = 1000);//alemuri
