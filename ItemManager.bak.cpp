...
void CItemManager::AddFunctionToolTip( cIcon& icon, const ITEMBASE& item )
{
	...
	// 강화 판매 교환 보관 
	//icon.AddToolTipLine( "" );
	icon.AddToolTipLine("__________________________________________________________", RGB_HALF(132, 190, 222));
	DWORD ColorActive = RGB_HALF(255, 255, 0);
	DWORD ColorInactive = RGB_HALF(150, 150, 150);
	// 판매 
	if( info->Sell )
	{
		icon.AddToolTipPartialLine(TRUE, CHATMGR->GetChatMsg(1958), ColorActive);
	}
	else
	{
		icon.AddToolTipPartialLine(TRUE, CHATMGR->GetChatMsg(1958), ColorInactive);
	}
	...
	if( info->Trade )
	{
		if( item.nSealed != eITEM_TYPE_SEAL_NORMAL && item.nSealed !=  eITEM_TYPE_SEAL )
		{
			...
			icon.AddToolTipPartialLine(FALSE, CHATMGR->GetChatMsg(1957), ColorInactive);
		}
		else
		{
			icon.AddToolTipPartialLine(FALSE, CHATMGR->GetChatMsg(1957), ColorActive);
		}
	}
	else
	{
	...
	if( info->Deposit )
	{
		icon.AddToolTipPartialLine(FALSE, CHATMGR->GetChatMsg(1959), ColorActive);
	}
	else
	{
		icon.AddToolTipPartialLine(FALSE, CHATMGR->GetChatMsg(1959), ColorInactive);
	}
	icon.AddToolTipPartialLine(FALSE, " | ");
	// 강화 
	if( info->Improvement )
	{
		icon.AddToolTipPartialLine(FALSE, CHATMGR->GetChatMsg(1965), ColorActive);
	}
	else
	{
		icon.AddToolTipPartialLine(FALSE, CHATMGR->GetChatMsg(1965), ColorInactive);
	}
	...
	{
		icon.AddToolTipPartialLine(TRUE, CHATMGR->GetChatMsg(1966), ColorActive);
	}
	else
	{
		icon.AddToolTipPartialLine(TRUE, CHATMGR->GetChatMsg(1966), ColorInactive);
	}
	icon.AddToolTipPartialLine(FALSE, " | ");
	// 분해
	if( GetDissolveScript( info->ItemIdx ) )
	{
		icon.AddToolTipPartialLine(FALSE, CHATMGR->GetChatMsg(1967), ColorActive);
	}
	else
	{
		icon.AddToolTipPartialLine(FALSE, CHATMGR->GetChatMsg(1967), ColorInactive);
	}
	icon.AddToolTipPartialLine(FALSE, " | ");
	// 인챈트
	if( info->IsEnchant )
	{
		icon.AddToolTipPartialLine(FALSE, CHATMGR->GetChatMsg(1968), ColorActive);
	}
	else
	{
		icon.AddToolTipPartialLine(FALSE, CHATMGR->GetChatMsg(1968), ColorInactive);
	}
}
...
void CItemManager::AddNametoolTip( cIcon& icon, const ITEMBASE& item )
{
	...
	// 초기화
	{
		cImage image;
		int	idx = 16;
		cImageRect rect = { 0, 0, 300, 601 };// { 802, 0, 1000, 369 }; //{ 500, 402, 749, 494 };// { 0, 0, 197, 134 };

		SCRIPTMGR->GetImage(idx, &image, &rect);
		//SCRIPTMGR->GetImage( 0, &image, PFT_HARDPATH );
		//icon.SetToolTip( "", RGBA_MAKE(255, 255, 255, 255), &image, TTCLR_ITEM_CANEQUIP );
		icon.SetModifiedToolTip("", RGBA_MAKE(255, 255, 255, 255), &image, RGB_HALF(255, 255, 255));
	}	
	...
	{
		//sprintf(line, "");
		//icon.AddToolTipLine(line, RGB_HALF(255, 0, 0));
...
#define TOOLTIPINFO_EXTENT		280//144
...
void CItemManager::AddOptionToolTip( cIcon& icon, const ITEMBASE& item )
{
	...
	//if( !	info ||	eEquipType_None == info->EquipType )
	//{
	//	return;
	//}
	if (!info)
