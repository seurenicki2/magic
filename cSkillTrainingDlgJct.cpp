#include "stdafx.h"
#include ".\cSkillTrainingDlgJct.h"
...
cSkillTrainingDlgJct::cSkillTrainingDlgJct(void) :
mGuildMode( FALSE )
{
	m_Index = 0;
}
...
void cSkillTrainingDlgJct::Linking()
{
	m_SkillListDlg = ( cListDialog* )GetWindowForID( GSKILL_TRAINLIST );
	m_SkillListDlg->SetShowSelect( TRUE );
	m_SkillListDlg->SetAutoScroll( FALSE );

	m_SkillTipListDlg = ( cListDialog* )GetWindowForID( GSKILL_TRAINTIPLIST );
	m_SkillTipListDlg->SetShowSelect( FALSE );
	m_SkillTipListDlg->SetAutoScroll( FALSE );;

	m_SkillPoint = ( cStatic* )GetWindowForID( GSKILL_TRAINPOINT );
	m_Gold = ( cStatic* )GetWindowForID( GSKILL_TRAINGOLD );

	m_Icon = ( cStatic* )GetWindowForID( GSKILL_TRAIN_ICON );
	m_NeedSp = ( cStatic* )GetWindowForID( GSKILL_TRAIN_SP );
	m_NeedGold = ( cStatic* )GetWindowForID( GSKILL_TRAIN_GOLD );

	m_AddDamageOptTooltip = ( cStatic* )GetWindowForID( GSKILL_TRAIN_ADDDAMAGE_OPT_TOOLTIP_TEXT );
}

// 080417 LUJ, 길드 스킬 표시
void cSkillTrainingDlgJct::OpenGuildSkill()
{
	...
	{
		cWindow* windowSkillText	= GetWindowForID( GSKILL_TRAIN_POINT_TEXT );
		cWindow* windowSkillImage	= GetWindowForID( GSKILL_TRAIN_POINT_IMAGE );

		cWindow* windowGuildText	= GetWindowForID( GSKILL_TRAIN_GUILD_POINT_TEXT );
		cWindow* windowGuildImage	= GetWindowForID( GSKILL_TRAIN_GUILD_POINT_IMAGE );
		...
		cStatic* windowTitle = ( cStatic* )GetWindowForID( GSKILL_TRAIN_TITLE );
...
void cSkillTrainingDlgJct::DisplayGuildSkill()
{
	cWindow* windowSkillText	= GetWindowForID( GSKILL_TRAIN_POINT_TEXT );
	cWindow* windowSkillImage	= GetWindowForID( GSKILL_TRAIN_POINT_IMAGE );
...
//-------------------------------------------------------------------------------------------------
int cSkillTrainingDlgJct::GetSkillLevelFromJobTree(DWORD dwSkillIdx, WORD wCurJobLevel)
{
	// 리턴할 결과를 담을 변수.
	int nResult = -1 ;
...
//-------------------------------------------------------------------------------------------------
void cSkillTrainingDlgJct::AddSkillToSkillListDlg(BYTE byJobType, WORD wJobLevel, WORD* pPos, LONG lId)
{
	// 직업 계열의 유효성 체크.
	if( wJobLevel == 1 ) byJobType = 1 ;
	if(!byJobType) return ;
	//JobTree
	BYTE Race = HERO->GetCharacterTotalInfo()->Race;
	WORD Fighter = 1000;
	WORD Rogue = 2000;
	WORD Mage = 3000;
	WORD Devil = 4000;
	...
	if( lId == CLASS_1 )
	{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel1 + 1;}
		else if( lId == CLASS_1_1 )
		{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel2 + 1;}
		else if( lId == CLASS_1_2 )
		{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel2 + 2;}
		else if( lId == CLASS_1_3 )
		{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel2 + 3;}
			else if( lId == CLASS_1_1_1 )
			{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel3 + 1;}
			else if( lId == CLASS_1_1_2 )
			{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel3 + 2;}
			else if( lId == CLASS_1_1_3 )
			{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel3 + 3;}
			else if( lId == CLASS_1_1_4 )
			{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel3 + 4;}
				else if( lId == CLASS_1_1_1_1 )
				{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel4 + 1;}
				else if( lId == CLASS_1_1_2_2 )
				{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel4 + 2;}
				else if( lId == CLASS_1_1_3_3 )
				{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel4 + 3;}
				else if( lId == CLASS_1_1_3_4 )
				{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel4 + 4;}
				else if( lId == CLASS_1_1_1_5 )
				{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel4 + 5;}
					else if( lId == CLASS_1_1_1_1_1 )
					{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel5 + 1;}
					else if( lId == CLASS_1_1_1_1_2 )
					{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel5 + 2;}
					else if( lId == CLASS_1_1_1_1_3 )
					{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel5 + 3;}
					else if( lId == CLASS_1_1_1_1_4 )
					{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel5 + 4;}
					else if( lId == CLASS_1_1_1_1_5 )
					{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel5 + 5;}
					else if( lId == CLASS_1_1_1_1_6 )
					{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel5 + 6;}
					else if( lId == CLASS_1_1_1_1_7 )
					{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel5 + 7;}
						else if( lId == CLASS_1_1_1_1_1_1 )
						{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel6 + 1;}
						else if( lId == CLASS_1_1_1_1_1_2 )
						{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel6 + 2;}
						else if( lId == CLASS_1_1_1_1_1_3 )
						{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel6 + 3;}
						else if( lId == CLASS_1_1_1_1_1_4 )
						{wJobIdx = Fighter + ( ( Race + 1 ) * 100 ) + JobLevel6 + 4;}

	else if( lId == CLASS_2 )
	{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel1 + 1;}
		else if( lId == CLASS_2_1 )
		{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel2 + 1;}
		else if( lId == CLASS_2_2 )
		{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel2 + 2;}
		else if( lId == CLASS_2_3 )
		{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel2 + 3;}
			else if( lId == CLASS_2_1_1 )
			{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel3 + 1;}
			else if( lId == CLASS_2_2_1 )
			{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel3 + 2;}
			else if( lId == CLASS_2_1_2 )
			{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel3 + 3;}
			else if( lId == CLASS_2_1_4 )
			{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel3 + 4;}
				else if( lId == CLASS_2_1_1_1 )
				{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel4 + 1;}
				else if( lId == CLASS_2_1_3_1 )
				{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel4 + 2;}
				else if( lId == CLASS_2_2_1_1 )
				{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel4 + 3;}
				else if( lId == CLASS_2_1_2_1 )
				{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel4 + 4;}
				else if( lId == CLASS_2_1_1_5 )
				{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel4 + 5;}
					else if( lId == CLASS_2_1_1_1_1 )
					{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel5 + 1;}
					else if( lId == CLASS_2_1_1_1_2 )
					{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel5 + 2;}
					else if( lId == CLASS_2_1_1_1_3 )
					{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel5 + 3;}
					else if( lId == CLASS_2_1_1_1_4 )
					{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel5 + 4;}
					else if( lId == CLASS_2_1_1_1_5 )
					{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel5 + 5;}
					else if( lId == CLASS_2_1_1_1_6 )
					{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel5 + 6;}
						else if( lId == CLASS_2_1_1_1_1_1 )
						{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel6 + 1;}
						else if( lId == CLASS_2_1_1_1_1_2 )
						{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel6 + 2;}
						else if( lId == CLASS_2_1_1_1_1_3 )
						{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel6 + 3;}
						else if( lId == CLASS_2_1_1_1_1_4 )
						{wJobIdx = Rogue + ( ( Race + 1 ) * 100 ) + JobLevel6 + 4;}

	else if( lId == CLASS_3 )
	{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel1 + 1;}
		else if( lId == CLASS_3_1 )
		{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel2 + 1;}
		else if( lId == CLASS_3_2 )
		{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel2 + 2;}
		else if( lId == CLASS_3_3 )
		{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel2 + 3;}
			else if( lId == CLASS_3_1_1 )
			{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel3 + 1;}
			else if( lId == CLASS_3_2_1 )
			{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel3 + 2;}
			else if( lId == CLASS_3_3_1 )
			{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel3 + 3;}
			else if( lId == CLASS_3_1_4 )
			{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel3 + 4;}
				else if( lId == CLASS_3_1_1_1 )
				{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel4 + 1;}
				else if( lId == CLASS_3_2_1_1 )
				{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel4 + 2;}
				else if( lId == CLASS_3_3_1_1 )
				{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel4 + 3;}
				else if( lId == CLASS_3_1_1_2 )
				{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel4 + 4;}
				else if( lId == CLASS_3_1_1_5 )
				{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel4 + 5;}
					else if( lId == CLASS_3_1_1_1_1 )
					{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel5 + 1;}
					else if( lId == CLASS_3_1_1_1_2 )
					{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel5 + 2;}
					else if( lId == CLASS_3_1_1_1_3 )
					{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel5 + 3;}
					else if( lId == CLASS_3_1_1_1_4 )
					{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel5 + 4;}
					else if( lId == CLASS_3_1_1_1_5 )
					{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel5 + 5;}
					else if( lId == CLASS_3_1_1_1_6 )
					{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel5 + 6;}
						else if( lId == CLASS_3_1_1_1_1_1 )
						{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel6 + 1;}
						else if( lId == CLASS_3_1_1_1_1_2 )
						{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel6 + 2;}
						else if( lId == CLASS_3_1_1_1_1_3 )
						{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel6 + 3;}
						else if( lId == CLASS_3_1_1_1_1_4 )
						{wJobIdx = Mage + ( ( Race + 1 ) * 100 ) + JobLevel6 + 4;}

	else if( lId == CLASS_4 )
	{wJobIdx = Devil + ( ( Race + 1 ) * 100 ) + JobLevel1 + 1;}
	else if( lId == CLASS_4_1 )
	{wJobIdx = Devil + ( ( Race + 1 ) * 100 ) + JobLevel2 + 1;}
	else if( lId == CLASS_4_1_1 )
	{wJobIdx = Devil + ( ( Race + 1 ) * 100 ) + JobLevel3 + 1;}
	else if( lId == CLASS_4_1_1_1 )
	{wJobIdx = Devil + ( ( Race + 1 ) * 100 ) + JobLevel4 + 1;}
	else if( lId == CLASS_4_1_1_1_1 )
	{wJobIdx = Devil + ( ( Race + 1 ) * 100 ) + JobLevel5 + 1;}
	else if( lId == CLASS_4_1_1_1_1_1 )
	{wJobIdx = Devil + ( ( Race + 1 ) * 100 ) + JobLevel6 + 1;}
	...
	for( wCount = 0 ; wCount < pInfo->Count ; wCount++ )
	{
		...
		if( pSkillBase && lId == STD_SKILLTRAININGBTN)
		{
...
//		 : 습득 가능하도록 수정 함.
//-------------------------------------------------------------------------------------------------
void cSkillTrainingDlgJct::OpenDialog(LONG lId)
{
	// 스킬 리스트 다이얼로그 체크.
	if(!m_SkillListDlg) return ;
	//cWindow* DisableTrain	= GetWindowForID( GSKILL_TRAINOK );
//if ( lId == STD_SKILLTRAININGBTN )
//{
//
//	// 길드 모드를 FALSE로 세팅.
//	mGuildMode = FALSE;
...
//
//		AddSkillToSkillListDlg(byJobType, CurGrade, &wPos, lId) ;
//	}
//
...
//	}
//
//	DisableTrain->SetDisable(FALSE);
//	DisableTrain->SetHide(FALSE);
//
//	SetSkillInfo();
//	SetGold();
//	SetSkillPoint();
...
//{
	...
	const LONG count = m_SkillListDlg->GetItemCount();
	const DWORD top = m_SkillListDlg->GetTopListItemIdx(); //vs upgrade warning
	const int clickedRowIndex = m_SkillListDlg->GetClickedRowIdx();
	...
	for( WORD CurGrade = JobGrade; CurGrade > 0; CurGrade-- )
	{
		...
		if( lId == CLASS_1 ||
			...
			lId == CLASS_4_1_1_1 ||
			lId == CLASS_4_1_1_1_1 ||
			lId == CLASS_4_1_1_1_1_1)
		{
			//DisableTrain->SetDisable(TRUE);
			//DisableTrain->SetHide(TRUE);
			AddSkillToSkillListDlg(byJobType, CurGrade, &wPos, lId) ;
		}
		//else
		//{
...
void cSkillTrainingDlgJct::OnActionEvent(LONG lId, void * p, DWORD we)
{
	//if( we & WE_BTNCLICK )
	//{
		//switch(lId)
...
void cSkillTrainingDlgJct::SetSkillInfo()
{
	if( m_Index == -1 )
		return;
	...
	if( mGuildMode )
	{
		...
		//cWindow* window	= GetWindowForID( GSKILL_TRAINOK );
	...
	m_Icon->Init((LONG)(m_Icon->GetRelX()), (LONG)(m_Icon->GetRelY()), m_Icon->GetWidth(), m_Icon->GetHeight(), &BasicImage, GSKILL_TRAIN_ICON);
...
void cSkillTrainingDlgJct::SetGold()
{
	m_Gold->SetStaticText( AddComma( HERO->GetHeroTotalInfo()->Money ) );
}

void cSkillTrainingDlgJct::SetSkillPoint()
{
	char buf[256];
...
void cSkillTrainingDlgJct::Training()
{
	if( m_Index != -1 )
	{
		const DWORD skillIndex		= m_SkillList[ m_Index ].index;
		...
		//	msg.Protocol = MP_SKILLTREE_PDATE_SYN;
