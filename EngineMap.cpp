...
//	return g_pExecutive->LoadMapScript(szMapFile,NULL,LOAD_MAP_FLAG_DEFAULT_PROC_AUTOANIMATION);
	...
	char	buf[1024]; //Alemuri 1024
	memset(buf, 0, 1024); //Alemuri 1024
		...
		if (!lstrcmp(buf,PID_GX_OBJECT))
		{
			...
			for (DWORD i=0; i<dwGXObjectsNum; i++)
			{
				...
				if (gxo)
				{
					...
					{
						// Client에서는 Lock이 필요 없을듯 싶어서 무조건 끈다.
						m_pExecutive->UnLockTransform(gxo);
					}

					m_pExecutive->SetID(gxo,dwID); //dwID
...
void CEngineMap::Release()
{
	if(g_pExecutive)
	{
		...
		for(GXOBJECT_HANDLE handle = m_GXLightList.RemoveTail();
			0 < handle;
...
BOOL CEngineMap::ImportStaticModel(char* szFileName)
{
	...
	if (0xffffffff == m_pExecutive->GetGeometry()->CreateStaticModel(&pStaticModel,4000,500,0)) //Alemuri 4000,500,0
		return FALSE;
...
DWORD CEngineMap::GetMultipleObjectWithRay(PICK_GXOBJECT_DESC* pPickDesc, DWORD dwMaxDescNum, VECTOR3* pv3Pos, VECTOR3* pv3Dir, DWORD dwFlag)
{
	ASSERT(pPickDesc);

	return m_pStaticModel->GetMultipleMeshObjectWithRay(pPickDesc, dwMaxDescNum, pv3Pos, pv3Dir, dwFlag);
}