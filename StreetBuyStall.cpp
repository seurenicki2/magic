...
//	m_nCurSelectedItem = pos;
	...
	BUY_REG_INFO temp;
	MONEYTYPE money = m_MoneyArray[pos];
...
void CStreetBuyStall::OnDivideItem( LONG iId, void* p, DWORDEX param1, void * vData1, void * vData2 )
{
	...
	if( !STREETSTALLMGR->SellItemDur( DWORD(param1) ) )
	{		
		pStall->SellError();
...
void CStreetBuyStall::ChangeItemStatus( POSTYPE pos, WORD volume, MONEYTYPE money)
{
	...
	
	{
		...
		MONEYTYPE money = pItem->GetMoney();
	...
	{
		DWORD color = 0xffffffff;
		MONEYTYPE money = pItem->GetMoney() * pItem->GetVolume();
