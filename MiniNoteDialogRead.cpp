...
void CMiniNoteDialogRead::SetMiniNote(char* Title, char* Date, char* Sender, char* Note, WORD ItemIdx)
{
	...
	if( stricmp(Sender, "<CONSIGN>") == 0 )
	{
		...
		char name[256];
		DWORD kind, initNum, curNum;
		DWORDEX deposit, commission, price;

		kind		= atoi( strtok(pBuf, tokens) );
		strcpy(name, strtok(NULL, ","));
		initNum		= atoi( strtok(NULL, ",") );
		curNum		= atoi( strtok(NULL, ",") );
		deposit		= _atoi64( strtok(NULL, ",") );
		commission	= _atoi64( strtok(NULL, ",") );
		price		= _atoi64( strtok(NULL, ",") );
...
void CMiniNoteDialogRead::SetPackageInfo(MONEYTYPE dwMoney, ITEMBASE* pBaseInfo, ITEM_OPTION* pOptionInfo)
{
	m_dwPackageMoney = 0;
