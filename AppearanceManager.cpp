...
void CAppearanceManager::SetMonsterAppearance(CMonster* pMonster)
{
	...
	//////////////////////////////////////////////////////////////////////////
	//disable select/pick in listed monsterid
	if( (mkind >= 1178 && mkind <= 1223) || (mkind >= 300 && mkind <= 319) ) //summoned monster from skill
		pMonster->m_EngineObject.DisablePick();
	// 몬스터 크기 적용
	VECTOR3 Scale;
	// taiyo test
	Scale.x = Scale.y = Scale.z = pList->Scale;
...
VOID CAppearanceManager::CancelReservation(CObject* pObject)
{
	...
	if (pObject->GetObjectKind() == eObjectKind_Player)
	{
		...
		CPlayer* pPlayer = (CPlayer*)pObject;	
		if(pPlayer->GetMarryPlayerID() == HERO->GetID() )
		{
			HERO->EndMarryEffect();
		}
			...
			if (0 < m_lstReserveToAddPlayerList.GetCount())
			{
				...
				if (NULL != pReservItem)
				{
					...
					if (NULL != pPlayer)
					{
						...
						// Check PlayerID to Enable Love Effect
						if(pPlayer->GetMarryPlayerID() == HERO->GetID() )
						{							
							HERO->StartMarryEffect(pPlayer);
						}
...
void PostProcessAddPlayer(CPlayer* pPlayer, SEND_CHARACTER_TOTALINFO* pmsg)
{
	...
	if( pmsg->TotalInfo.bPKMode && MAP->GetMapNum() != PVP )
	{
		//OBJECTEFFECTDESC desc(FindEffectNum("maintain_PK_S.beff"));
		OBJECTEFFECTDESC desc( 39 );	//---KES PK
		OBJECTEFFECTDESC descPWar( FindEffectNum("Char_PK-war.beff") );
		if (pPlayer->GetCharacterTotalInfo()->isInPartyWar) //Use different effect than PK if in Party War
			pPlayer->AddObjectEffect( PK_EFFECT_ID, &descPWar, 1);
		else
