...
{ 
	DWORD toolTipColor = RGB_HALF(0, 0, 0); //Alemuri
	if( msg )
	{
		toolTip.Init( 0, color, image, toolTipColor ); //Alemuri replace imgColor with toolTipColor	//폰트 아이디는? 무조건 0인가
		toolTip.SetImageAlpha( TOOLTIP_ALPHA );	//200대충.. 조정하기.
		toolTip = msg;
	}
}

//Alemuri Tooltip with font-------------------------------------------------------------------------------------------------------
void cWindow::SetToolTipWithFont( int fontIdx, char* msg, DWORD color, cImage * image, DWORD imgColor )
{ 
	DWORD toolTipColor = RGB_HALF(0, 0, 0); //Alemuri
	if( msg )
	{
		toolTip.Init( fontIdx, color, image, toolTipColor ); //Alemuri replace imgColor with toolTipColor	//폰트 아이디는? 무조건 0인가
		toolTip.SetImageAlpha( TOOLTIP_ALPHA );	//200대충.. 조정하기.
		toolTip = msg;
	}
}
...
void cWindow::SetModifiedToolTip(char* msg, DWORD color, cImage * image, DWORD imgColor)
{
	if (msg)
	{
		toolTip.Init(0, color, image, imgColor, TRUE);	//폰트 아이디는? 무조건 0인가
		toolTip.SetImageAlpha(TOOLTIP_ALPHA);	//200대충.. 조정하기.
		toolTip = msg;
	}
}

void cWindow::AddToolTipLine(const char* msg, DWORD color, DWORD wItemIdx, int imageIndexArray)
{
	toolTip.AddLine(msg, color, wItemIdx, imageIndexArray);
}
