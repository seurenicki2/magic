...
/*
				...
				//{
				//	CHATMGR->AddMsg( CTC_SYSMSG, "Minimum required VIP Level 2, for use this shop"  );		// 상점을 먼저 닫으세요
				//	break;
				//}
				MSGBASE msg;
				ZeroMemory( &msg, sizeof(msg) );
...
/*
				...
				//{
				//	CHATMGR->AddMsg( CTC_SYSMSG, "Minimum required VIP Level 3, for use this shop"  );		// 상점을 먼저 닫으세요
				//	break;
				//}
		...
		case MSD_CIBTN_VIP_NOTE:
			{
				...
				//{
				//	CHATMGR->AddMsg( CTC_SYSMSG, "Minimum required VIP Level 1, for use this menu"  );		// 상점을 먼저 닫으세요
				//	break;
				//}
				pHandler = (cDialog*)GAMEIN->GetNoteDialog();
				if( !pHandler ) return ;
		...
		case MSD_CIBTN_VIP_WAREHOUSE: //Alemuri MapMove Info Dlg
			{
				...
				//if( cekvip->GetVIPstatus() < 1)
				//{
				//	CHATMGR->AddMsg( CTC_SYSMSG, "Minimum required VIP Level 1, for use this shop"  );		// 상점을 먼저 닫으세요
				//	break;
				//}
				...
				if(GAMEIN->GetStorageDialog()->IsItemInit())									// 창고 아이템의 아이템 초기화가 되었으면,
				{
					...
					if(OBJECTMGR->GetHero()->GetStorageNum() == 0)												// 개설한 창고가 없다면,
					{
...
/*					
				...
				if( pDlg )
				{
					if (pDlg->IsActive())
					{
						...
						msg.Category = MP_NOTE;															// 카테고리를 아이템으로 세팅한다.
						msg.Protocol = MP_NOTE_BROWSER_OPEN_SYN;										// 프로토콜을 아이템 샵 정보 싱크로 세팅한다.
